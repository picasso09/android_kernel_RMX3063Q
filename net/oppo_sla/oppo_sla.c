...
//#include <net/oppo/oppo_apps_monitor.h>
...
static void game_app_switch_network(u32 game_type)
{
	...
	if(!wlan_bad &&
	   game_uid[index].mark == CELLULAR_MARK && game_rx_bad){
	...
	if (oppo_sla_info[WLAN_INDEX].if_up || oppo_sla_info[WLAN_INDEX].need_up ||
	    ...
	    if (oppo_sla_debug) {
	        printk("oppo_sla:send_speed_and_rtt_to_user wlan:max_speed=%d, avg_rtt=%d  "
	               "cell:max_speed=%d, avg_rtt=%d, oppo_sla_enable=%d,tcp_rx = %llu, tcp_tx = %llu\n",
	               payload[0], payload[1], payload[2], payload[3], oppo_sla_enable,tcp_tx_rx[0], tcp_tx_rx[1]);
	    }
	}
	return;
}
