...
static int mmc_read_ext_csd(struct mmc_card *card)
{
	...
	if (get_boot_mode() == FACTORY_BOOT) { //ftm mode
		if((ext_csd[EXT_CSD_BKOPS_EN] & EXT_CSD_AUTO_BKOPS_MASK)) {
			err = __mmc_switch(card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_BKOPS_EN,
				   ext_csd[EXT_CSD_BKOPS_EN] & (~EXT_CSD_AUTO_BKOPS_MASK), card->ext_csd.generic_cmd6_time,
					   true, false, true);
			if(err) {
	...
	struct mmc_card *oldcard)
{
	...
	if (card->ext_csd.auto_bkops && !card->ext_csd.man_bkops_en) {
		if ((get_boot_mode() != FACTORY_BOOT) && (!card->ext_csd.auto_bkops_en)) {
			err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
