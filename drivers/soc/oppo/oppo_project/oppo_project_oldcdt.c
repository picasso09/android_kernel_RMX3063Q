...
static int __init update_feature(void)
{
	mm_segment_t fs;
	fs = get_fs();
	pr_err("%s: oldcdt -- Operator Version [%d]\n", __func__, (get_Operator_Version()));
	set_fs(KERNEL_DS);
	if (oppoVersion) {
		if ((get_project() == 18073)||
		    (get_Operator_Version() == OPERATOR_20701_RIO_D_THAILAND_NFC)||
		    (get_Operator_Version() == OPERATOR_20701_SEPARATE_All_BAND_NFC)||
		    (get_Operator_Version() == OPERATOR_20701_All_BAND_NFC)||
		    (get_Operator_Version() == OPERATOR_20701_RIO_D_All_BAND_VDF_NFC)) {
...
static int init_project_version_oldcdt(void)
{
	...
	ret = of_property_read_u32(np,"engVersion",&(projectInfo.nENGVersion));
	if(ret)
	{
...
/*chenguanhua@RM.BSP.Fingerprint.Secure 2020/08/28 Add main board*/
#define MAINBOARD_LEN 2
                ...
                size_t count, loff_t *off)
{
        ...
        ptr += strlen("board_id=");
        board_id = simple_strtol(ptr, NULL, 10);
        main_board = (board_id & 0xff);
        printk("%s, main_board=%X\n", __func__, main_board);

        len = sprintf(page, "%X", main_board);
        if (len > *off) {
                len -= *off;
        }
        else{
...
struct file_operations mainboard_proc_fops = {
