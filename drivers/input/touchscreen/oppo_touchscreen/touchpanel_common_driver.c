...
static void init_parse_dts(struct device *dev, struct touchpanel_data *ts)
{
    ...
    /*add for hmx83112a & FT8722 start */
    if(strstr(boot_command_line, "hx83112a_txd_boe_fives")){
        rc = of_property_read_u32_array(np, "touchpanel_HX83112A,tx-rx-num", tx_rx_num, 2);
        TPD_INFO("touchpanel_HX83112A,tx-rx-num \n");
    }else if(strstr(boot_command_line, "hx83112a_txd_boe_new_fives")){
        rc = of_property_read_u32_array(np, "touchpanel_HX83112A,tx-rx-num", tx_rx_num, 2);
        TPD_INFO("touchpanel_HX83112A,tx-rx-num \n");
	}else if(strstr(boot_command_line, "ft8722_hlt_boe_eighth")){
        rc = of_property_read_u32_array(np, "touchpanel_FT8722,tx-rx-num", tx_rx_num, 2);
        TPD_INFO("touchpanel_FT8722,tx-rx-num \n");
    }else{
        rc = of_property_read_u32_array(np, "touchpanel,tx-rx-num", tx_rx_num, 2);
        TPD_INFO("touchpanel,tx-rx-num \n");
    }
    /*add for hmx83112a FT8722  end */
