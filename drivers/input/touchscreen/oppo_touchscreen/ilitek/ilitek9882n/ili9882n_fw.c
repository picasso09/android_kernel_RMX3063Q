...
static int ilitek_tddi_fw_ili_convert(u8 *pfw)
{
    ...
    if(!ilits->p_firmware_headfile->firmware_data || ilits->p_firmware_headfile->firmware_size <= ILI_FILE_HEADER
       || ilits->p_firmware_headfile->firmware_size >= MAX_HEX_FILE_SIZE) {
...
int ili_fw_upgrade(int op)
{
	...
	if (ilits->oppo_fw_update || ilits->force_fw_update || ERR_ALLOC_MEM(pfw)) {
