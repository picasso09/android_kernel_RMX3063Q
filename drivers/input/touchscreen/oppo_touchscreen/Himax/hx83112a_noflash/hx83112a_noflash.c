...
struct hx83112a_report_data *hx83112a_hx_touch_data;
struct chip_data_hx83112b *hx83112a_g_chip_info;
int hx83112a_touch_data_size = 128;
int hx83112a_HX_HW_RESET_ACTIVATE = 0;
static int HX_TOUCH_INFO_POINT_CNT   = 0;
int hx83112a_g_lcd_vendor = 0;
int hx83112a_irq_en_cnt = 0;

int hx83112a_g_1kind_raw_size = 0;
uint32_t hx83112a_hx83112a_g_rslt_data_len;
char *hx83112a_g_rslt_data;
int **hx83112a_nf_inspection_criteria;
int *hx83112a_nf_inspt_crtra_flag;
int hx83112a_HX_CRITERIA_ITEM = 4;
int hx83112a_HX_CRITERIA_SIZE;
char *hx83112a_g_file_path_OK;
char *hx83112a_g_file_path_NG;
bool hx83112a_isEA006proj = false;
bool hx83112a_isBD12proj = false;
bool hx83112a_isRead_csv = true;
int hx83112a_nf_fail_write_count;

#ifdef HX_ZERO_FLASH
extern int hx83112a_g_f_0f_updat;
/* 128k+ */
...
int hx83112a_check_point_format;
unsigned char hx83112a_switch_algo;
uint8_t hx83112a_HX_PROC_SEND_FLAG;

extern unsigned char HX_IC_TYPE;

/*******Part0: SPI Interface***************/
#ifdef CONFIG_TOUCHPANEL_MTK_PLATFORM
const struct mtk_chip_config hx83112a_hx_spi_ctrdata = {
    .rx_mlsb = 1,
    .tx_mlsb = 1,
    .cs_pol = 0,
	.cs_setuptime = 35,
...
#else
const struct mt_chip_conf hx83112a_hx_spi_ctrdata = {
    .setuptime = 25,
    .holdtime = 25,
    .high_time = 3, /* 16.6MHz */
    .low_time = 3,
...
static ssize_t hx83112a_spi_sync(struct touchpanel_data *ts, struct spi_message *message)
{
    int status;
...
static int hx83112a_spi_read(uint8_t *command, uint8_t command_len, uint8_t *data, uint32_t length, uint8_t toRetry)
{
    ...
    for (retry = 0; retry < toRetry; retry++) {
        error = spi_sync(hx83112a_private_ts->s_client, &message);
        if (error) {
...
static int hx83112a_spi_write(uint8_t *buf, uint32_t length)
{
    ...
    return hx83112a_spi_sync(hx83112a_private_ts, &m);

}
#endif // end of CONFIG_VMAP_STACK

static int hx83112a_bus_read(uint8_t command, uint32_t length, uint8_t *data)
{
    int result = 0;
    uint8_t spi_format_buf[3];

    mutex_lock(&(hx83112a_g_chip_info->spi_lock));
    spi_format_buf[0] = 0xF3;
    spi_format_buf[1] = command;
    spi_format_buf[2] = 0x00;
#ifdef CONFIG_VMAP_STACK
    result = spi_write_then_read(hx83112a_private_ts->s_client, &spi_format_buf[0], 3, data, length);
#else
    result = hx83112a_spi_read(&spi_format_buf[0], 3, data, length, 10);
#endif  // end of CONFIG_VMAP_STACK
    mutex_unlock(&(hx83112a_g_chip_info->spi_lock));

    return result;
}

static int hx83112a_bus_write(uint8_t command, uint32_t length, uint8_t *data)
{
...
#ifdef CONFIG_VMAP_STACK
    result = spi_write_then_read(hx83112a_private_ts->s_client, &spi_format_buf[0], length + 2, &spi_format_buf[0], 0);
#else
    result = hx83112a_spi_write(spi_format_buf, length + 2);
#endif  // end of CONFIG_VMAP_STACK
    mutex_unlock(&(hx83112a_g_chip_info->spi_lock));
...
static uint32_t hx83112a_hw_check_CRC(uint8_t *start_addr, int reload_length);
...
static void hx83112a_read_FW_ver(void);
static int hx83112b_resetgpio_set(struct hw_resource *hw_res, bool on);

/*******Part2:Call Back Function implement*******/

/* add for himax */
void hx83112a_flash_write_burst(uint8_t *reg_byte, uint8_t *write_data)
{
    uint8_t data_byte[8];
    int i = 0;
    int j = 0;
    ...
    if (hx83112a_bus_write(0x00, 8, data_byte) < 0) {
        TPD_INFO("%s: i2c access fail!\n", __func__);
        return;
    }
}

void hx83112a_flash_write_burst_length(uint8_t *reg_byte, uint8_t *write_data, int length)
{
    uint8_t *data_byte;
    ...
    if (hx83112a_bus_write(0, length + 4, data_byte) < 0) {
        TPD_INFO("%s: i2c access fail!\n", __func__);
    }
    kfree(data_byte);
}

void hx83112a_burst_enable(uint8_t auto_add_4_byte)
{
    uint8_t tmp_data[4];
    tmp_data[0] = 0x31;

    if (hx83112a_bus_write(0x13, 1, tmp_data) < 0) {
        TPD_INFO("%s: i2c access fail!\n", __func__);
        return;
    }

    tmp_data[0] = (0x10 | auto_add_4_byte);
    if (hx83112a_bus_write(0x0D, 1, tmp_data) < 0) {
        TPD_INFO("%s: i2c access fail!\n", __func__);
        return;
    }
    /*isBusrtOn = true;*/
...
void hx83112a_register_read(uint8_t *read_addr, int read_length, uint8_t *read_data, bool hx83112a_cfg_flag)
{
    uint8_t tmp_data[4];
    int ret;
    if(hx83112a_cfg_flag == false) {
        if(read_length > 256) {
            TPD_INFO("%s: read len over 256!\n", __func__);
            return;
        }
        if (read_length > 4) {
            hx83112a_burst_enable(1);
        } else {
            hx83112a_burst_enable(0);
        }
        ...
        ret = hx83112a_bus_write(0x00, 4, tmp_data);
        if (ret < 0) {
            TPD_INFO("%s: i2c access fail!\n", __func__);
            return;
        }
        tmp_data[0] = 0x00;
        ret = hx83112a_bus_write(0x0C, 1, tmp_data);
        if (ret < 0) {
            TPD_INFO("%s: i2c access fail!\n", __func__);
            return;
        }

        if (hx83112a_bus_read(0x08, read_length, read_data) < 0) {
            TPD_INFO("%s: i2c access fail!\n", __func__);
            return;
        }
        if (read_length > 4) {
    ...
    } else if(hx83112a_cfg_flag == true) {
        if(hx83112a_bus_read(read_addr[0], read_length, read_data) < 0) {
            TPD_INFO("%s: i2c access fail!\n", __func__);
            return;
        }
    } else {
        TPD_INFO("%s: hx83112a_cfg_flag = %d, value is wrong!\n", __func__, hx83112a_cfg_flag);
        return;
    }
}

void hx83112a_register_write(uint8_t *write_addr, int write_length, uint8_t *write_data, bool hx83112a_cfg_flag)
{
    int i = 0;
    int address = 0;
    if (hx83112a_cfg_flag == false) {
        ...
        for (i = address; i < address + write_length; i++) {
            ...
            hx83112a_flash_write_burst_length(write_addr, write_data, write_length);
        }
    } else if(hx83112a_cfg_flag == true) {
        if(hx83112a_bus_write(write_addr[0], write_length, write_data) < 0) {
            TPD_INFO("%s: i2c access fail!\n", __func__);
            return;
        }
    } else {
...
static int hx83112a_mcu_register_write(uint8_t *write_addr, uint32_t write_length, uint8_t *write_data, uint8_t hx83112a_cfg_flag)
{
    ...
    for (i = 0; i < (total_read_times); i++) {
        if (total_size_temp >= max_bus_size) {
            ...
            hx83112a_flash_write_burst_length (tmp_addr, tmp_data, max_bus_size);
...
bool hx83112a_sense_off(void)
{
    ...
    tmp_data[1] = 0x00;
    tmp_data[0] = 0xA5;
    hx83112a_flash_write_burst(tmp_addr, tmp_data);
    ...
    do {
        ...
        //===========================================
        tmp_data[0] = 0x27;
        if (hx83112a_bus_write(0x31, 1, tmp_data) < 0) {
            TPD_INFO("%s: i2c access fail!\n", __func__);
            return false;
        }
        //===========================================
        ...
        //===========================================
        tmp_data[0] = 0x95;
        if (hx83112a_bus_write(0x32, 1, tmp_data) < 0) {
            TPD_INFO("%s: i2c access fail!\n", __func__);
            return false;
        }
        ...
        tmp_addr[1] = 0x00;
        tmp_addr[0] = 0xA8;
        hx83112a_register_read(tmp_addr, 4, tmp_data, false);
...
bool hx83112a_sense_off_mptest(void)
{
    uint8_t cnt = 0;
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
bool hx83112a_enter_safe_mode(void)
{
    uint8_t cnt = 0;
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
void hx83112a_interface_on(void)
{
    uint8_t tmp_data[5];
    uint8_t tmp_data2[2];
    int cnt = 0;
    ...
    if ( hx83112a_bus_read(0x08, 4, tmp_data) < 0) { // to knock I2C
        TPD_INFO("%s: i2c access fail!\n", __func__);
        return;
    }
    ...
    do {
        ...
        // Check cmd
        hx83112a_bus_read(0x13, 1, tmp_data);
        hx83112a_bus_read(0x0D, 1, tmp_data2);
...
void hx83112a_diag_register_set(uint8_t diag_command)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
bool hx83112a_wait_wip(int Timing)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    ...
    do {
        ...
        tmp_addr[1] = 0x00;
        tmp_addr[0] = 0x2C;
        hx83112a_register_read(tmp_addr, 4, in_buffer, false);
...
void hx83112a_sense_on(uint8_t FlashMode)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    int retry = 0;
    ...
    if (!FlashMode) {
        ...
        tmp_data[1] = 0x00;
        tmp_data[0] = 0x55;
        hx83112a_register_write(tmp_addr, 4, tmp_data, false);
    } else {
        do {
            tmp_addr[3] = 0x90;
            tmp_addr[2] = 0x00;
...
static int hx83112b_enable_interrupt(struct chip_data_hx83112b *chip_info, bool enable)
{
    ...
    if (enable == true && hx83112a_irq_en_cnt == 0) {
        enable_irq(chip_info->hx_irq);
        hx83112a_irq_en_cnt = 1;
    } else if (enable == false && hx83112a_irq_en_cnt == 1) {
        disable_irq_nosync(chip_info->hx_irq);
        hx83112a_irq_en_cnt = 0;
    } else {
        TPD_DETAIL("irq is not pairing! enable= %d, cnt = %d\n", enable, hx83112a_irq_en_cnt);
    }
...
#ifdef HX_ZERO_FLASH
struct hx83112a_core_fp hx83112a_g_core_fp;
struct zf_operation *hx83112a_pzf_op = NULL;
//char *i_CTPM_firmware_name = "Himax_firmware.bin";
bool hx83112a_g_auto_update_flag = false;
int hx83112a_G_POWERONOF = 1;

void hx83112a_in_parse_assign_cmd(uint32_t addr, uint8_t *cmd, int len)
{
    /*TPD_INFO("%s: Entering!\n", __func__);*/
    switch (len) {
    case 1:
...
void hx83112a_hx_update_dirly_0f(void)
{
    TPD_INFO("It will update fw after esd event in zero flash mode!\n");
    hx83112a_g_core_fp.fp_0f_operation_dirly();
}

void hx83112a_mcu_sys_reset(void)
{
    hx83112a_register_write (hx83112a_pzf_op->addr_system_reset,  4,  hx83112a_pzf_op->data_system_reset,  false);
}

int hx83112a_hx_dis_rload_0f(int disable)
{
    /*Diable Flash Reload*/
    int retry = 10;
    int check_val = 0;
    ...
    do {
        hx83112a_flash_write_burst (hx83112a_pzf_op->addr_dis_flash_reload,  hx83112a_pzf_op->data_dis_flash_reload);
        hx83112a_register_read(hx83112a_pzf_op->addr_dis_flash_reload, 4, tmp_data, false);
...
void hx83112a_mcu_clean_sram_0f(uint8_t *addr, int write_len, int type)
{
    int total_read_times = 0;
    ...
    for (i = 0; i < (total_read_times); i++) {
        ...
        if (total_size_temp >= max_bus_size) {
            ...
            hx83112a_flash_write_burst_length(tmp_addr, tmp_data,  total_size_temp % max_bus_size);
        }
        address = ((i + 1) * max_bus_size);
...
void hx83112a_mcu_write_sram_0f(const struct firmware *fw_entry, uint8_t *addr, int start_index, uint32_t write_len)
{
    int total_read_times = 0;
    int max_bus_size = MAX_TRANS_SZ;
    int total_size_temp = 0;
    ...
    if (hx83112a_g_chip_info->tmp_data == NULL) {
        //TPD_INFO("%s,  enteralloc hx83112a_g_chip_info->tmp_data\n", __func__);
        hx83112a_g_chip_info->tmp_data = kzalloc (sizeof (uint8_t) * firmware_update_space, GFP_KERNEL);
        if (hx83112a_g_chip_info->tmp_data == NULL) {
            TPD_INFO("%s, alloc hx83112a_g_chip_info->tmp_data failed\n", __func__);
            return ;
        }
        //TPD_INFO("%s, end---------alloc hx83112a_g_chip_info->tmp_data\n", __func__);
    }
    memcpy (hx83112a_g_chip_info->tmp_data, fw_entry->data, total_size);
    /*
    for (i = 0;i < 10;i++) {
    ...
    */
    ...
    for (i = 0; i < (total_read_times); i++) {
        ...
        */
        if (total_size_temp >= max_bus_size) {
            hx83112a_flash_write_burst_length(tmp_addr, &(hx83112a_g_chip_info->tmp_data[start_index + i * max_bus_size]),  max_bus_size);
            total_size_temp = total_size_temp - max_bus_size;
        } else {
            TPD_DETAIL("last total_size_temp=%d\n", total_size_temp);
            hx83112a_flash_write_burst_length(tmp_addr, &(hx83112a_g_chip_info->tmp_data[start_index + i * max_bus_size]),  total_size_temp % max_bus_size);
        }
    ...
    TPD_DETAIL("%s, ----END \n", __func__);
    //kfree (tmp_data);
    memset(hx83112a_g_chip_info->tmp_data, 0, total_size);
}
int hx83112a_sram_write_crc_check(const struct firmware *fw_entry, uint8_t *addr, int strt_idx, uint32_t len)
{
    int retry = 0;
    int crc = -1;

    do {
        hx83112a_g_core_fp.fp_write_sram_0f(fw_entry, addr, strt_idx, len);
        crc = hx83112a_hw_check_CRC (hx83112a_pzf_op->data_sram_start_addr, HX_48K_SZ);
        retry++;
...
static int hx83112a_mcu_Calculate_CRC_with_AP(unsigned char *FW_content, int CRC_from_FW, int len)
{
    int i, j, length = 0;
    int fw_data;
    int fw_data_2;
...
bool hx83112a_hx_parse_bin_cfg_data(const struct firmware *fw_entry)
{
    int part_num = 0;
    int i = 0;
    uint8_t buf[16];
    ...
    hx83112a_nf_cfg_crc = hx83112a_mcu_Calculate_CRC_with_AP(hx83112a_nf_FW_buf, 0, hx83112a_nf_cfg_sz);
...
static int hx83112b_nf_zf_part_info(const struct firmware *fw_entry)
{
    ...
    if (!hx83112a_hx_parse_bin_cfg_data(fw_entry))
        TPD_INFO("%s, Parse cfg from bin failed\n", __func__);
    hx83112a_register_write(hx83112a_pzf_op->addr_system_reset, 4, hx83112a_pzf_op->data_system_reset, false);
    hx83112a_enter_safe_mode();
    getnstimeofday(&timeStart);
    /* first 48K */

    hx83112a_sram_write_crc_check(fw_entry, hx83112a_pzf_op->data_sram_start_addr, 0, HX_48K_SZ);
    crc = hx83112a_hw_check_CRC(hx83112a_pzf_op->data_sram_start_addr, HX_48K_SZ);
    ret = (crc == 0) ? true : false;
    if (crc != 0)
        TPD_INFO("48k CRC Failed! CRC = %X", crc);
    do {
        hx83112a_mcu_register_write(hx83112a_nf_sram_min, hx83112a_nf_cfg_sz, hx83112a_nf_FW_buf, 0);
        /*hx83112a_register_write(hx83112a_nf_sram_min, hx83112a_nf_cfg_sz, hx83112a_nf_FW_buf, 0); */
        crc = hx83112a_hw_check_CRC(hx83112a_nf_sram_min, hx83112a_nf_cfg_sz);
        if (crc != hx83112a_nf_cfg_crc)
            TPD_INFO("Config CRC FAIL, HW CRC = %X, SW CRC = %X, retry time = %d", crc, hx83112a_nf_cfg_crc, retry);
        retry++;
    } while (!ret && retry < 10);
    if (hx83112a_G_POWERONOF == 1)
        hx83112a_g_core_fp.fp_write_sram_0f(fw_entry, hx83112a_pzf_op->data_mode_switch, 0xC33C, 4);
    else
        hx83112a_g_core_fp.fp_clean_sram_0f(hx83112a_pzf_op->data_mode_switch, 4, 2);
...
void hx83112a_mcu_firmware_update_0f(const struct firmware *fw_entry)
{
    int retry = 0;
    int crc = -1;
    int ret = 0;
...
fw_reload:
    if (fw_entry == NULL || reload) {
        ...
        if (hx83112a_g_chip_info->g_fw_sta) {
            TPD_INFO("request firmware failed, get from g_fw_buf\n");
            request_fw_headfile->size = hx83112a_g_chip_info->g_fw_len;
            request_fw_headfile->data = hx83112a_g_chip_info->g_fw_buf;
            tmp_fw_entry = request_fw_headfile;

        } else {
            TPD_INFO("request firmware failed, get from headfile\n");
            if(hx83112a_g_chip_info->p_firmware_headfile->firmware_data) {
                request_fw_headfile->size = hx83112a_g_chip_info->p_firmware_headfile->firmware_size;
                request_fw_headfile->data = hx83112a_g_chip_info->p_firmware_headfile->firmware_data;
                tmp_fw_entry = request_fw_headfile;
                hx83112a_g_chip_info->using_headfile = true;
            } else {
    ...
    if ((int)tmp_fw_entry->size > HX64K) {
        ...
        /* first 48K */
        do {
            hx83112a_g_core_fp.fp_write_sram_0f (tmp_fw_entry, hx83112a_pzf_op->data_sram_start_addr, 0, HX_48K_SZ);
            crc = hx83112a_hw_check_CRC (hx83112a_pzf_op->data_sram_start_addr,  HX_48K_SZ);
            ...
            temp_addr[1] = 0xBF;
            temp_addr[0] = 0xFC;
            hx83112a_register_read(temp_addr, 4, temp_data, false);
            ...
            retry++;
        } while (((temp_data[3] == 0 && temp_data[2] == 0 && temp_data[1] == 0 && temp_data[0] == 0 && retry < 80) || (crc != 0 && retry < 30)) && !(hx83112a_g_chip_info->using_headfile && retry < 3));
        ...
        /* if hx83112a_G_POWERONOF!= 1, it will be clean mode! */
        /*config and setting */
        /*config info*/
        if (hx83112a_G_POWERONOF == 1) {
            retry = 0;
            do {
                hx83112a_g_core_fp.fp_write_sram_0f(tmp_fw_entry, hx83112a_pzf_op->data_cfg_info, 0xC000, 128);//132
                crc = hx83112a_hw_check_CRC(hx83112a_pzf_op->data_cfg_info, 128);
                if (crc == 0) {
                ...
                retry++;
            } while ((crc != 0 && retry < 30) || (hx83112a_g_chip_info->using_headfile && retry < 15));
            ...
            hx83112a_g_core_fp.fp_clean_sram_0f(hx83112a_pzf_op->data_cfg_info, 128, 2);
        }
        /*FW config*/
        if (hx83112a_G_POWERONOF == 1) {
            retry = 0;
            do {
                hx83112a_g_core_fp.fp_write_sram_0f(tmp_fw_entry, hx83112a_pzf_op->data_fw_cfg_p1, 0xC100, 528);//482
                crc = hx83112a_hw_check_CRC(hx83112a_pzf_op->data_fw_cfg_p1, 528);
                if (crc == 0) {
            ...
            hx83112a_g_core_fp.fp_clean_sram_0f(hx83112a_pzf_op->data_fw_cfg_p1, 528, 1);
        }

        if (hx83112a_G_POWERONOF == 1) {
            retry = 0;
            do {
                hx83112a_g_core_fp.fp_write_sram_0f(tmp_fw_entry, hx83112a_pzf_op->data_fw_cfg_p3, 0xCA00, 128);
                crc = hx83112a_hw_check_CRC(hx83112a_pzf_op->data_fw_cfg_p3, 128);
                if (crc == 0) {
            ...
            hx83112a_g_core_fp.fp_clean_sram_0f(hx83112a_pzf_op->data_fw_cfg_p3, 128, 1);
        }
        ...
        if (hx83112a_G_POWERONOF == 1) {
            retry = 0;
            do {
                hx83112a_g_core_fp.fp_write_sram_0f(tmp_fw_entry, hx83112a_pzf_op->data_adc_cfg_1, 0xD640, 1200);
                crc = hx83112a_hw_check_CRC(hx83112a_pzf_op->data_adc_cfg_1, 1200);
                if (crc == 0) {
            ...
            hx83112a_g_core_fp.fp_clean_sram_0f(hx83112a_pzf_op->data_adc_cfg_1, 1200, 2);
        }

        if (hx83112a_G_POWERONOF == 1) {
            retry = 0;
            do {
                hx83112a_g_core_fp.fp_write_sram_0f(tmp_fw_entry, hx83112a_pzf_op->data_adc_cfg_2, 0xD320, 800);
                crc = hx83112a_hw_check_CRC(hx83112a_pzf_op->data_adc_cfg_2, 800);
                if (crc == 0) {
            ...
            hx83112a_g_core_fp.fp_clean_sram_0f(hx83112a_pzf_op->data_adc_cfg_2, 800, 2);
        }
        ...
        if (hx83112a_G_POWERONOF == 1) {
            retry = 0;
            do {
                hx83112a_g_core_fp.fp_write_sram_0f(tmp_fw_entry, hx83112a_pzf_op->data_map_table, 0xE000, 1536);
                crc = hx83112a_hw_check_CRC(hx83112a_pzf_op->data_map_table, 1536);
                if (crc == 0) {
            ...
            hx83112a_g_core_fp.fp_clean_sram_0f(hx83112a_pzf_op->data_map_table, 1536, 2);
        }
    }

    /* switch mode*/
    if (hx83112a_G_POWERONOF == 1) {
        hx83112a_g_core_fp.fp_write_sram_0f(tmp_fw_entry, hx83112a_pzf_op->data_mode_switch, 0xC30C, 4);
    } else {
        hx83112a_g_core_fp.fp_clean_sram_0f(hx83112a_pzf_op->data_mode_switch, 4, 2);
    }

    hx83112b_fw_check(hx83112a_private_ts->chip_data, &hx83112a_private_ts->resolution_info, &hx83112a_private_ts->panel_data);
...
int hx83112a_hx_0f_op_file_dirly(char *file_name)
{
    int err = NO_ERR;
    ...
    if (hx83112a_private_ts->fw_update_app_support) {
        err = request_firmware_select (&fw_entry, file_name, hx83112a_private_ts->dev);
    } else {
        err = request_firmware (&fw_entry, file_name, hx83112a_private_ts->dev);
    }
    if (err < 0) {
    ...
    //hx83112a_int_enable (0);

    if(hx83112a_g_f_0f_updat == 1) {
        ...
        hx83112a_g_f_0f_updat = 1;
    }

    hx83112b_enable_interrupt(hx83112a_g_chip_info, false);

    /* trigger reset */
    hx83112b_resetgpio_set(hx83112a_g_chip_info->hw_res, false); // reset gpio
    hx83112b_resetgpio_set(hx83112a_g_chip_info->hw_res, true); // reset gpio

    hx83112a_g_core_fp.fp_firmware_update_0f (fw_entry);
    release_firmware (fw_entry);

    hx83112a_g_f_0f_updat = 0;
    TPD_INFO("%s, END \n", __func__);
    return err;
}
int hx83112a_mcu_0f_operation_dirly(void)
{
    int err = NO_ERR;
    //const struct firmware *fw_entry = NULL;
    ...
    hx83112a_g_core_fp.fp_firmware_update_0f(NULL);
    //release_firmware (hx83112a_g_chip_info->g_fw_entry);
...
int hx83112a_mcu_0f_operation_test_dirly(char *fw_name)
{
    int err = NO_ERR;
    const struct firmware *fw_entry = NULL;
    ...
    err = request_firmware (&fw_entry, fw_name, hx83112a_private_ts->dev);
    if (err < 0) {
        TPD_INFO("%s, fail in line%d error code=%d, file maybe fail\n", __func__, __LINE__, err);
        if (fw_entry != NULL) {
            release_firmware(fw_entry);
...
void hx83112a_mcu_0f_operation(struct work_struct *work)
{
    TPD_INFO("%s, Entering \n", __func__);

#ifdef CONFIG_TOUCHPANEL_MTK_PLATFORM
    if (hx83112a_private_ts->boot_mode != RECOVERY_BOOT  && !is_oem_unlocked())
#else
    if (hx83112a_private_ts->boot_mode != MSM_BOOT_MODE__RECOVERY  && !is_oem_unlocked())
#endif
    {
        TPD_INFO("file name = %s\n", hx83112a_private_ts->panel_data.fw_name);
    ...
    hx83112a_g_core_fp.fp_reload_disable(0);
    msleep (10);
    hx83112a_read_FW_ver();
    msleep (10);
    hx83112a_sense_on(0x00);
    msleep (10);
    TPD_INFO("%s:End \n", __func__);

#ifdef CONFIG_OPPO_TP_APK
    if(hx83112a_g_chip_info->debug_mode_sta) {
        if(hx83112a_private_ts->apk_op && hx83112a_private_ts->apk_op->apk_debug_set) {
            hx83112a_private_ts->apk_op->apk_debug_set(hx83112a_private_ts->chip_data, true);
        }
    }
#endif // end of CONFIG_OPPO_TP_APK

    hx83112b_enable_interrupt(hx83112a_g_chip_info, true);
...
#ifdef HX_0F_DEBUG
void hx83112a_mcu_read_sram_0f(const struct firmware *fw_entry, uint8_t *addr, int start_index, int read_len)
{
    int total_read_times = 0;
    int max_bus_size = MAX_TRANS_SZ;
    int total_size_temp = 0;
    ...
    for (i = 0; i < (total_read_times); i++) {
        if (total_size_temp >= max_bus_size) {
            hx83112a_register_read(tmp_addr, max_bus_size, &temp_info_data[i * max_bus_size], false);
            total_size_temp = total_size_temp - max_bus_size;
        } else {
            hx83112a_register_read(tmp_addr, total_size_temp % max_bus_size, &temp_info_data[i * max_bus_size], false);
        }
    ...
    for (i = 0; i < read_len; i++, j++) {
        if (hx83112a_g_chip_info->g_fw_buf[j] != temp_info_data[i]) {
            not_same++;
            not_same_buff[i] = 1;
        }
    ...
    if (not_same != 0) {
        ...
        for (i = 0; i < read_len; i++, j++) {
            if (not_same_buff[i] == 1) {
                TPD_INFO("bin = [%d] 0x%2.2X\n", i, hx83112a_g_chip_info->g_fw_buf[j]);
            }
        }
        for (i = 0; i < read_len; i++, j++) {
            if (not_same_buff[i] == 1) {
...
void hx83112a_mcu_read_all_sram(uint8_t *addr, int read_len)
{
    int total_read_times = 0;
...
void hx83112a_mcu_firmware_read_0f(const struct firmware *fw_entry, int type)
{
    uint8_t tmp_addr[4] = {0};

    TPD_INFO("%s, Entering \n", __func__);
    if (type == 0) { /* first 48K */
        hx83112a_g_core_fp.fp_read_sram_0f (fw_entry, hx83112a_pzf_op->data_sram_start_addr, 0, HX_48K_SZ);
        hx83112a_g_core_fp.fp_read_all_sram (tmp_addr, 0xC000);
    } else { /*last 16k*/
        hx83112a_g_core_fp.fp_read_sram_0f (fw_entry, hx83112a_pzf_op->data_cfg_info, 0xC000, 132);
        hx83112a_g_core_fp.fp_read_sram_0f (fw_entry, hx83112a_pzf_op->data_fw_cfg, 0xC0FE, 512);
        hx83112a_g_core_fp.fp_read_sram_0f (fw_entry, hx83112a_pzf_op->data_adc_cfg_1, 0xD000, 376);
        hx83112a_g_core_fp.fp_read_sram_0f (fw_entry, hx83112a_pzf_op->data_adc_cfg_2, 0xD178, 376);
        hx83112a_g_core_fp.fp_read_sram_0f (fw_entry, hx83112a_pzf_op->data_adc_cfg_3, 0xD000, 376);
        hx83112a_g_core_fp.fp_read_all_sram (hx83112a_pzf_op->data_sram_clean, HX_32K_SZ);
    }
    TPD_INFO("%s, END \n", __func__);
}

void hx83112a_mcu_0f_operation_check(int type)
{


    TPD_INFO("%s, Entering \n", __func__);

    TPD_INFO("first 4 bytes 0x%2X, 0x%2X, 0x%2X, 0x%2X !\n", hx83112a_g_chip_info->g_fw_buf[0], hx83112a_g_chip_info->g_fw_buf[1], hx83112a_g_chip_info->g_fw_buf[2], hx83112a_g_chip_info->g_fw_buf[3]);
    TPD_INFO("next 4 bytes 0x%2X, 0x%2X, 0x%2X, 0x%2X !\n", hx83112a_g_chip_info->g_fw_buf[4], hx83112a_g_chip_info->g_fw_buf[5], hx83112a_g_chip_info->g_fw_buf[6], hx83112a_g_chip_info->g_fw_buf[7]);
    TPD_INFO("and next 4 bytes 0x%2X, 0x%2X, 0x%2X, 0x%2X !\n", hx83112a_g_chip_info->g_fw_buf[8], hx83112a_g_chip_info->g_fw_buf[9], hx83112a_g_chip_info->g_fw_buf[10], hx83112a_g_chip_info->g_fw_buf[11]);

    hx83112a_g_core_fp.fp_firmware_read_0f(NULL, type);
...
int hx83112a_hx_0f_init(void)
{
    hx83112a_pzf_op = kzalloc(sizeof(struct zf_operation), GFP_KERNEL);

    hx83112a_g_core_fp.fp_reload_disable = hx83112a_hx_dis_rload_0f;
    hx83112a_g_core_fp.fp_sys_reset = hx83112a_mcu_sys_reset;
    hx83112a_g_core_fp.fp_clean_sram_0f = hx83112a_mcu_clean_sram_0f;
    hx83112a_g_core_fp.fp_write_sram_0f = hx83112a_mcu_write_sram_0f;
    hx83112a_g_core_fp.fp_firmware_update_0f = hx83112a_mcu_firmware_update_0f;
    hx83112a_g_core_fp.fp_0f_operation = hx83112a_mcu_0f_operation;
    hx83112a_g_core_fp.fp_0f_operation_dirly = hx83112a_mcu_0f_operation_dirly;
    hx83112a_g_core_fp.fp_0f_op_file_dirly = hx83112a_hx_0f_op_file_dirly;
#ifdef HX_0F_DEBUG
    hx83112a_g_core_fp.fp_read_sram_0f = hx83112a_mcu_read_sram_0f;
    hx83112a_g_core_fp.fp_read_all_sram = hx83112a_mcu_read_all_sram;
    hx83112a_g_core_fp.fp_firmware_read_0f = hx83112a_mcu_firmware_read_0f;
    hx83112a_g_core_fp.fp_0f_operation_check = hx83112a_mcu_0f_operation_check;
#endif

    hx83112a_in_parse_assign_cmd(zf_addr_dis_flash_reload, hx83112a_pzf_op->addr_dis_flash_reload, sizeof(hx83112a_pzf_op->addr_dis_flash_reload));
    hx83112a_in_parse_assign_cmd(zf_data_dis_flash_reload, hx83112a_pzf_op->data_dis_flash_reload, sizeof(hx83112a_pzf_op->data_dis_flash_reload));
    hx83112a_in_parse_assign_cmd(zf_addr_system_reset, hx83112a_pzf_op->addr_system_reset, sizeof(hx83112a_pzf_op->addr_system_reset));
    hx83112a_in_parse_assign_cmd(zf_data_system_reset, hx83112a_pzf_op->data_system_reset, sizeof(hx83112a_pzf_op->data_system_reset));
    hx83112a_in_parse_assign_cmd(zf_data_sram_start_addr, hx83112a_pzf_op->data_sram_start_addr, sizeof(hx83112a_pzf_op->data_sram_start_addr));
    hx83112a_in_parse_assign_cmd(zf_data_sram_clean, hx83112a_pzf_op->data_sram_clean, sizeof(hx83112a_pzf_op->data_sram_clean));
    hx83112a_in_parse_assign_cmd(zf_data_cfg_info, hx83112a_pzf_op->data_cfg_info, sizeof(hx83112a_pzf_op->data_cfg_info));
    hx83112a_in_parse_assign_cmd(zf_data_fw_cfg_p1, hx83112a_pzf_op->data_fw_cfg_p1, sizeof(hx83112a_pzf_op->data_fw_cfg_p1));
    hx83112a_in_parse_assign_cmd(zf_data_fw_cfg_p2, hx83112a_pzf_op->data_fw_cfg_p2, sizeof(hx83112a_pzf_op->data_fw_cfg_p2));
    hx83112a_in_parse_assign_cmd(zf_data_fw_cfg_p3, hx83112a_pzf_op->data_fw_cfg_p3, sizeof(hx83112a_pzf_op->data_fw_cfg_p3));
    hx83112a_in_parse_assign_cmd(zf_data_adc_cfg_1, hx83112a_pzf_op->data_adc_cfg_1, sizeof(hx83112a_pzf_op->data_adc_cfg_1));
    hx83112a_in_parse_assign_cmd(zf_data_adc_cfg_2, hx83112a_pzf_op->data_adc_cfg_2, sizeof(hx83112a_pzf_op->data_adc_cfg_2));
    hx83112a_in_parse_assign_cmd(zf_data_adc_cfg_3, hx83112a_pzf_op->data_adc_cfg_3, sizeof(hx83112a_pzf_op->data_adc_cfg_3));
    hx83112a_in_parse_assign_cmd(zf_data_map_table, hx83112a_pzf_op->data_map_table, sizeof(hx83112a_pzf_op->data_map_table));
    hx83112a_in_parse_assign_cmd(zf_data_mode_switch, hx83112a_pzf_op->data_mode_switch, sizeof(hx83112a_pzf_op->data_mode_switch));
...
bool hx83112a_ic_package_check(void)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    ...
    for (i = 0; i < 5; i++) {
        ...
        if (1) {
            hx83112a_IC_TYPE = HX_83112A_SERIES_PWON;
			HX_IC_TYPE = hx83112a_IC_TYPE;
            hx83112a_IC_CHECKSUM = HX_TP_BIN_CHECKSUM_CRC;
            hx83112a_hx_0f_init();
            //Himax: Set FW and CFG Flash Address
            hx83112a_FW_VER_MAJ_FLASH_ADDR   = 49157;  //0x00C005
            hx83112a_FW_VER_MAJ_FLASH_LENG   = 1;
            hx83112a_FW_VER_MIN_FLASH_ADDR   = 49158;  //0x00C006
            hx83112a_FW_VER_MIN_FLASH_LENG   = 1;
            hx83112a_CFG_VER_MAJ_FLASH_ADDR = 49408;  //0x00C100
            hx83112a_CFG_VER_MAJ_FLASH_LENG = 1;
            hx83112a_CFG_VER_MIN_FLASH_ADDR = 49409;  //0x00C101
            hx83112a_CFG_VER_MIN_FLASH_LENG = 1;
            hx83112a_CID_VER_MAJ_FLASH_ADDR = 49154;  //0x00C002
            hx83112a_CID_VER_MAJ_FLASH_LENG = 1;
            hx83112a_CID_VER_MIN_FLASH_ADDR = 49155;  //0x00C003
            hx83112a_CID_VER_MIN_FLASH_LENG = 1;
            //PANEL_VERSION_ADDR = 49156;  //0x00C004
            //PANEL_VERSION_LENG = 1;
#ifdef HX_AUTO_UPDATE_FW
            g_i_FW_VER = i_CTPM_FW[hx83112a_FW_VER_MAJ_FLASH_ADDR] << 8 | i_CTPM_FW[hx83112a_FW_VER_MIN_FLASH_ADDR];
            g_i_CFG_VER = i_CTPM_FW[hx83112a_CFG_VER_MAJ_FLASH_ADDR] << 8 | i_CTPM_FW[hx83112a_CFG_VER_MIN_FLASH_ADDR];
            g_i_CID_MAJ = i_CTPM_FW[hx83112a_CID_VER_MAJ_FLASH_ADDR];
            g_i_CID_MIN = i_CTPM_FW[hx83112a_CID_VER_MIN_FLASH_ADDR];
#endif
...
void hx83112a_power_on_init(void)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
static void hx83112a_read_FW_ver(void)
{
    ...
    while(reload_status == 0) {
        ...
        cmd[1] = 0x7f;
        cmd[0] = 0x00;
        hx83112a_register_read(cmd, 4, data, false);

        cmd[3] = 0x10;
        cmd[2] = 0x00;
        cmd[1] = 0x72;
        cmd[0] = 0xc0;
        hx83112a_register_read(cmd, 4, data2, false);
    ...
    if (hx83112a_private_ts->panel_data.manufacture_info.version) {
        if (hx83112a_private_ts->panel_data.vid_len == 0) {
            strlcpy(&(hx83112a_private_ts->panel_data.manufacture_info.version[12]), dev_version, 3);
        } else {
            ver_len = hx83112a_private_ts->panel_data.vid_len;
            if (ver_len > MAX_DEVICE_VERSION_LENGTH - 4) {
                ver_len = MAX_DEVICE_VERSION_LENGTH - 4;
            }

            strlcpy(&hx83112a_private_ts->panel_data.manufacture_info.version[ver_len],
                     dev_version, MAX_DEVICE_VERSION_LENGTH - ver_len);
        }
    }
   TPD_INFO("manufacture_info.version: %s\n", hx83112a_private_ts->panel_data.manufacture_info.version);
...
void hx83112a_read_OPPO_FW_ver(struct chip_data_hx83112b *chip_info)
{
    uint8_t cmd[4];
    uint8_t data[4];
    ...
    if (data[0] == 0xBD && data[1] == 0x12)
        hx83112a_isBD12proj = true;
    if (data[0] == 0xEA && data[1] == 0x00 && (data[2] & 0xF0) == 0x60)
        hx83112a_isEA006proj = true;
...
uint32_t hx83112a_hw_check_CRC(uint8_t *start_addr, int reload_length)
{
    uint32_t result = 0;
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    ...
    hx83112a_flash_write_burst(tmp_addr, start_addr);
...
bool hx83112a_calculateChecksum(bool change_iref)
{
    ...
    CRC_result = hx83112a_hw_check_CRC(tmp_data, FW_SIZE_64k);
...
int hx83112a_cal_data_len(int raw_cnt_rmd, int HX_MAX_PT, int raw_cnt_max)
{
    int RawDataLen;
    if (raw_cnt_rmd != 0x00) {
...
int hx83112a_report_data_init(int max_touch_point, int tx_num, int rx_num)
{
    if (hx83112a_hx_touch_data->hx_coord_buf != NULL) {
        kfree(hx83112a_hx_touch_data->hx_coord_buf);
    }

    if (hx83112a_hx_touch_data->diag_mutual != NULL) {
        kfree(hx83112a_hx_touch_data->diag_mutual);
    }

    //#if defined(HX_SMART_WAKEUP)
    hx83112a_hx_touch_data->event_size = 128;
    //#endif*/

    hx83112a_hx_touch_data->touch_all_size = 128; //hx83112a_get_touch_data_size();
    ...
    hx83112a_hx_touch_data->raw_cnt_max = max_touch_point / 4;
    hx83112a_hx_touch_data->raw_cnt_rmd = max_touch_point % 4;

    if (hx83112a_hx_touch_data->raw_cnt_rmd != 0x00) {//more than 4 fingers
        hx83112a_hx_touch_data->rawdata_size = hx83112a_cal_data_len(hx83112a_hx_touch_data->raw_cnt_rmd, max_touch_point, hx83112a_hx_touch_data->raw_cnt_max);
        hx83112a_hx_touch_data->touch_info_size = (max_touch_point + hx83112a_hx_touch_data->raw_cnt_max + 2) * 4;
    } else {//less than 4 fingers
        hx83112a_hx_touch_data->rawdata_size = hx83112a_cal_data_len(hx83112a_hx_touch_data->raw_cnt_rmd, max_touch_point, hx83112a_hx_touch_data->raw_cnt_max);
        hx83112a_hx_touch_data->touch_info_size = (max_touch_point + hx83112a_hx_touch_data->raw_cnt_max + 1) * 4;
    }

    if ((tx_num * rx_num + tx_num + rx_num) % hx83112a_hx_touch_data->rawdata_size == 0) {
        hx83112a_hx_touch_data->rawdata_frame_size = (tx_num * rx_num + tx_num + rx_num) / hx83112a_hx_touch_data->rawdata_size;
    } else {
        hx83112a_hx_touch_data->rawdata_frame_size = (tx_num * rx_num + tx_num + rx_num) / hx83112a_hx_touch_data->rawdata_size + 1;
    }
    TPD_INFO("%s: rawdata_frame_size = %d ", __func__, hx83112a_hx_touch_data->rawdata_frame_size);
    TPD_INFO("%s: max_touch_point:%d, hx_raw_cnt_max:%d, hx_raw_cnt_rmd:%d, g_hx_rawdata_size:%d, hx83112a_hx_touch_data->touch_info_size:%d\n",
             __func__, max_touch_point, hx83112a_hx_touch_data->raw_cnt_max, hx83112a_hx_touch_data->raw_cnt_rmd, hx83112a_hx_touch_data->rawdata_size, hx83112a_hx_touch_data->touch_info_size);

    hx83112a_hx_touch_data->hx_coord_buf = kzalloc(sizeof(uint8_t) * (hx83112a_hx_touch_data->touch_info_size), GFP_KERNEL);
    if (hx83112a_hx_touch_data->hx_coord_buf == NULL) {
        goto mem_alloc_fail;
    }

    hx83112a_hx_touch_data->diag_mutual = kzalloc(tx_num * rx_num * sizeof(int32_t), GFP_KERNEL);
    if (hx83112a_hx_touch_data->diag_mutual == NULL) {
        goto mem_alloc_fail;
    }

    //#ifdef HX_TP_PROC_DIAG
    hx83112a_hx_touch_data->hx_rawdata_buf = kzalloc(sizeof(uint8_t) * (hx83112a_hx_touch_data->touch_all_size - hx83112a_hx_touch_data->touch_info_size), GFP_KERNEL);
    if (hx83112a_hx_touch_data->hx_rawdata_buf == NULL) {
        goto mem_alloc_fail;
    }
    //#endif

    //#if defined(HX_SMART_WAKEUP)
    hx83112a_hx_touch_data->hx_event_buf = kzalloc(sizeof(uint8_t) * (hx83112a_hx_touch_data->event_size), GFP_KERNEL);
    if (hx83112a_hx_touch_data->hx_event_buf == NULL) {
        goto mem_alloc_fail;
    }
    //#endif
...
mem_alloc_fail:
    ...
    kfree(hx83112a_hx_touch_data->hx_rawdata_buf);
    //#endif
    //#if defined(HX_SMART_WAKEUP)
    kfree(hx83112a_hx_touch_data->hx_event_buf);
    //#endif
...
bool hx83112a_read_event_stack(uint8_t *buf, uint8_t length)
{
    uint8_t cmd[4];

    //  AHB_I2C Burst Read Off
    cmd[0] = 0x00;
    if (hx83112a_bus_write(0x11, 1, cmd) < 0) {
        TPD_INFO("%s: i2c access fail!\n", __func__);
        return 0;
    }

    hx83112a_bus_read(0x30, length, buf);
...
int hx83112a_g_zero_event_count = 0;
int hx83112a_ic_esd_recovery(int hx_esd_event, int hx_zero_event, int length)
{
    if (hx_esd_event == length) {
        hx83112a_g_zero_event_count = 0;
        goto checksum_fail;
    } else if (hx_zero_event == length) {
        hx83112a_g_zero_event_count++;
        TPD_INFO("[HIMAX TP MSG]: ALL Zero event is %d times.\n", hx83112a_g_zero_event_count);
        if (hx83112a_g_zero_event_count > 5) {
            hx83112a_g_zero_event_count = 0;
...
void hx83112a_esd_hw_reset(struct chip_data_hx83112b *chip_info)
{
    int ret = 0;
    int load_fw_times = 10;

    TPD_DETAIL("START_Himax TP: ESD - Reset\n");
    hx83112a_HX_ESD_RESET_ACTIVATE = 1;
    ...
    do {
        ...
        load_fw_times--;
        hx83112a_g_core_fp.fp_0f_operation_dirly();
        ret = hx83112a_g_core_fp.fp_reload_disable(0);//success return 1, fail return 0
    ...
    hx83112a_report_all_leave_event(hx83112a_private_ts);*/

    hx83112b_enable_interrupt(hx83112a_g_chip_info, true);
}


int hx83112a_checksum_cal(struct chip_data_hx83112b *chip_info, uint8_t *buf, int ts_status)
{
    ...
    if (ts_status == HX_REPORT_COORD) {
        length = hx83112a_hx_touch_data->touch_info_size;
    }
    ...
    if (ts_status == HX_REPORT_COORD) {
        ...
        if (hx_esd_event == length || hx_zero_event == length) {
            ...
            if (shaking_ret == CHECKSUM_FAIL) {
                hx83112a_esd_hw_reset(chip_info);
                goto checksum_fail;
        ...
        } else if (hx83112a_HX_ESD_RESET_ACTIVATE) {
            /* drop 1st interrupts after chip reset */
            hx83112a_HX_ESD_RESET_ACTIVATE = 0;
            TPD_INFO("[hx83112a_HX_ESD_RESET_ACTIVATE]:%s: Back from reset, ready to serve.\n", __func__);
            goto checksum_fail;
        } else if (hx83112a_HX_HW_RESET_ACTIVATE) {
            /* drop 1st interrupts after chip reset */
            hx83112a_HX_HW_RESET_ACTIVATE = 0;
            TPD_INFO("[hx83112a_HX_HW_RESET_ACTIVATE]:%s: Back from reset, ready to serve.\n", __func__);
            goto ready_to_serve;
        }
    }
    //#endif
...
void hx83112a_log_touch_data(uint8_t *buf, struct hx83112a_report_data *hx83112a_hx_touch_data)
{
    int loop_i = 0;
    int print_size = 0;

    if (!hx83112a_hx_touch_data->diag_cmd) {
        print_size = hx83112a_hx_touch_data->touch_info_size;
    } else {
        print_size = hx83112a_hx_touch_data->touch_all_size;
    }
...
void hx83112a_idle_mode(int disable)
{
    int retry = 20;
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
void hx83112a_reload_disable(int on)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
int hx83112a_hx_test_data_pop_out(struct chip_data_hx83112b *chip_info, char *g_Test_list_log, char *g_Company_info_log, char *g_project_test_info_log, char *rslt_buf, char *filepath)
{
    ...
    vfs_write(raw_file, rslt_buf, hx83112a_g_1kind_raw_size * hx83112a_HX_CRITERIA_ITEM * sizeof(char), &pos);
    if (raw_file != NULL)
...
int hx83112a_hx_test_data_get(struct chip_data_hx83112b *chip_info, uint32_t RAW[], char *start_log, char *result, int now_item)
{
    uint32_t i;

    ssize_t len = 0;
    char *testdata = NULL;
    uint32_t SZ_SIZE = hx83112a_g_1kind_raw_size;

    TPD_INFO("%s: Entering, Now type=%s!\n", __func__,
             g_hx83112a_inspection_mode[now_item]);
    ...
    memcpy(&hx83112a_g_rslt_data[hx83112a_hx83112a_g_rslt_data_len], testdata, len);
    hx83112a_hx83112a_g_rslt_data_len += len;
    TPD_INFO("%s: hx83112a_hx83112a_g_rslt_data_len=%d!\n", __func__, hx83112a_hx83112a_g_rslt_data_len);
...
int hx83112a_get_rawdata(struct chip_data_hx83112b *chip_info, uint32_t *RAW, uint32_t datalen)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    uint8_t *tmp_rawdata;
    ...
    while (retry < 200) {
        ...
        hx83112a_flash_write_burst_length(tmp_addr, tmp_data, 4);
    ...
    while (retry < 10) {
        ...
        for (i = 0; i < (total_read_times); i++) {
            if (total_size_temp >= max_i2c_size) {
                hx83112a_register_read(tmp_addr, max_i2c_size, &tmp_rawdata[i * max_i2c_size], false);
                total_size_temp = total_size_temp - max_i2c_size;
            } else {
                //TPD_INFO("last total_size_temp=%d\n",total_size_temp);
                hx83112a_register_read(tmp_addr, total_size_temp % max_i2c_size, &tmp_rawdata[i * max_i2c_size], false);
            }
...
void hx83112a_switch_data_type(uint8_t checktype)
{
    ...
    hx83112a_diag_register_set(datatype);
}

int hx83112a_switch_mode(int mode)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    TPD_INFO("%s: Entering\n", __func__);
...
void hx83112a_set_N_frame(uint16_t Nframe, uint8_t checktype)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
uint32_t hx83112a_check_mode(uint8_t checktype)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    uint8_t wait_pwd[2];
    ...
    tmp_addr[0] = 0x04;
    hx83112a_register_read(tmp_addr, 4, tmp_data, false);
    TPD_INFO("%s: hx83112a_wait_sorting_mode, tmp_data[0]=%x, tmp_data[1]=%x\n", __func__, tmp_data[0], tmp_data[1]);

    if (wait_pwd[0] == tmp_data[0] && wait_pwd[1] == tmp_data[1]) {
        TPD_INFO("Change to mode=%s\n", g_hx83112a_inspection_mode[checktype]);
        return 0;
    } else
        return 1;
}

void hx83112a_get_noise_base(void)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
uint16_t hx83112a_get_noise_weight(void)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    uint16_t weight;
...
uint32_t hx83112a_wait_sorting_mode(uint8_t checktype)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    uint8_t wait_pwd[2];
...
#define HIMAX_SKIP_NOTCH_CHECK_IN_DRIVER
int hx83112a_check_notch(int index)
{
#ifndef HIMAX_SKIP_NOTCH_CHECK_IN_DRIVER
    int tx = hx83112a_g_chip_info->hw_res->TX_NUM;
    int rx = hx83112a_g_chip_info->hw_res->RX_NUM;
...
int hx83112a_mpTestFunc(struct chip_data_hx83112b *chip_info, uint8_t checktype, uint32_t datalen)
{
    uint8_t tmp_addr[4] = {0};
    uint8_t tmp_data[4] = {0};
    ...
    if (hx83112a_check_mode(checktype)) {
        TPD_INFO("Need Change Mode, target=%s", g_hx83112a_inspection_mode[checktype]);

        hx83112a_sense_off_mptest();

#ifndef HX_ZERO_FLASH
        hx83112a_reload_disable(1);
#endif

        hx83112a_switch_mode(checktype);

        if (checktype == HIMAX_INSPECTION_NOISE) {
            hx83112a_set_N_frame(NOISEFRAME, checktype);
            /*hx83112a_get_noise_base();*/
        } else if(checktype >= HIMAX_INSPECTION_LPWUG_RAWDATA) {
            TPD_INFO("N frame = %d\n", 1);
            hx83112a_set_N_frame(1, checktype);
        } else {
            hx83112a_set_N_frame(2, checktype);
        }


        hx83112a_sense_on(1);

        ret = hx83112a_wait_sorting_mode(checktype);
        if (ret) {
            TPD_INFO("%s: hx83112a_wait_sorting_mode FAIL\n", __func__);
            return ret;
        }
    }

    hx83112a_switch_data_type(checktype);

    ret = hx83112a_get_rawdata(chip_info, RAW, datalen);
    if (ret) {
        TPD_INFO("%s: hx83112a_get_rawdata FAIL\n", __func__);
    ...
    /* back to normal */
    hx83112a_switch_data_type(HIMAX_INSPECTION_BACK_NORMAL);
    ...
    snprintf(start_log, 256 * sizeof(char), "\n%s%s\n",
             g_hx83112a_inspection_mode[checktype], ": data as follow!\n");
    //Check Data
    switch (checktype) {
    case HIMAX_INSPECTION_OPEN:
        for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
            if (hx83112a_check_notch(i)) {
                continue;
            }
            if(hx83112a_isRead_csv == false) {
    ...
    case HIMAX_INSPECTION_NOISE:
        ...
        hx83112a_get_noise_base();
        if(hx83112a_isBD12proj || hx83112a_isEA006proj) {
            for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
                if (hx83112a_check_notch(i)) {
                    continue;
                }
            ...
            snprintf(start_log, 256 * sizeof(char), "\n Threshold = %d\n", NOISEMAX);
            weight = hx83112a_get_noise_weight();
            if (weight > NOISEMAX) {
                TPD_INFO("%s: noise test FAIL\n", __func__);
                ret = RESULT_ERR;
            }
        ...
        hx83112a_switch_data_type(HIMAX_INSPECTION_RAWDATA);
        ret = hx83112a_get_rawdata(chip_info, RAW, datalen);
        if (ret == RESULT_ERR) {
            TPD_INFO("%s: hx83112a_get_rawdata FAIL\n", __func__);
            ret = RESULT_ERR;
    ...
    snprintf(rslt_log, 256 * sizeof(char), "\n%s%s\n",
             g_hx83112a_inspection_mode[checktype], " Test Pass!\n");

    hx83112a_hx_test_data_get(chip_info, RAW, start_log, rslt_log, checktype);
...
static int hx83112a_saperate_comma(const struct firmware *file_entry,
...
int hx83112a_hx_find_crtra_id(char *input)
{
    int i = 0;
    int result = 0;

    for (i = 0 ; i < hx83112a_HX_CRITERIA_SIZE ; i++) {
        if (hx_diff_str(hx83112a_g_hx_inspt_crtra_name[i], input) == 0) {
            result = i;
            TPD_INFO("find the str=%s, idx=%d\n",
                     hx83112a_g_hx_inspt_crtra_name[i], i);
            break;
        }
    }
    if (i > (hx83112a_HX_CRITERIA_SIZE - 1)) {
...
static int hx83112a_power_cal(int pow, int number)
{
    int i = 0;
    int result = 1;
...
static int hiamx_parse_str2int(char *str)
{
    ...
    for (i = 0; i < strlen(str); i++) {
        ...
        temp_cal = str[i] - '0';
        result += hx83112a_power_cal(str_len - i - 1, temp_cal);
...
int hx83112a_hx_print_crtra_after_parsing(struct chip_data_hx83112b *chip_info)
{
    int i = 0, j = 0;
    int all_mut_len = chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM;

    for (i = 0; i < hx83112a_HX_CRITERIA_SIZE; i++) {
        TPD_DETAIL("Now is %s\n", hx83112a_g_hx_inspt_crtra_name[i]);
...
static int hx_get_crtra_by_name(struct chip_data_hx83112b *chip_info, char **result, int size_of_result_str)
{
    ...
    for (i = 0; i < size_of_result_str && result[i] != NULL; i++) {
        ...
        if (i == 0 || i == ((i / (all_mut_len)) + (i / (all_mut_len) * (all_mut_len)))) {
            ...
            * or not
            */
            count_type = hx83112a_hx_find_crtra_id(result[i]);
            if (count_type < 0) {
                TPD_INFO("1. %s:Name Not match!\n", __func__);
                /* E("can recognize[%d]=%s\n", count_type,
                 * hx83112a_g_hx_inspt_crtra_name[count_type]);
                 */
                ...
                TPD_INFO("Now str=%s, idx=%d\n",
                         hx83112a_g_hx_inspt_crtra_name[count_type], count_type);
    ...
    hx83112a_hx_print_crtra_after_parsing(chip_info);
...
static int hx83112a_parse_criteria_file(struct chip_data_hx83112b *chip_info)
{
    int err = THP_AFE_INSPECT_OK;
    ...
    int i = 0;
    int j = 0;
    int crtra_count = hx83112a_HX_CRITERIA_SIZE;
    ...
    err = request_firmware(&file_entry, chip_info->test_limit_name, hx83112a_private_ts->dev);
    if (err < 0) {
        TPD_INFO("%s, fail in line%d error code=%d\n", __func__, __LINE__, err);
        err = THP_AFE_INSPECT_EFILE;
        hx83112a_isRead_csv = false;
     ...
     *  name string
     */
    while (hx83112a_g_hx_inspt_crtra_name[j] != NULL) {
        if (strlen(hx83112a_g_hx_inspt_crtra_name[j]) > str_max_len)
            str_max_len = strlen(hx83112a_g_hx_inspt_crtra_name[j]);
        j++;
    }
    ...
    size_of_result_str =
        hx83112a_saperate_comma(file_entry, result, data_size);
...
int hx83112a_hx_get_size_str_arr(char **input)
{
    int i = 0;
    int result = 0;
...
static int hx83112a_self_test_data_init(struct chip_data_hx83112b *chip_info)
{
    int ret = THP_AFE_INSPECT_OK;
    int i = 0;

    /* get test item and its items of criteria*/
    hx83112a_HX_CRITERIA_ITEM = hx83112a_hx_get_size_str_arr(g_hx83112a_inspection_mode);
    hx83112a_HX_CRITERIA_SIZE = hx83112a_hx_get_size_str_arr(hx83112a_g_hx_inspt_crtra_name);
    TPD_INFO("There is %d hx83112a_HX_CRITERIA_ITEM and %d hx83112a_HX_CRITERIA_SIZE\n",
             hx83112a_HX_CRITERIA_ITEM, hx83112a_HX_CRITERIA_SIZE);

    /* init criteria data*/
    hx83112a_nf_inspt_crtra_flag = kzalloc(hx83112a_HX_CRITERIA_SIZE * sizeof(int), GFP_KERNEL);
    hx83112a_nf_inspection_criteria = kzalloc(sizeof(int *)*hx83112a_HX_CRITERIA_SIZE, GFP_KERNEL);
...
static void hx83112b_black_screen_test(void *chip_data, char *message)
{
    ...
    hx83112a_hx83112a_g_rslt_data_len = 0;

    /*init criteria data*/
    error = hx83112a_self_test_data_init(chip_info);
    /*init criteria data*/

    /*Init Log Data */
    hx83112a_g_1kind_raw_size = 5 * chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM * 2;
    ...
    hx83112a_g_file_path_OK = kcalloc(256, sizeof(char), GFP_KERNEL);
    if (!hx83112a_g_file_path_OK) {
        TPD_INFO("%s:%d hx83112a_g_file_path_OK kzalloc buf error\n", __func__, __LINE__);
        goto RET_OUT;
    }
    hx83112a_g_file_path_NG = kcalloc(256, sizeof(char), GFP_KERNEL);
    if (!hx83112a_g_file_path_NG) {
        TPD_INFO("%s:%d hx83112a_g_file_path_NG kzalloc buf error\n", __func__, __LINE__);
        goto RET_OUT;
    }

    if (hx83112a_g_rslt_data == NULL) {
        TPD_INFO("hx83112a_g_rslt_data is NULL");
        hx83112a_g_rslt_data = kcalloc(hx83112a_g_1kind_raw_size * hx83112a_HX_CRITERIA_ITEM,
                              sizeof(char), GFP_KERNEL);
        if (!hx83112a_g_rslt_data) {
            TPD_INFO("%s:%d hx83112a_g_rslt_data kzalloc buf error\n", __func__, __LINE__);
            goto RET_OUT;
        }
    } else {
        memset(hx83112a_g_rslt_data, 0x00, hx83112a_g_1kind_raw_size * hx83112a_HX_CRITERIA_ITEM *
               sizeof(char));
    }
    /*Init Log Data */
    ...
    hx83112a_sense_off();
    hx83112a_switch_mode(HIMAX_INSPECTION_LPWUG_RAWDATA);
    ...
    retry_cnt = 3;
    do {
        error = hx83112a_mpTestFunc(chip_info, HIMAX_INSPECTION_LPWUG_RAWDATA, (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) + chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
        retry_cnt--;
    ...
    do {
        error = hx83112a_mpTestFunc(chip_info, HIMAX_INSPECTION_LPWUG_NOISE, (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) + chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
        retry_cnt--;
    } while ((error == RESULT_RETRY) && (retry_cnt > 0));
    ...
    do {
        error = hx83112a_mpTestFunc(chip_info, HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA, (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) + chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
        retry_cnt--;
    } while ((error == RESULT_RETRY) && (retry_cnt > 0));
    ...
    do {
        error = hx83112a_mpTestFunc(chip_info, HIMAX_INSPECTION_LPWUG_IDLE_NOISE, (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) + chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
        retry_cnt--;
    } while ((error == RESULT_RETRY) && (retry_cnt > 0));
    ...
    if (error) {
        snprintf(hx83112a_g_file_path_NG,
                 (int)(strlen(HX_GES_RSLT_OUT_PATH_NG) + strlen(g_file_name_NG) + 1),
                 "%s%s", HX_GES_RSLT_OUT_PATH_NG, g_file_name_NG);
        hx83112a_hx_test_data_pop_out(chip_info, g_Test_list_log, g_Company_info_log, g_project_test_info_log, hx83112a_g_rslt_data, hx83112a_g_file_path_NG);
    } else {
        snprintf(hx83112a_g_file_path_OK,
                 (int)(strlen(HX_GES_RSLT_OUT_PATH_OK) + strlen(g_file_name_OK) + 1),
                 "%s%s", HX_GES_RSLT_OUT_PATH_OK, g_file_name_OK);
        hx83112a_hx_test_data_pop_out(chip_info, g_Test_list_log, g_Company_info_log, g_project_test_info_log, hx83112a_g_rslt_data, hx83112a_g_file_path_OK);
    }
    /*Save Log Data */
...
#ifndef HX_ZERO_FLASH
    hx83112a_sense_off();
    //hx83112a_set_N_frame(1, HIMAX_INSPECTION_NOISE);

    hx83112a_reload_disable(0);

    hx83112a_sense_on(0);
#endif
    /*hx83112a_mcu_0f_operation_dirly();
    msleep(5);
    hx83112a_g_core_fp.fp_reload_disable(0);
    msleep(5);
    hx83112a_sense_on(0x00);
    hx83112a_read_OPPO_FW_ver();*/
RET_OUT:
    ...
    /*
    if (hx83112a_g_rslt_data) {
        kfree(hx83112a_g_rslt_data);
        hx83112a_g_rslt_data = NULL;
    }
    */
    if (hx83112a_g_file_path_OK) {
        kfree(hx83112a_g_file_path_OK);
        hx83112a_g_file_path_OK = NULL;
    }
    if (hx83112a_g_file_path_NG) {
        kfree(hx83112a_g_file_path_NG);
        hx83112a_g_file_path_NG = NULL;
    }
    if (g_Test_list_log) {
...
int hx83112a_chip_self_test(struct seq_file *s, struct chip_data_hx83112b *chip_info, char *g_Test_list_log)
{
    int error = 0;
    int error_num = 0;
    char buf[128] = {0};
    ...
    do {
        error = hx83112a_mpTestFunc(chip_info, HIMAX_INSPECTION_OPEN, (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) + chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
        retry_cnt--;
    } while ((error == RESULT_RETRY) && (retry_cnt > 0));
    hx83112a_nf_fail_write_count += snprintf(g_Test_list_log + hx83112a_nf_fail_write_count, 15, "test Item:\n");
    ...
    do {
        error = hx83112a_mpTestFunc(chip_info, HIMAX_INSPECTION_MICRO_OPEN, (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) + chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
        retry_cnt--;
    } while ((error == RESULT_RETRY) && (retry_cnt > 0));
    ...
    do {
        error = hx83112a_mpTestFunc(chip_info, HIMAX_INSPECTION_SHORT, (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) + chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
        retry_cnt--;
    } while ((error == RESULT_RETRY) && (retry_cnt > 0));
    ...
    TPD_INFO("[MP_RAW_TEST_RAW]\n");
    do {
        error = hx83112a_mpTestFunc(chip_info, HIMAX_INSPECTION_RAWDATA, (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) + chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
        retry_cnt--;
    } while ((error == RESULT_RETRY) && (retry_cnt > 0));
    ...
    do {
        error = hx83112a_mpTestFunc(chip_info, HIMAX_INSPECTION_NOISE, (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) + chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
        retry_cnt--;
    } while ((error == RESULT_RETRY) && (retry_cnt > 0));
    ...
    //hx83112a_set_SMWP_enable(ts->SMWP_enable,suspended);
...
void hx83112a_init_psl(void) //power saving level
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
void hx83112a_chip_erase(void)
{
    ...
    if (!hx83112a_wait_wip(100)) {
        TPD_INFO("%s:83112_Chip_Erase Fail\n", __func__);
    }

}

void hx83112a_flash_programming(uint8_t *FW_content, int FW_Size)
{
    ...
    for (page_prog_start = 0; page_prog_start < FW_Size; page_prog_start = page_prog_start + 256) {
        ...
        if (hx83112a_bus_write(0x00, 20, buring_data) < 0) {
            TPD_INFO("%s: i2c access fail!\n", __func__);
            return;
        }
        //=================================
        ...
        for (j = 0; j < 5; j++) {
            ...
            if (hx83112a_bus_write(0x00, program_length + 4, buring_data) < 0) {
                TPD_INFO("%s: i2c access fail!\n", __func__);
                return;
            }
        }

        if (!hx83112a_wait_wip(1)) {
...
int hx83112a_fts_ctpm_fw_upgrade_with_sys_fs_64k(unsigned char *fw, int len, bool change_iref) /* Alice - Un */
{
    ...
    hx83112a_enter_safe_mode();
    hx83112a_chip_erase();
    hx83112a_flash_programming(fw, FW_SIZE_64k);
    ...
    if (hx83112a_hw_check_CRC(tmp_data, FW_SIZE_64k) == 0) {
        burnFW_success = 1;
    } else {
        burnFW_success = 0;
    }
...
static size_t hx83112b_proc_register_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
    ...
    if (!hx83112a_HX_PROC_SEND_FLAG) {
        temp_buf = kzalloc(len, GFP_KERNEL);

        TPD_INFO("hx83112a_register_show: %02X, %02X, %02X, %02X\n", hx83112a_register_command[3], hx83112a_register_command[2], hx83112a_register_command[1], hx83112a_register_command[0]);

        hx83112a_register_read(hx83112a_register_command, max_bus_size, data, hx83112a_cfg_flag);
        ...
        kfree(temp_buf);
        hx83112a_HX_PROC_SEND_FLAG = 1;
    } else {
        hx83112a_HX_PROC_SEND_FLAG = 0;
    }
    return ret;
}
    ...
    if ((buf[0] == 'r' || buf[0] == 'w') && buf[1] == ':' && buf[2] == 'x') {
        ...
        if (buf[0] == 'r') {
            if (buf[3] == 'F' && buf[4] == 'E' && length == 4) {
                length = length - base;
                hx83112a_cfg_flag = true;
                memcpy(buf_tmp, data_str + base + 1, length);
            } else {
                hx83112a_cfg_flag = false;
                memcpy(buf_tmp, data_str + 1, length);
            }

            hx83112a_byte_length = length / 2;
            if (!kstrtoul(buf_tmp, 16, &result)) {
                for (loop_i = 0 ; loop_i < hx83112a_byte_length ; loop_i++) {
                    hx83112a_register_command[loop_i] = (uint8_t)(result >> loop_i * 8);
                }
            }
        } else if (buf[0] == 'w') {
            if (buf[3] == 'F' && buf[4] == 'E') {
            ...
            if (count < 3) {
                ...
                hx83112a_register_write(hx83112a_register_command, hx83112a_byte_length, w_data, hx83112a_cfg_flag);
            } else {
                hx83112a_byte_length = x_pos[1] - x_pos[0] - 2;
                for (loop_i = 0; loop_i < count; loop_i++) {//parsing addr after 'x'
                    memcpy(buf_tmp, buf + x_pos[loop_i] + 1, hx83112a_byte_length);
                    //TPD_INFO("%s: buf_tmp = %s\n", __func__,buf_tmp);
                    if (!kstrtoul(buf_tmp, 16, &result)) {
                        if (loop_i == 0) {
                            hx83112a_register_command[loop_i] = (uint8_t)(result);
                            //TPD_INFO("%s: hx83112a_register_command = %X\n", __func__,hx83112a_register_command[0]);
                        } else {
                ...
                hx83112a_byte_length = count - 1;
                hx83112a_register_write(hx83112a_register_command, hx83112a_byte_length, &w_data[0], hx83112a_cfg_flag);
            }
        } else {
            return len;
        }
...
void hx83112a_return_event_stack(void)
{
    int retry = 20;
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
int hx83112a_write_read_reg(uint8_t *tmp_addr, uint8_t *tmp_data, uint8_t hb, uint8_t lb)
{
    int cnt = 0;
    ...
    if (cnt >= 99) {
        TPD_INFO("hx83112a_write_read_reg ERR Now register 0x%08X : high byte = 0x%02X, low byte = 0x%02X\n", tmp_addr[3], tmp_data[1], tmp_data[0]);
        return -1;
    }
...
void hx83112a_get_DSRAM_data(uint8_t *info_data, uint8_t x_num, uint8_t y_num)
{
    int i = 0;
    //int cnt = 0;
    ...
    tmp_data[1] = 0x5A;
    tmp_data[0] = 0xA5;
    fw_run_flag = hx83112a_write_read_reg(tmp_addr, tmp_data, 0xA5, 0x5A);
    if (fw_run_flag < 0) {
    ...
    for (i = 0; i < (total_read_times); i++) {
        if (total_size_temp >= max_i2c_size) {
            hx83112a_register_read(tmp_addr, max_i2c_size, &temp_info_data[i * max_i2c_size], false);
            total_size_temp = total_size_temp - max_i2c_size;
        } else {
            //TPD_INFO("last total_size_temp=%d\n",total_size_temp);
            hx83112a_register_read(tmp_addr, total_size_temp % max_i2c_size,
    ...
    /* 4. FW stop outputing */
    //TPD_INFO("hx83112a_DSRAM_Flag=%d\n",hx83112a_DSRAM_Flag);
    if (hx83112a_DSRAM_Flag == false) {
...
void hx83112a_ts_diag_func(struct chip_data_hx83112b *chip_info, int32_t *mutual_data)
{
    int i = 0;
    int j = 0;
    ...
    dsram_type = hx83112a_g_diag_command / 10;

    TPD_INFO("%s:Entering hx83112a_g_diag_command=%d\n!", __func__, hx83112a_g_diag_command);
    ...
    hx83112a_burst_enable(1);
    hx83112a_get_DSRAM_data(info_data, chip_info->hw_res->RX_NUM, chip_info->hw_res->TX_NUM);
...
void hx83112a_diag_parse_raw_data(struct hx83112a_report_data *hx83112a_hx_touch_data, int mul_num, int self_num, uint8_t diag_cmd, int32_t *mutual_data, int32_t *self_data)
{
    int RawDataLen_word;
    int index = 0;
    int temp1, temp2, i;

    if (hx83112a_hx_touch_data->hx_rawdata_buf[0] == 0x3A
        && hx83112a_hx_touch_data->hx_rawdata_buf[1] == 0xA3
        && hx83112a_hx_touch_data->hx_rawdata_buf[2] > 0
        && hx83112a_hx_touch_data->hx_rawdata_buf[3] == diag_cmd) {
        RawDataLen_word = hx83112a_hx_touch_data->rawdata_size / 2;
        index = (hx83112a_hx_touch_data->hx_rawdata_buf[2] - 1) * RawDataLen_word;
        ...
        for (i = 0; i < RawDataLen_word; i++) {
            ...
            if (temp1 < mul_num) {
                //mutual
                mutual_data[index + i] = ((int8_t)hx83112a_hx_touch_data->hx_rawdata_buf[i * 2 + 4 + 1]) * 256 + hx83112a_hx_touch_data->hx_rawdata_buf[i * 2 + 4]; /* 4: RawData Header, 1:HSB  */
            } else {
                //self
                temp1 = i + index;
                ...
                self_data[i + index - mul_num] = (((int8_t)hx83112a_hx_touch_data->hx_rawdata_buf[i * 2 + 4 + 1]) << 8) | hx83112a_hx_touch_data->hx_rawdata_buf[i * 2 + 4]; /* 4: RawData Header */
                //self_data[i+index-mul_num+1] = hx83112a_hx_touch_data->hx_rawdata_buf[i*2 + 4 + 1];
            }
        }
    }
...
bool hx83112a_diag_check_sum(struct hx83112a_report_data *hx83112a_hx_touch_data) /*return checksum value  */
{
    uint16_t check_sum_cal = 0;
    int i;

    //Check 128th byte CRC
    for (i = 0, check_sum_cal = 0; i < (hx83112a_hx_touch_data->touch_all_size - hx83112a_hx_touch_data->touch_info_size); i = i + 2) {
        check_sum_cal += (hx83112a_hx_touch_data->hx_rawdata_buf[i + 1] * 256 + hx83112a_hx_touch_data->hx_rawdata_buf[i]);
    }
    if (check_sum_cal % 0x10000 != 0) {
        TPD_INFO("%s fail=%2X \n", __func__, check_sum_cal);
        return 0;
...
static size_t hx83112b_proc_diag_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
    ...
    if (messages[1] == 0x0A) {
        hx83112a_g_diag_command = messages[0] - '0';
    } else {
        hx83112a_g_diag_command = (messages[0] - '0') * 10 + (messages[1] - '0');
    }

    storage_type = hx83112a_g_diag_command / 10;
    rawdata_type = hx83112a_g_diag_command % 10;

    TPD_INFO(" messages       = %s\n"
             " hx83112a_g_diag_command = 0x%x\n"
             " storage_type   = 0x%x\n"
             " rawdata_type   = 0x%x\n",
             messages, hx83112a_g_diag_command, storage_type, rawdata_type);

    if (hx83112a_g_diag_command > 0 && rawdata_type == 0) {
        TPD_INFO("[Himax]hx83112a_g_diag_command = 0x%x, storage_type=%d, rawdata_type=%d! Maybe no support!\n", hx83112a_g_diag_command, storage_type, rawdata_type);
        hx83112a_g_diag_command = 0x00;
    } else {
        TPD_INFO("[Himax]hx83112a_g_diag_command = 0x%x, storage_type=%d, rawdata_type=%d\n", hx83112a_g_diag_command, storage_type, rawdata_type);
    }

    if (storage_type == 0 && rawdata_type > 0 && rawdata_type < 8) {
        TPD_INFO("%s, common\n", __func__);
        if (hx83112a_DSRAM_Flag) {
            //(1) Clear DSRAM flag
            hx83112a_DSRAM_Flag = false;
            //(2) Enable ISR
            enable_irq(chip_info->hx_irq);
            //(3) FW leave sram and return to event stack
            hx83112a_return_event_stack();
        }

        command[0] = hx83112a_g_diag_command;
        hx83112a_diag_register_set(command[0]);
        ...
        hx83112a_g_diag_command = 0x00;
        hx83112a_diag_register_set(command[0]);
        TPD_INFO("return to normal hx83112a_g_diag_command = 0x%x\n", hx83112a_g_diag_command);
    }
    return len;
}
#ifndef VENDOR_EDIT
static size_t hx83112b_proc_diag_read(struct file *file, char *buff, size_t len, loff_t *pos)
{
    ...
    if (!hx83112a_HX_PROC_SEND_FLAG) {
        ...
        dsram_type = hx83112a_g_diag_command / 10;
        data_type = hx83112a_g_diag_command % 10;
        ...
        if ((data_type >= 1 && data_type <= 7)) {
            if (dsram_type > 0)
                hx83112a_ts_diag_func(chip_info, hx83112a_hx_touch_data->diag_mutual);

            for (j = 0; j < chip_info->hw_res->RX_NUM ; j++) {
                for (i = 0; i < chip_info->hw_res->TX_NUM; i++) {
                    k = ((mutual_num - j) - chip_info->hw_res->RX_NUM * i) - 1;
                    ret += snprintf(temp_buf + ret, len - ret, "%6d", hx83112a_hx_touch_data->diag_mutual[k]);
                }
                ret += snprintf(temp_buf + ret, len - ret, " %6d\n", hx83112a_diag_self[j]);
            }

            ret += snprintf(temp_buf + ret, len - ret, "\n");
            for (i = 0; i < chip_info->hw_res->TX_NUM; i++) {
                ret += snprintf(temp_buf + ret, len - ret, "%6d", hx83112a_diag_self[i]);
            }
        }
        ...
        //if ((hx83112a_g_diag_command >= 1 && hx83112a_g_diag_command <= 7) || dsram_type > 0)
        {
            /* print Mutual/Slef Maximum and Minimum */
            //hx83112a_get_mutual_edge();
            for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
                if (hx83112a_hx_touch_data->diag_mutual[i] > hx83112a_g_max_mutual) {
                    hx83112a_g_max_mutual = hx83112a_hx_touch_data->diag_mutual[i];
                }
                if (hx83112a_hx_touch_data->diag_mutual[i] < hx83112a_g_min_mutual) {
                    hx83112a_g_min_mutual = hx83112a_hx_touch_data->diag_mutual[i];
                }
            }

            //hx83112a_get_self_edge();
            for (i = 0; i < (chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM); i++) {
                if (hx83112a_diag_self[i] > hx83112a_g_max_self) {
                    hx83112a_g_max_self = hx83112a_diag_self[i];
                }
                if (hx83112a_diag_self[i] < hx83112a_g_min_self) {
                    hx83112a_g_min_self = hx83112a_diag_self[i];
                }
            }

            ret += snprintf(temp_buf + ret, len - ret, "Mutual Max:%3d, Min:%3d\n", hx83112a_g_max_mutual, hx83112a_g_min_mutual);
            ret += snprintf(temp_buf + ret, len - ret, "Self Max:%3d, Min:%3d\n", hx83112a_g_max_self, hx83112a_g_min_self);

            /* recovery status after print*/
            hx83112a_g_max_mutual = 0;
            hx83112a_g_min_mutual = 0xFFFF;
            hx83112a_g_max_self = 0;
            hx83112a_g_min_self = 0xFFFF;
        }
...
static size_t hx83112b_proc_diag_read(struct seq_file *s, void *chip_data)
{
    ...
    if ((data_type >= 1 && data_type <= 7)) {
       if (dsram_type > 0)
          ...
          for (j = 0; j < chip_info->hw_res->RX_NUM ; j++) {
              for (i = 0; i < chip_info->hw_res->TX_NUM; i++) {
                  k = ((mutual_num - j) - chip_info->hw_res->RX_NUM * i) - 1;
                  seq_printf(s,"%6d", hx83112a_hx_touch_data->diag_mutual[k]);
              }
              seq_printf(s," %6d\n", hx83112a_diag_self[j]);
          }
          ...
          seq_printf(s,"\n");
          for (i = 0; i < chip_info->hw_res->TX_NUM; i++) {
              seq_printf(s,"%6d", hx83112a_diag_self[i]);
          }
        }
    ...
    //if ((g_diag_command >= 1 && g_diag_command <= 7) || dsram_type > 0)
    {
        ...
        for (i = 0; i < (chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM); i++) {
            if (hx83112a_diag_self[i] > hx83112a_g_max_mutual) {
                hx83112a_g_max_self = hx83112a_diag_self[i];
            }
            if (hx83112a_diag_self[i] < hx83112a_g_min_self) {
                hx83112a_g_min_self = hx83112a_diag_self[i];
        ...
        seq_printf(s,"Mutual Max:%3d, Min:%3d\n", hx83112a_g_max_mutual, hx83112a_g_min_self);
...
uint8_t hx83112a_read_DD_status(struct chip_data_hx83112b *chip_info, uint8_t *hx83112a_cmd_set, uint8_t *tmp_data)
{
    int cnt = 0;
    uint8_t req_size = hx83112a_cmd_set[0];
    ...
    hx83112a_cmd_set[3] = 0xAA;
    hx83112a_register_write(cmd_addr, 4, hx83112a_cmd_set, 0);

    TPD_INFO("%s: hx83112a_cmd_set[0] = 0x%02X, hx83112a_cmd_set[1] = 0x%02X, hx83112a_cmd_set[2] = 0x%02X, hx83112a_cmd_set[3] = 0x%02X\n", __func__, hx83112a_cmd_set[0], hx83112a_cmd_set[1], hx83112a_cmd_set[2], hx83112a_cmd_set[3]);

    for (cnt = 0; cnt < 100; cnt++) {
        hx83112a_register_read(cmd_addr, 4, tmp_data, false);
    ...
    hx83112a_register_read(tmp_addr, req_size, tmp_data, false);
    return NO_ERR;
}
...
static size_t hx83112b_proc_DD_debug_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
    ...
    if (!hx83112a_HX_PROC_SEND_FLAG) {
        ...
        if (hx83112a_mutual_set_flag == 1) {
            if (hx83112a_read_DD_status(chip_info, hx83112a_cmd_set, tmp_data) == NO_ERR) {
                for (loop_i = 0; loop_i < hx83112a_cmd_set[0]; loop_i++) {
                    if ((loop_i % 8) == 0) {
...
static size_t hx83112b_proc_DD_debug_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
    ...
    if (buf_tmp[2] == 'x' && buf_tmp[6] == 'x' && buf_tmp[10] == 'x') {
        hx83112a_mutual_set_flag = 1;
        for (i = 3; i < 12; i = i + 4) {
            memcpy(buf_tmp2, buf_tmp + i, 2);
            if (!kstrtoul(buf_tmp2, 16, &result))
                hx83112a_cmd_set[cnt] = (uint8_t)result;
            else
                TPD_INFO("String to oul is fail in cnt = %d, buf_tmp2 = %s", cnt, buf_tmp2);
            cnt--;
        }
        TPD_INFO("hx83112a_cmd_set[2] = %02X, hx83112a_cmd_set[1] = %02X, hx83112a_cmd_set[0] = %02X\n", hx83112a_cmd_set[2], hx83112a_cmd_set[1], hx83112a_cmd_set[0]);
    } else
        hx83112a_mutual_set_flag = 0;

    return len;
}

int hx83112a_read_FW_status(struct chip_data_hx83112b *chip_info, uint8_t *state_addr, uint8_t *tmp_addr)
{
    uint8_t req_size = 0;
    ...
    if (state_addr[0] == 0x01) {
        ...
        req_size = 0x04;
        hx83112a_sense_off();
        hx83112a_register_read(status_addr, req_size, tmp_addr, false);
        hx83112a_sense_on(1);
        ...
        req_size = 0x30;
        hx83112a_register_read(cmd_addr, req_size, tmp_addr, false);
    }
        ...
        size_t len, loff_t *pos)
{
    ...
    if (!hx83112a_HX_PROC_SEND_FLAG) {
        ...
        hx83112a_cmd_set[0] = 0x01;
        if (hx83112a_read_FW_status(chip_info, hx83112a_cmd_set, tmp_data) == NO_ERR) {
            ret += snprintf(temp_buf + ret, len - ret, "0x%02X%02X%02X%02X :\t", hx83112a_cmd_set[5], hx83112a_cmd_set[4], hx83112a_cmd_set[3], hx83112a_cmd_set[2]);
            for (loop_i = 0; loop_i < hx83112a_cmd_set[1]; loop_i++) {
                ret += snprintf(temp_buf + ret, len - ret, "%5d\t", tmp_data[loop_i]);
            }
            ret += snprintf(temp_buf + ret, len - ret, "\n");
        }
        hx83112a_cmd_set[0] = 0x02;
        if (hx83112a_read_FW_status(chip_info, hx83112a_cmd_set, tmp_data) == NO_ERR) {
            for (loop_i = 0; loop_i < hx83112a_cmd_set[1]; loop_i = loop_i + 2) {
                if ((loop_i % 16) == 0)
                    ret += snprintf(temp_buf + ret, len - ret, "0x%02X%02X%02X%02X :\t",
                                    hx83112a_cmd_set[5], hx83112a_cmd_set[4], hx83112a_cmd_set[3] + (((hx83112a_cmd_set[2] + loop_i) >> 8) & 0xFF), (hx83112a_cmd_set[2] + loop_i) & 0xFF);
...
int hx83112a_ic_reset(struct chip_data_hx83112b *chip_info, uint8_t loadconfig, uint8_t int_off)
{
    int ret = 0;
    hx83112a_HX_HW_RESET_ACTIVATE = 1;
                                        ...
                                        size_t len, loff_t *pos)
{
    ...
    if (buf_tmp[0] == '1')
        hx83112a_ic_reset(chip_info, false, false);
    else if (buf_tmp[0] == '2')
        hx83112a_ic_reset(chip_info, false, true);
    else if (buf_tmp[0] == '3')
        hx83112a_ic_reset(chip_info, true, false);
    else if (buf_tmp[0] == '4')
        hx83112a_ic_reset(chip_info, true, true);
    else if ((buf_tmp[0] == 'z') && (buf_tmp[1] == 'r')) {
        hx83112a_mcu_0f_operation_check(0);
        hx83112a_mcu_0f_operation_check(1);
...
//add for himax end
#ifdef CONFIG_OPPO_TP_APK
static void hx83112a_gesture_debug_mode_set(bool on_off)
{
    uint8_t tmp_addr[4] = {0};
    uint8_t tmp_data[4] = {0};
    char buf[80] = {0};
    ...
    if (on_off) {
        hx83112a_switch_algo = buf[0];
        hx83112a_check_point_format = 1;
        tmp_data[3] = 0xA1;
        tmp_data[2] = 0x1A;
        tmp_data[1] = 0xA1;
        tmp_data[0] = 0x1A;
        hx83112a_register_write(tmp_addr, 4, tmp_data, 0);
        TPD_INFO("%s: Report 40 trajectory coordinate points .\n", __func__);
    }  else {
        hx83112a_switch_algo = 0;
        hx83112a_check_point_format = 0;
        tmp_data[3] = 0x00;
        tmp_data[2] = 0x00;
        tmp_data[1] = 0x00;
        tmp_data[0] = 0x00;
...
static void hx83112a_debug_mode_set(bool on_off)
{
    uint8_t tmp_addr[4] = {0};
    uint8_t tmp_data[4] = {0};
    char buf[80] = {0};
...
static void hx83112a_debug_sta_judge(struct chip_data_hx83112b *chip_info)
{
    static struct hx83112a_fw_debug_info last_sta;
    struct hx83112a_fw_debug_info sta;

    memcpy(&sta, &hx83112a_hx_touch_data->hx_state_info[3], sizeof(sta));

    if (last_sta.recal0 != sta.recal0) {
        if (sta.recal0) {
            log_buf_write(hx83112a_private_ts, 1);
        } else {
            log_buf_write(hx83112a_private_ts, 2);
        }
    ...
    if (last_sta.recal1 != sta.recal1) {
        if (sta.recal1) {
            log_buf_write(hx83112a_private_ts, 4);
        } else {
            log_buf_write(hx83112a_private_ts, 3);
        }
    ...
    if (last_sta.paseline != sta.paseline) {
        if (sta.paseline) {
            log_buf_write(hx83112a_private_ts, 5);
        } else {
            //log_buf_write(hx83112a_private_ts, 4);
        }
    ...
    if (last_sta.palm != sta.palm) {
        if (sta.palm) {
            log_buf_write(hx83112a_private_ts, 7);
        } else {
            log_buf_write(hx83112a_private_ts, 6);
        }

    }
    if (last_sta.idle != sta.idle) {
        if (sta.idle) {
            log_buf_write(hx83112a_private_ts, 9);
        } else {
            log_buf_write(hx83112a_private_ts, 8);
        }
    ...
    if (last_sta.water != sta.water) {
        if (sta.water) {
            log_buf_write(hx83112a_private_ts, 11);
        } else {
            log_buf_write(hx83112a_private_ts, 10);
        }
    ...
    if (last_sta.hopping != sta.hopping) {
        if (sta.hopping) {
            log_buf_write(hx83112a_private_ts, 13);
        } else {
            log_buf_write(hx83112a_private_ts, 12);
        }
    ...
    if (last_sta.noise != sta.noise) {
        if (sta.noise) {
            log_buf_write(hx83112a_private_ts, 15);
        } else {
            log_buf_write(hx83112a_private_ts, 14);
        }
    ...
    if (last_sta.glove != sta.glove) {
        if (sta.glove) {
            log_buf_write(hx83112a_private_ts, 17);
        } else {
            log_buf_write(hx83112a_private_ts, 16);
        }
    ...
    if (last_sta.border != sta.border) {
        if (sta.border) {
            log_buf_write(hx83112a_private_ts, 19);
        } else {
            log_buf_write(hx83112a_private_ts, 18);
        }
    ...
    if (last_sta.vr != sta.vr) {
        if (sta.vr) {
            log_buf_write(hx83112a_private_ts, 21);
        } else {
            log_buf_write(hx83112a_private_ts, 20);
        }
    ...
    if (last_sta.big_small != sta.big_small) {
        if (sta.big_small) {
            log_buf_write(hx83112a_private_ts, 23);
        } else {
            log_buf_write(hx83112a_private_ts, 22);
        }
    ...
    if (last_sta.one_block != sta.one_block) {
        if (sta.one_block) {
            log_buf_write(hx83112a_private_ts, 25);
        } else {
            log_buf_write(hx83112a_private_ts, 24);
        }
    ...
    if (last_sta.blewing != sta.blewing) {
        if (sta.blewing) {
            log_buf_write(hx83112a_private_ts, 27);
        } else {
            log_buf_write(hx83112a_private_ts, 26);
        }
    ...
    if (last_sta.thumb_flying != sta.thumb_flying) {
        if (sta.thumb_flying) {
            log_buf_write(hx83112a_private_ts, 29);
        } else {
            log_buf_write(hx83112a_private_ts, 28);
        }
    ...
    if (last_sta.border_extend != sta.border_extend) {
        if (sta.border_extend) {
            log_buf_write(hx83112a_private_ts, 31);
        } else {
            log_buf_write(hx83112a_private_ts, 30);
        }
    ...
    if (tp_debug > 0) {
        TPD_INFO("The sta  is = 0x%02X,0x%02X\n",
                 hx83112a_hx_touch_data->hx_state_info[3],
                 hx83112a_hx_touch_data->hx_state_info[4]);
    }
...
static int hx83112b_get_touch_points(void *chip_data, struct point_info *points, int max_num)
{
    ...
    if (!hx83112a_hx_touch_data) {
        TPD_INFO("%s:%d hx83112a_hx_touch_data is NULL\n", __func__, __LINE__);
    }

    if (!hx83112a_hx_touch_data->hx_coord_buf) {
        TPD_INFO("%s:%d hx83112a_hx_touch_data->hx_coord_buf is NULL\n", __func__, __LINE__);
        return 0;
    }

    hx83112a_burst_enable(0);
    if (hx83112a_g_diag_command)
        ret = hx83112a_read_event_stack(buf, 128);
    else
        ret = hx83112a_read_event_stack(buf, hx83112a_hx_touch_data->touch_info_size);
    if (!ret) {
    ...
    if (LEVEL_DEBUG == tp_debug) {
        hx83112a_log_touch_data(buf, hx83112a_hx_touch_data);
    }

    check_sum_cal = hx83112a_checksum_cal(chip_info, buf, ts_status);//????checksum
    ...
    //hx83112a_assign_touch_data(buf,ts_status);//??buf??, ??hx_coord_buf

    hx_state_info_pos = hx83112a_hx_touch_data->touch_info_size - 6;
    if(ts_status == HX_REPORT_COORD) {
        memcpy(hx83112a_hx_touch_data->hx_coord_buf, &buf[0], hx83112a_hx_touch_data->touch_info_size);
        if(buf[hx_state_info_pos] != 0xFF && buf[hx_state_info_pos + 1] != 0xFF) {
            memcpy(hx83112a_hx_touch_data->hx_state_info, &buf[hx_state_info_pos], 5);
#ifdef CONFIG_OPPO_TP_APK
            if (chip_info->debug_mode_sta) {
                hx83112a_debug_sta_judge(chip_info);
            }
#endif
        } else {
            memset(hx83112a_hx_touch_data->hx_state_info, 0x00, sizeof(hx83112a_hx_touch_data->hx_state_info));
        }
    }
    if (hx83112a_g_diag_command) {
        mutual_num = chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM;
        self_num = chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM;
        TPD_INFO("hx83112a_hx_touch_data->touch_all_size= %d hx83112a_hx_touch_data->touch_info_size = %d, %d\n", \
                 hx83112a_hx_touch_data->touch_all_size, hx83112a_hx_touch_data->touch_info_size, hx83112a_hx_touch_data->touch_all_size - hx83112a_hx_touch_data->touch_info_size);
        memcpy(hx83112a_hx_touch_data->hx_rawdata_buf, &buf[hx83112a_hx_touch_data->touch_info_size], hx83112a_hx_touch_data->touch_all_size - hx83112a_hx_touch_data->touch_info_size);
        if (!hx83112a_diag_check_sum(hx83112a_hx_touch_data)) {
            goto err_workqueue_out;
        }
        hx83112a_diag_parse_raw_data(hx83112a_hx_touch_data, mutual_num, self_num, hx83112a_g_diag_command, hx83112a_hx_touch_data->diag_mutual, hx83112a_diag_self);
    }

    if (hx83112a_hx_touch_data->hx_coord_buf[HX_TOUCH_INFO_POINT_CNT] == 0xff)//HX_TOUCH_INFO_POINT_CNT buf???????
        hx_point_num = 0;
    else
        hx_point_num = hx83112a_hx_touch_data->hx_coord_buf[HX_TOUCH_INFO_POINT_CNT] & 0x0f;


    for (i = 0; i < 10; i++) {
        x = hx83112a_hx_touch_data->hx_coord_buf[i * 4] << 8 | hx83112a_hx_touch_data->hx_coord_buf[i * 4 + 1];
        y = (hx83112a_hx_touch_data->hx_coord_buf[i * 4 + 2] << 8 | hx83112a_hx_touch_data->hx_coord_buf[i * 4 + 3]);
        z = hx83112a_hx_touch_data->hx_coord_buf[i + 40];
        if(x >= 0 && x <= hx83112a_private_ts->resolution_info.max_x && y >= 0 && y <= hx83112a_private_ts->resolution_info.max_y) {
            points[i].x = x;
            points[i].y = y;
...
static fw_check_state hx83112b_fw_check(void *chip_data, struct resolution_info *resolution_info, struct panel_info *panel_data)
{
    ...
    //if (panel_data->manufacture_info.version)
     //   sprintf(panel_data->manufacture_info.version, "0x%x-%d", panel_data->TP_FW, hx83112a_g_lcd_vendor);
...
void hx83112a_ultra_enter(void)
{
    uint8_t tmp_data[4];
    int rtimes = 0;
    ...
    do {
        ...
        tmp_data[0] = 0x11;
        if (hx83112a_bus_write(0x34, 1, tmp_data) < 0) {
            TPD_INFO("%s: spi write fail!\n", __func__);
            continue;
        }
        tmp_data[0] = 0x00;
        if (hx83112a_bus_read(0x34, 1, tmp_data) < 0) {
    ...
    do {
        ...
        tmp_data[0] = 0x33;
        if (hx83112a_bus_write(0x33, 1, tmp_data) < 0) {
            TPD_INFO("%s: spi write fail!\n", __func__);
            continue;
        }
        tmp_data[0] = 0x00;
        if (hx83112a_bus_read(0x33, 1, tmp_data) < 0) {
            TPD_INFO("%s: spi read fail!\n", __func__);
            continue;
        }
    ...
    if (ts->is_suspended) {
        if (enable) {
            ...
            if (chip_info->debug_gesture_sta) {
                hx83112a_gesture_debug_mode_set(true);
            }
#endif // end of CONFIG_OPPO_TP_APK
            //hx83112a_sense_on(0);
            /*if (!hx83112a_HX_RESET_STATE) {
...
void hx83112a_set_SMWP_enable(uint8_t SMWP_enable, bool suspended)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    uint8_t back_data[4];
...
static int hx83112b_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
    ...
    buf = kzalloc(hx83112a_hx_touch_data->event_size * sizeof(uint8_t), GFP_KERNEL);
    if (!buf) {
    ...
    hx83112a_burst_enable(0);
    if (!hx83112a_read_event_stack(buf, hx83112a_hx_touch_data->event_size)) {
    ...
    check_sum_cal = hx83112a_checksum_cal(chip_info, buf, ts_status);
    if (check_sum_cal == CHECKSUM_FAIL) {
        return -1;
    } else if (check_sum_cal == ERR_WORK_OUT) {
        goto err_workqueue_out;
...
#ifdef CONFIG_OPPO_TP_APK
        if(hx83112a_check_point_format == 0) {
#endif
            while (i < (gest_len + 1) / 2) {
                ...
                if (i == 6) {
                    ...
                    gest_pt_x[gest_pt_cnt] = buf[GEST_PTLG_ID_LEN + 4 + i * 2] * hx83112a_private_ts->resolution_info.max_x / 255;
                }
                gest_pt_y[gest_pt_cnt] = buf[GEST_PTLG_ID_LEN + 4 + i * 2 + 1] * hx83112a_private_ts->resolution_info.max_y / 255;
                i++;
            ...
            int pt_num;
            gest_pt_cnt = 40;
            if (hx83112a_private_ts->gesture_buf) {

                pt_num = gest_len + buf[126];
                if (pt_num > 104) {
                    pt_num = 104;
                }
                hx83112a_private_ts->gesture_buf[0] = gesture_sign;
                hx83112a_private_ts->gesture_buf[1] = buf[127];

                if (hx83112a_private_ts->gesture_buf[0] == 0x07) {
                    for(j = 0; j < gest_len * 2; j = j + 2) {
                        hx83112a_private_ts->gesture_buf[3 + j] = buf[n];
                        hx83112a_private_ts->gesture_buf[3 + j + 1] = buf[n + 1];
                        n = n + 4;
                    }

                    for(nn = 0; nn < (pt_num - gest_len)   * 2 ; nn = nn + 2) {
                        hx83112a_private_ts->gesture_buf[3 + j + nn] = buf[m];
                        hx83112a_private_ts->gesture_buf[3 + j + nn + 1] = buf[m + 1];
                        m = m + 4;
                    }
                    hx83112a_private_ts->gesture_buf[2] = pt_num;
                } else {
                    hx83112a_private_ts->gesture_buf[2] = gest_len;
                    memcpy(&hx83112a_private_ts->gesture_buf[3], &buf[24], 80);
                }
...
static void hx83112b_auto_test(struct seq_file *s, void *chip_data, struct syna_testdata *syna_testdata)
{
    ...
    /*init criteria data*/
    ret = hx83112a_self_test_data_init(chip_info);
    /*init criteria data*/
    ...
    p_node = strstr(hx83112a_private_ts->panel_data.fw_name, ".");
    copy_len = p_node - hx83112a_private_ts->panel_data.fw_name;
    memcpy(fw_name_test, hx83112a_private_ts->panel_data.fw_name, copy_len);
    strlcat(fw_name_test, postfix, MAX_FW_NAME_LENGTH);
    TPD_INFO("%s : fw_name_test is %s\n", __func__, fw_name_test);

    hx83112a_mcu_0f_operation_test_dirly(fw_name_test);
    msleep(5);
    hx83112a_g_core_fp.fp_reload_disable(0);
    msleep(5);
    hx83112a_sense_on(0x00);
    hx83112a_read_OPPO_FW_ver(chip_info);

    ret = hx83112b_enable_interrupt(chip_info, false);

    error_count += hx83112b_int_pin_test(s, chip_info, syna_testdata, g_Test_list_log);
    error_count += hx83112a_chip_self_test(s, chip_info, g_Test_list_log);
...
static void hx83112b_read_debug_data(struct seq_file *s, void *chip_data, int debug_data_type)
{
    ...
    if (debug_data_type == DEBUG_DATA_DOWN) {
        ...
        tmp_data[0] = 0x5A;
        hx83112a_register_write(tmp_addr, 4, tmp_data, 0);
        hx83112a_diag_register_set(DEBUG_DATA_DELTA);
    } else {
        hx83112a_diag_register_set(debug_data_type);
    }
    TPD_INFO("%s: Start get debug data in DSRAM\n", __func__);
    hx83112a_DSRAM_Flag = true;

    hx83112a_ts_diag_func(chip_info, data_mutual_sram);
    ...
    //Clear DSRAM flag
    hx83112a_diag_register_set(0x00);
    hx83112a_DSRAM_Flag = false;
    hx83112a_return_event_stack();
...
static fw_update_state hx83112b_fw_update(void *chip_data, const struct firmware *fw, bool force)
{
    ...
    // hx83112a_fts_ctpm_fw_upgrade_with_sys_fs_64k((unsigned char *)fw->data, fw->size, false);
    hx83112a_g_core_fp.fp_firmware_update_0f(fw);
    hx83112a_g_core_fp.fp_reload_disable(0);
    msleep (10);
...
static int hx83112b_reset_gpio_control(void *chip_data, bool enable)
{
    ...
    if (gpio_is_valid(chip_info->hw_res->reset_gpio)) {
        ...
        hx83112b_resetgpio_set(hx83112a_g_chip_info->hw_res, enable);
...
static struct himax_proc_operations hx83112b_proc_ops = {
    ...
    .himax_proc_diag_seq_read =  hx83112b_proc_diag_read,
    ...
    //.himax_proc_enter_algorithm_switch_write = hx83112a_enter_algorithm_number_write,
    //.himax_proc_enter_algorithm_switch_read  = hx83112a_enter_algorithm_number_read,
#endif
};
...
static void hx83112a_enter_hopping_write(bool on_off)
{
    uint8_t tmp_addr[4] = {0};
    uint8_t tmp_data[4] = {0};
...
static void hx83112a_apk_game_set(void *chip_data, bool on_off)
{
    hx83112b_mode_switch(chip_data, MODE_GAME, on_off);
}

static bool hx83112a_apk_game_get(void *chip_data)
{
    struct chip_data_hx83112b *chip_info;
    chip_info = (struct chip_data_hx83112b *)chip_data;
...
static void hx83112a_apk_debug_set(void *chip_data, bool on_off)
{
    //u8 cmd[1];
    struct chip_data_hx83112b *chip_info;
    chip_info = (struct chip_data_hx83112b *)chip_data;

    hx83112a_debug_mode_set(on_off);
    chip_info->debug_mode_sta = on_off;
}

static bool hx83112a_apk_debug_get(void *chip_data)
{
    struct chip_data_hx83112b *chip_info;
    chip_info = (struct chip_data_hx83112b *)chip_data;
...
static void hx83112a_apk_gesture_debug(void *chip_data, bool on_off)
{
...
static bool  hx83112a_apk_gesture_get(void *chip_data)
{
    struct chip_data_hx83112b *chip_info;
    chip_info = (struct chip_data_hx83112b *)chip_data;
...
static int  hx83112a_apk_gesture_info(void *chip_data, char *buf, int len)
{
    int ret = 0;
    int i;
    int num;
    ...
    temp = hx83112a_private_ts->gesture_buf[0];
    if (temp == 0x00) {
        temp = hx83112a_private_ts->gesture_buf[1] | 0x80;
    }
    buf[0] = temp;

    //buf[0] = gesture_buf[0];
    num = hx83112a_private_ts->gesture_buf[2];
    ...
    for (i = 0; i < num; i++) {
        int x;
        int y;
        x = hx83112a_private_ts->gesture_buf[i * 2 + 3];
        x = x * hx83112a_private_ts->resolution_info.max_x / 255;

        y = hx83112a_private_ts->gesture_buf[i * 2 + 4];
        y = y * hx83112a_private_ts->resolution_info.max_y / 255;
...
static void hx83112a_apk_earphone_set(void *chip_data, bool on_off)
{
    struct chip_data_hx83112b *chip_info;
    chip_info = (struct chip_data_hx83112b *)chip_data;
...
static bool hx83112a_apk_earphone_get(void *chip_data)
{
    struct chip_data_hx83112b *chip_info;
    chip_info = (struct chip_data_hx83112b *)chip_data;
...
static void hx83112a_apk_charger_set(void *chip_data, bool on_off)
{
    struct chip_data_hx83112b *chip_info;
    chip_info = (struct chip_data_hx83112b *)chip_data;
...
static bool hx83112a_apk_charger_get(void *chip_data)
{
    struct chip_data_hx83112b *chip_info;
    chip_info = (struct chip_data_hx83112b *)chip_data;
...
static void hx83112a_apk_noise_set(void *chip_data, bool on_off)
{
    struct chip_data_hx83112b *chip_info;
    chip_info = (struct chip_data_hx83112b *)chip_data;
    hx83112a_enter_hopping_write(on_off);
    chip_info->noise_sta = on_off;

}

static bool hx83112a_apk_noise_get(void *chip_data)
{
    struct chip_data_hx83112b *chip_info;
    chip_info = (struct chip_data_hx83112b *)chip_data;
...
static int  hx83112a_apk_tp_info_get(void *chip_data, char *buf, int len)
{
    int ret;
    struct chip_data_hx83112b *chip_info;
    chip_info = (struct chip_data_hx83112b *)chip_data;
...
static void hx83112a_init_oppo_apk_op(struct touchpanel_data *ts)
{
    ts->apk_op = kzalloc(sizeof(APK_OPERATION), GFP_KERNEL);
    if(ts->apk_op) {
        ts->apk_op->apk_game_set = hx83112a_apk_game_set;
        ts->apk_op->apk_game_get = hx83112a_apk_game_get;
        ts->apk_op->apk_debug_set = hx83112a_apk_debug_set;
        ts->apk_op->apk_debug_get = hx83112a_apk_debug_get;
        //apk_op->apk_proximity_set = ili_apk_proximity_set;
        //apk_op->apk_proximity_dis = ili_apk_proximity_dis;
        ts->apk_op->apk_noise_set = hx83112a_apk_noise_set;
        ts->apk_op->apk_noise_get = hx83112a_apk_noise_get;
        ts->apk_op->apk_gesture_debug = hx83112a_apk_gesture_debug;
        ts->apk_op->apk_gesture_get = hx83112a_apk_gesture_get;
        ts->apk_op->apk_gesture_info = hx83112a_apk_gesture_info;
        ts->apk_op->apk_earphone_set = hx83112a_apk_earphone_set;
        ts->apk_op->apk_earphone_get = hx83112a_apk_earphone_get;
        ts->apk_op->apk_charger_set = hx83112a_apk_charger_set;
        ts->apk_op->apk_charger_get = hx83112a_apk_charger_get;
        ts->apk_op->apk_tp_info_get = hx83112a_apk_tp_info_get;
...
static int hx83112b_tp_probe(struct spi_device *spi)
{
    ...
    hx83112a_g_chip_info = chip_info;

    /* allocate himax report data */
    hx83112a_hx_touch_data = kzalloc(sizeof(struct hx83112a_report_data), GFP_KERNEL);
    if (hx83112a_hx_touch_data == NULL) {
	...
	ts->s_client->max_speed_hz =10000000;
#ifdef CONFIG_TOUCHPANEL_MTK_PLATFORM
    /* new usage of MTK spi API */
    memcpy(&chip_info->hx_spi_mcc, &hx83112a_hx_spi_ctrdata, sizeof(struct mtk_chip_config));
    ts->s_client->controller_data = (void *)&chip_info->hx_spi_mcc;
#else
    /* old usage of MTK spi API */
    memcpy(&chip_info->hx_spi_mcc, &hx83112a_hx_spi_ctrdata, sizeof(struct mt_chip_conf));
    ts->s_client->controller_data = (void *)&chip_info->hx_spi_mcc;
    ...
    hx83112a_private_ts = ts;

#ifdef CONFIG_OPPO_TP_APK
    hx83112a_init_oppo_apk_op(ts);
#endif // end of CONFIG_OPPO_TP_APK
    ...
    disable_irq_nosync(chip_info->hx_irq);
    if (hx83112a_ic_package_check() == false) {
    ...
    hx83112a_g_auto_update_flag = true;
    chip_info->hx83112a_0f_update_wq = create_singlethread_workqueue("HMX_0f_update_reuqest");
    INIT_DELAYED_WORK(&chip_info->work_0f_update, hx83112a_mcu_0f_operation);
    //queue_delayed_work(chip_info->hx83112a_0f_update_wq, &chip_info->work_0f_update, msecs_to_jiffies(2000));
#else
    hx83112a_read_FW_ver();
    hx83112a_calculateChecksum(false);
#endif

    hx83112a_power_on_init();

    //touch data init
    ret = hx83112a_report_data_init(ts->max_num, ts->hw_res.TX_NUM, ts->hw_res.RX_NUM);
    if (ret) {
        goto err_register_driver;
    }
...
#endif
    {
        ...
        queue_delayed_work(chip_info->hx83112a_0f_update_wq, &chip_info->work_0f_update, msecs_to_jiffies(500));
    } else if (is_oem_unlocked()||ts->fw_update_in_probe_with_headfile) {
        enable_irq(chip_info->hx_irq);
        TPD_INFO("It's oem unlocked, updata fw by bspFwUpdate\n");
        queue_delayed_work(chip_info->hx83112a_0f_update_wq, &chip_info->work_0f_update, msecs_to_jiffies(5000));
    }/* else {
        //disable_irq_nosync(chip_info->hx_irq);
        ...
        if (ts->fw_update_in_probe_with_headfile) {
            ...
            hx83112a_mcu_firmware_update_0f(fw);
        }
    }*/
    hx_tp_oppo6765=1;
...
static int hx83112b_i2c_resume(struct device *dev)
{
    ...
    /* if (ts->black_gesture_support) {
         if (ts->gesture_enable == 1) {
             TPD_INFO("hx83112a_set_SMWP_enable 1\n");
             hx83112a_set_SMWP_enable(1, ts->is_suspended);
         }
     }*/
    return 0;
}
...
static struct spi_driver hx83112a_common_driver = {
...
static int __init tp_driver_init(void)
{
    ...
    status = spi_register_driver(&hx83112a_common_driver);
    if (status < 0) {
...
{
    spi_unregister_driver(&hx83112a_common_driver);
    return;
}
