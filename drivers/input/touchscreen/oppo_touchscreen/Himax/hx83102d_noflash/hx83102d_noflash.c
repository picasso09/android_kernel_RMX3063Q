...
//qiang.xue@ODM_WT.BSP.TP,2020/12/01,add for WT hardwareinfo
...
static uint32_t hx83102d_get_fw_id(struct chip_data_hx83102d *chip_info)
{
    ...
    TPD_INFO("CURRENT_FIRMWARE_ID = 0x%04x\n", current_firmware);

    return current_firmware;

}

#if 1   //add by wanglongfei 20200212
...
static fw_update_state hx83102d_fw_update(void *chip_data, const struct firmware *fw, bool force)
{
    ...
    TPD_INFO("CURRENT TP FIRMWARE ID is 0x%04x, FIRMWARE IMAGE ID is 0x%04x\n", CURRENT_FIRMWARE_ID, FIRMWARE_ID);
...
int __maybe_unused hx83102d_tp_probe(struct spi_device *spi)
{
	...
	ts->s_client->max_speed_hz = 8000000;
