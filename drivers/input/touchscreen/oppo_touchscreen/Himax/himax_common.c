...
unsigned char HX_IC_TYPE = 11;

/*******Part1:Call Back Function implement*******/

int hx_tp_oppo6765 = 0;
...
static int himax_proc_diag_seq_read(struct seq_file *s, void *v)
{
    struct touchpanel_data *ts = s->private;
    struct himax_proc_operations *syna_ops;
    ssize_t ret = 0;
    ...
    if (!syna_ops->himax_proc_diag_seq_read) {
        seq_printf(s,"Not support auto-test proc node\n");
        TPD_INFO("%s,here:%d\n",__func__,__LINE__);
        return 0;
    }

    ret = syna_ops->himax_proc_diag_seq_read(s, ts->chip_data);
    return 0;
}

static int himax_diag_seq_open(struct inode *inode, struct file *file)
{
	    return single_open(file, himax_proc_diag_seq_read, PDE_DATA(inode));
};
...
static struct file_operations himax_proc_diag_seq_ops =
{
    .owner = THIS_MODULE,
...
int himax_create_proc(struct touchpanel_data *ts, struct himax_proc_operations *syna_ops)
{
   ...
   //proc files: /proc/touchpanel/debug_info/himax/register
    ...
    if (HX_IC_TYPE == 16){//#define HX_83112A_SERIES_PWON        16
	    prEntry_tmp = proc_create_data("diag", 0666, prEntry_himax, &himax_proc_diag_seq_ops, ts);
    }else{
