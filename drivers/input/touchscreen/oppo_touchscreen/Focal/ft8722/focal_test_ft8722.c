 ...
 * VENDOR_EDIT
 * File           : focaltech_test_ft8722.c
 * Description: Source file for focal FT8722
 * Version   : 1.0
 * Date                : 2020/05/13
...
#define FTS_TEST_SAVE_INFO(fmt, args...) do { \
    if (fts8722_data->s) { \
        seq_printf(fts8722_data->s, fmt, ##args); \
    } \
    TPD_INFO(fmt, ##args); \
} while (0)
...
void fts8722_print_buffer(int *buffer, int length, int line_num)
{
    int i = 0;
    int j = 0;
    int tmpline = 0;
...
static int fts_test_bus_read(u8 *cmd, int cmdlen, u8 *data, int datalen)
{
    ...
    ret = fts8722_read(fts8722_data->spi, cmd, cmdlen, data, datalen);
    if (ret < 0)
        return ret;
    else
        return 0;
...
static int fts_test_bus_write(u8 *writebuf, int writelen)
{
    ...
    ret = fts8722_write(fts8722_data->spi, (char *)writebuf, writelen);
    if (ret < 0)
        return ret;
    else
        return 0;
...
static int get_channel_num(struct fts8722_ts_data *ts_data)
{
    int ret = 0;
    u8 tx_num = 0;
    u8 rx_num = 0;
...
#define NUM_MODE 2
static int fts_auto_preoperation(struct fts8722_ts_data *ts_data)
{
...
static int fts_short_test(struct fts8722_ts_data *ts_data, bool *test_result)
{
    int ret = 0;
    int i = 0;
    ...
    for (i = 0; i < node_num; i++) {
        ...
        ts_data->short_data[i] = (67 * tmp_adc + 7735) / (4095 - tmp_adc);
    }
...
static int fts_open_test(struct fts8722_ts_data *ts_data, bool *test_result)
{
    int ret = 0;
    int i = 0;
    bool tmp_result = false;
    ...
    ret = fts_test_write_reg(FACTORY_REG_OPEN_DV, FACTORY_VAL_OPEN_DV);
    if (ret < 0) {
    ...
    /* get cb data */
    ret = read_mass_data(FACTORY_REG_OPEN_ADDR2, byte_num, ts_data->open_data);
    if (ret) {
...
static int fts_cb_test(struct fts8722_ts_data *ts_data, bool *test_result)
{
    int ret = 0;
    int i = 0;
    bool tmp_result = false;
...
static int fts_rawdata_test(struct fts8722_ts_data *ts_data, bool *test_result)
{
    int ret = 0;
    int i = 0;
    bool tmp_result = false;
...
static int fts_lcdnoise_test(struct fts8722_ts_data *ts_data, bool *test_result)
{
    int ret = 0;
    int i = 0;
    bool tmp_result = false;
...
static void fts_auto_write_result(struct fts8722_ts_data *ts_data, int failed_count)
{
    //uint8_t  data_buf[64];
...
static int fts_auto_endoperation(struct fts8722_ts_data *ts_data)
{
    TPD_INFO("%s +\n", __func__);
    if (ts_data->short_data) {
        kfree(ts_data->short_data);
...
static int fts_start_test(struct fts8722_ts_data *ts_data)
{
    int ret = 0;
...
static void fts_autotest_endoperation(struct fts8722_ts_data *ts_data, const struct firmware *limit_fw)
{
    TPD_INFO("%s +\n", __func__);
...
static int fts_get_threshold_from_img(struct fts8722_ts_data *ts_data, char *data, const struct firmware *limit_fw)
{
...
if (FTS_THR_DEBUG) {
    ...
    ts_data->fts_autotest_offset->node_valid[8 * ts_data->hw_res->RX_NUM] = 0;
    ts_data->fts_autotest_offset->node_valid[9 * ts_data->hw_res->RX_NUM] = 0;
    ...
    for (i = 0; i < node_num; i++) {
        ...
        ts_data->fts_autotest_offset->fts_open_data_N[i] = 3500;
        ...
        ts_data->fts_autotest_offset->fts_raw_data_N[i] = 6000;
        ts_data->fts_autotest_offset->fts_raw_data_P[i] = 10000;

        ts_data->fts_autotest_offset->fts_lcdnoise_data_N[i] = 0;
        ts_data->fts_autotest_offset->fts_lcdnoise_data_P[i] = 230;
    }
...
static void fts_print_threshold(struct fts8722_ts_data *ts_data)
{
    int tx_num = ts_data->hw_res->TX_NUM;
    int rx_num = ts_data->hw_res->RX_NUM;
    int node_num = tx_num * rx_num;
    ...
    fts8722_print_buffer(ts_data->fts_autotest_offset->fts_short_data_P, node_num, rx_num);
    fts8722_print_buffer(ts_data->fts_autotest_offset->fts_short_data_N, node_num, rx_num);

    TPD_DEBUG("open threshold max/min:\n");
    fts8722_print_buffer(ts_data->fts_autotest_offset->fts_open_data_P, node_num, rx_num);
    fts8722_print_buffer(ts_data->fts_autotest_offset->fts_open_data_N, node_num, rx_num);

    TPD_DEBUG("cb threshold max/min:\n");
    fts8722_print_buffer(ts_data->fts_autotest_offset->fts_cb_data_P, node_num, rx_num);
    fts8722_print_buffer(ts_data->fts_autotest_offset->fts_cb_data_N, node_num, rx_num);

    TPD_DEBUG("rawdata threshold max/min:\n");
    fts8722_print_buffer(ts_data->fts_autotest_offset->fts_raw_data_P, node_num, rx_num);
    fts8722_print_buffer(ts_data->fts_autotest_offset->fts_raw_data_N, node_num, rx_num);

    TPD_DEBUG("lcdnoise threshold max/min:\n");
    fts8722_print_buffer(ts_data->fts_autotest_offset->fts_lcdnoise_data_P, node_num, rx_num);
    fts8722_print_buffer(ts_data->fts_autotest_offset->fts_lcdnoise_data_N, node_num, rx_num);
}

#define FTS_TEST_FW_NAME    "tp/20701/FW_FT8722_NF_90HZ_HLT_TEST.img"
static int fts_enter_test_environment(struct touchpanel_data *ts, bool test_state)
{
    ...
    if (test_state) {
        ...
        ret = request_firmware(&fw, FTS_TEST_FW_NAME, ts->dev);
        if (ret) {
...
int fts8722_test_entry(struct fts8722_ts_data *ts_data, bool black_screen)
{
    int ret = 0;
    struct touchpanel_data *ts = ts_data->ts;
    ...
    if (fts8722_data->s) {
        seq_printf(fts8722_data->s, "%d error(s). %s\n", ret, ret ? "" : "All test passed.");
    }
