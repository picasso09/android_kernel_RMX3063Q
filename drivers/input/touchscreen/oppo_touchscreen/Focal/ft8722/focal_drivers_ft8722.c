 ...
 * VENDOR_EDIT
 * File           : focaltech_drivers_ft8722.c
 * Description: focal FT8722 driver
 * Version   : 1.0
 * Date                : 2020/05/13
...
#include "ft8722_core.h"

struct fts8722_ts_data *fts8722_data = NULL;
extern int tp_register_times;

/*******Part0:LOG TAG Declear********************/

#define TPD_DEVICE "focaltech,ft8722"
...
u8 ft8722_fw_file[] = {
#include "include/RIO-X_HLT_B6_FT_01_app.i"
};
...
int fts8722_write(struct spi_device *spi, u8 *writebuf, u32 writelen)
{
    int ret = 0;
    int i = 0;
    u8 *txbuf = NULL;
...
int fts8722_write_reg(struct spi_device *spi, u8 addr, u8 value)
{
    u8 writebuf[2] = { 0 };

    writebuf[0] = addr;
    writebuf[1] = value;
    return fts8722_write(spi, writebuf, 2);
}

int fts8722_read(struct spi_device *spi, u8 *cmd, u32 cmdlen, u8 *data, u32 datalen)
{
    int ret = 0;
    int i = 0;
    u8 *txbuf = NULL;
...
int fts8722_read_reg(struct spi_device *spi, u8 addr, u8 *value)
{
    return fts8722_read(spi, &addr, 1, value, 1);
}
 ...
 */
static int fts_hw_reset(struct fts8722_ts_data *ts_data, u32 delayms)
{
    TPD_INFO("%s.\n", __func__);
...
{
    struct fts8722_ts_data *ts_data = (struct fts8722_ts_data *)chip_data;
...
static void fts_release_all_finger(struct fts8722_ts_data *ts_data)
{
...
{
    struct fts8722_ts_data *ts_data = container_of(work, struct fts8722_ts_data, prc_work.work);
...
static void fts_prc_queue_work(struct fts8722_ts_data *ts_data)
{
    if (ts_data->prc_enable) {
...
static int fts_point_report_check_init(struct fts8722_ts_data *ts_data)
{
    TPD_INFO("call.");
...
static ssize_t fts_debug_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    struct fts8722_ts_data *ts_data = PDE_DATA(file_inode(filp));
    ...
    case PROC_WRITE_REGISTER:
        ret = fts8722_write_reg(ts_data->spi, writebuf[1], writebuf[2]);
        if (ret < 0) {
    ...
    case PROC_WRITE_DATA:
        writelen = buflen - 1;
        ret = fts8722_write(ts_data->spi, writebuf + 1, writelen);
        if (ret < 0) {
...
static ssize_t fts_debug_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    ...
    case PROC_READ_REGISTER:
        num_read_chars = 1;
        ret = fts8722_read_reg(ts_data->spi, proc->cmd[0], &readbuf[0]);
        if (ret < 0) {
    ...
    case PROC_READ_DATA:
        ...
        ret = fts8722_read(ts_data->spi, proc->cmd, proc->cmd_len, readbuf, num_read_chars);
        if (ret < 0) {
...
static int fts_create_apk_debug_channel(struct fts8722_ts_data *ts_data)
{
...
static void fts_release_apk_debug_channel(struct fts8722_ts_data *ts_data)
{
    struct ftxxxx_proc *proc = &ts_data->proc;
...
static int focal_dump_reg_state(void *chip_data, char *buf)
{
    ...
    fts8722_read_reg(ts_data->spi, FTS_REG_POWER_MODE, &regvalue);
    count += sprintf(buf + count, "Power Mode:0x%02x\n", regvalue);

    //FW version
    fts8722_read_reg(ts_data->spi, FTS_REG_FW_VER, &regvalue);
    count += sprintf(buf + count, "FW Ver:0x%02x\n",regvalue);

    //Vendor ID
    fts8722_read_reg(ts_data->spi, FTS_REG_VENDOR_ID, &regvalue);
    count += sprintf(buf + count, "Vendor ID:0x%02x\n", regvalue);

    // 1 Gesture mode,0 Normal mode
    fts8722_read_reg(ts_data->spi, FTS_REG_GESTURE_EN, &regvalue);
    count += sprintf(buf + count, "Gesture Mode:0x%02x\n", regvalue);

    // 3 charge in
    fts8722_read_reg(ts_data->spi, FTS_REG_CHARGER_MODE_EN, &regvalue);
    count += sprintf(buf + count, "charge stat:0x%02x\n", regvalue);

    //Interrupt counter
    fts8722_read_reg(ts_data->spi, FTS_REG_INT_CNT, &regvalue);
    count += sprintf(buf + count, "INT count:0x%02x\n", regvalue);

    //Flow work counter
    fts8722_read_reg(ts_data->spi, FTS_REG_FLOW_WORK_CNT, &regvalue);
...
static int focal_get_fw_version(void *chip_data)
{
    ...
    fts8722_read_reg(ts_data->spi, FTS_REG_FW_VER, &fw_ver);
    return (int)fw_ver;
}
...
static int fts_esd_handle(void *chip_data)
{
    ...
    ret = fts8722_read_reg(ts_data->spi, 0x00, &val);
    if ((ret >= 0) && ((val & 0x70) == 0x40)) { //work in factory mode
        goto NORMAL_END;
    }

    for (i = 0; i < 3; i++) {
        ret = fts8722_read_reg(ts_data->spi, FTS_REG_CHIP_ID, &val);
    ...
    ret = fts8722_read_reg(ts_data->spi, FTS_REG_FLOW_WORK_CNT, &val);
    if (ret < 0) {
...
static int fts_enter_into_boot(struct fts8722_ts_data *ts_data)
{
    int ret = 0;
    int i = 0;
    u8 cmd = 0;
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
        ...
        /* check boot id*/
        cmd = FTS_CMD_START;
        ret = fts8722_write(ts_data->spi, &cmd, 1);
        mdelay(FTS_CMD_START_DELAY);
        cmd = FTS_CMD_READ_ID;
        ret = fts8722_read(ts_data->spi, &cmd, 1, id, 2);
...
static int fts_dpram_write(struct fts8722_ts_data *ts_data, u32 saddr, const u8 *buf, u32 len, bool wpram)
{
    int ret = 0;
    int i = 0;
    int j = 0;
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        cmd[2] = (addr >> 8);
        cmd[3] = (addr);
        ret = fts8722_write(ts_data->spi, &cmd[0], 4);
        if (ret < 0) {
        ...
        ret = fts8722_write(ts_data->spi, &cmd[0], 1 + packet_len);
        if (ret < 0) {
...
static int fts_ecc_cal_tp(struct fts8722_ts_data *ts_data, u32 ecc_saddr, u32 ecc_len, u16 *ecc_value)
{
    int ret = 0;
    int i = 0;
    ...
    ret = fts8722_write(ts_data->spi, cmd, FTS_CMD_ECC_LEN);
    if (ret < 0) {
    ...
    for (i = 0; i < FTS_ECC_FINISH_TIMEOUT; i++) {
        ret = fts8722_read(ts_data->spi, cmd, 1, value, 1);
        if (ret < 0) {
    ...
    ret = fts8722_read(ts_data->spi, cmd, 1, value, 2);
    if (ret < 0) {
...
static int fts_pram_write_ecc(struct fts8722_ts_data *ts_data, u8 *buf, u32 len)
{
    int ret = 0;
...
static int fts_dram_write_ecc(struct fts8722_ts_data *ts_data, u8 *buf, u32 len)
{
    int ret = 0;
    u32 dram_size = 0;
    u32 pram_app_size = 0;
...
static int fts_pram_start(struct fts8722_ts_data *ts_data)
{
    int ret = 0;
...
static int fts_fw_write_start(struct fts8722_ts_data *ts_data, u8 *buf, u32 len, bool need_reset)
{
    int ret = 0;
...
static int fts_fw_download(struct fts8722_ts_data *ts_data, u8 *buf, u32 len, bool need_reset)
{
    int ret = 0;
    int i = 0;
...
void fts8722_auto_test(struct seq_file *s, void *chip_data, struct focal_testdata *focal_testdata)
{
    struct fts8722_ts_data *ts_data = (struct fts8722_ts_data *)chip_data;
...
static int fts_enter_factory_work_mode(struct fts8722_ts_data *ts_data, u8 mode_val)
{
    int ret = 0;
    int retry = 20;
    u8 regval = 0;
    ...
    ret = fts8722_write_reg(ts_data->spi, DEVIDE_MODE_ADDR, mode_val);
    if (ret < 0) {
    ...
    while (--retry) {
        ret = fts8722_read_reg(ts_data->spi, DEVIDE_MODE_ADDR, &regval);
        if (regval == mode_val)
            break;
        msleep(20);
    }
...
static int fts_start_scan(struct fts8722_ts_data *ts_data)
{
    int ret = 0;
    int retry = 50;
    u8 regval = 0;
    ...
    ret = fts8722_write_reg(ts_data->spi, DEVIDE_MODE_ADDR, scanval);
    if (ret < 0) {
...
static int fts_get_rawdata(struct fts8722_ts_data *ts_data, int *raw, bool is_diff)
{
    int ret = 0;
    int i = 0;
    ...
    if (is_diff) {
        ret = fts8722_read_reg(ts_data->spi, FACTORY_REG_DATA_SELECT, &regval);
        ret = fts8722_write_reg(ts_data->spi, FACTORY_REG_DATA_SELECT, 0x01);
        if (ret < 0) {
    ...
    ret = fts8722_write_reg(ts_data->spi, FACTORY_REG_LINE_ADDR, 0xAD);
    if (ret < 0) {
    ...
    raw_addr = 0x6A;
    ret = fts8722_read(ts_data->spi, &raw_addr, 1, buf, byte_num);
...
reg_restore:
    if (is_diff) {
        ret = fts8722_write_reg(ts_data->spi, FACTORY_REG_DATA_SELECT, regval);
        if (ret < 0) {
...
static void fts_delta_read(struct seq_file *s, void *chip_data)
{
    ...
    ret = fts8722_write_reg(ts_data->spi, FTS_REG_AUTOCLB_ADDR, 0x01);
    if (ret < 0){
...
static void fts_main_register_read(struct seq_file *s, void *chip_data)
{
    ...
    /*edge limit*/
    fts8722_read_reg(ts_data->spi, FTS_REG_EDGE_LIMIT, &regvalue);
    seq_printf(s, "edge Mode:0x%02x\n", regvalue);

    /*game mode*/
    fts8722_read_reg(ts_data->spi, FTS_REG_GAME_MODE_EN, &regvalue);
    seq_printf(s, "Game Mode:0x%02x\n", regvalue);

    /*FOD mode*/
    fts8722_read_reg(ts_data->spi, FTS_REG_FOD_EN, &regvalue);
    ...
    /*Panel ID*/
    fts8722_read_reg(ts_data->spi, FTS_REG_MODULE_ID, &regvalue);
...
static int fts_enable_black_gesture(struct fts8722_ts_data *ts_data, bool enable)
{
    TPD_INFO("MODE_GESTURE, write 0xD0=%d", enable);
    return fts8722_write_reg(ts_data->spi, FTS_REG_GESTURE_EN, enable);
}

static int fts_enable_edge_limit(struct fts8722_ts_data *ts_data, bool enable)
{
    u8 edge_mode = 0;
    ...
    return fts8722_write_reg(ts_data->spi, FTS_REG_EDGE_LIMIT, edge_mode);
}

static int fts_enable_charge_mode(struct fts8722_ts_data *ts_data, bool enable)
{
    TPD_INFO("MODE_CHARGE, write 0x8B=%d", enable);
    return fts8722_write_reg(ts_data->spi, FTS_REG_CHARGER_MODE_EN, enable);
}

static int fts_enable_game_mode(struct fts8722_ts_data *ts_data, bool enable)
{
    TPD_INFO("MODE_GAME, write 0x86=%d", enable);
    return fts8722_write_reg(ts_data->spi, FTS_REG_GAME_MODE_EN, !enable);;
}

static int fts_enable_headset_mode(struct fts8722_ts_data *ts_data, bool enable)
{
    TPD_INFO("MODE_HEADSET, write 0xC3=%d \n", enable);
    return fts8722_write_reg(ts_data->spi, FTS_REG_HEADSET_MODE_EN, enable);
}
...
static int fts_mode_switch(void *chip_data, work_mode mode, bool flag)
{
    ...
    case MODE_SLEEP:
        ...
        ret = fts8722_write_reg(ts_data->spi, FTS_REG_POWER_MODE, 0x03);
        if (ret < 0) {
...
static int fts_get_chip_info(void *chip_data)
{
    ...
    for (cnt = 0; cnt < 3; cnt++) {
        ...
        /* check boot id*/
        cmd = FTS_CMD_START;
        fts8722_write(ts_data->spi, &cmd, 1);
        mdelay(FTS_CMD_START_DELAY);
        cmd = FTS_CMD_READ_ID;
        fts8722_read(ts_data->spi, &cmd, 1, id, 2);
        TPD_INFO("read boot id:0x%02x%02x", id[0], id[1]);
        if (id[0] == FTS_VAL_BOOT_ID) {
            tpd_load_status = 1;
            return 0;
...
{
    int ret = -1;
	struct fts8722_ts_data *chip_info = (struct fts8722_ts_data *)chip_data;
...
static fw_check_state fts_fw_check(void *chip_data, struct resolution_info *resolution_info, struct panel_info *panel_data)
{
    ...
    for(i = 0; i < 10; i++) {
        msleep(10);
        fts8722_read_reg(ts_data->spi, FTS_REG_CHIP_ID, &id[0]);
    ...
    fts8722_read_reg(ts_data->spi, FTS_REG_FW_VER, &ts_data->fwver);
...
static int fts_fw_recovery(struct fts8722_ts_data *ts_data)
{
    int ret = 0;
    u8 boot_state = 0;
    u8 chip_id = 0;
    ...
    ret = fts8722_read_reg(ts_data->spi, 0xD0, &boot_state);
    if (ret < 0) {
   ...
   TPD_INFO("ts_data->h_fw_size == %d\n", ts_data->h_fw_size);
    ...
    msleep(10);
    ret = fts8722_read_reg(ts_data->spi, FTS_REG_CHIP_ID, &chip_id);
...
static u8 fts_trigger_reason(void *chip_data, int gesture_enable, int is_suspended)
{
    ...
    ret = fts8722_read(ts_data->spi, &cmd, 1, &buf[0], FTS_REPORT_BUFFER_SIZE);
    if (ret < 0) {
...
static void fts_health_report(void *chip_data, struct monitor_data *mon_data)
{
    ...
    ret = fts8722_read_reg(ts_data->spi, 0x01, &val);
    TPD_INFO("Health register(0x01):0x%x", ret);
    ret = fts8722_read_reg(ts_data->spi, FTS_REG_HEALTH_1, &val);
    TPD_INFO("Health register(0xFD):0x%x", ret);
    ret = fts8722_read_reg(ts_data->spi, FTS_REG_HEALTH_2, &val);
...
static void fts_register_info_read(void *chip_data, uint16_t register_addr, uint8_t *result, uint8_t length)
{
    ...
    fts8722_read(ts_data->spi, &addr, 1, result, length);
}
...
void fts8722_black_screen_test(void *chip_data, char *msg)
{
    int ret = 0;
    struct fts8722_ts_data *ts_data = (struct fts8722_ts_data *)chip_data;
    struct touchpanel_data *ts = ts_data->ts;
    ...
    ret = fts8722_test_entry(ts_data, 1);
...
static struct oppo_touchpanel_operations fts_ops = {
    ...
    .black_screen_test          = fts8722_black_screen_test,
};

static struct fts_proc_operations fts_proc_ops = {
    .auto_test              = fts8722_auto_test,
};
...
struct focal_debug_func focal8722_debug_ops =
{
...
{
        struct fts8722_ts_data *ts_data;
        ...
        spi->mode = SPI_MODE_0;
        spi->bits_per_word = 8;
		spi->max_speed_hz = 6000000;  //for focal spec 6M
...
*/
        ...
        ts_data = kzalloc(sizeof(struct fts8722_ts_data), GFP_KERNEL);
        if (ts_data == NULL) {
        ...
        ts_data->h_fw_file = ft8722_fw_file;
        ts_data->h_fw_size = sizeof(ft8722_fw_file);
        ...
        ts->ts_ops = &fts_ops;
        ts->private_data = &focal8722_debug_ops;
        ...
        fts_create_proc_new(ts, ts_data->proc_ops);
...
static int fts_tp_remove(struct spi_device *spi)
{
        ...
        struct fts8722_ts_data *ts_data = (struct fts8722_ts_data *)ts->chip_data;
