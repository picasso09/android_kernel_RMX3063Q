...
#define FTS_TEST_FW_NAME    "tp/20701/FW_FT8006S_NF_90HZ_TRULY_TEST.img"
#define FTS_HLT_TEST_FW_NAME    "tp/20701/FW_FT8006S_NF_90HZ_HLT_TEST.img"
static int fts_enter_test_environment(struct touchpanel_data *ts, bool test_state)
{
 ...
 //     ret = request_firmware(&fw, FTS_TEST_FW_NAME, ts->dev);
        if(strstr(ts->panel_data.fw_name,"FW_FT8006S_NF_90HZ_TRULY")){
		       if (request_firmware(&fw, FTS_TEST_FW_NAME, ts->dev)) {
			       TPD_INFO("request_firmware(%s) fail", FTS_TEST_FW_NAME);
			       return -ENODATA;
	           }
        }else {
		        if(request_firmware(&fw, FTS_HLT_TEST_FW_NAME, ts->dev)) {
		    		TPD_INFO("request_firmware(%s) fail", FTS_HLT_TEST_FW_NAME);
		            return -ENODATA;
		    	}
        }
    } else {
        /*write normal firmware.bin*/
		TPD_INFO(" %s %d fw_name = %s \n",__func__,__LINE__,ts->panel_data.fw_name);
