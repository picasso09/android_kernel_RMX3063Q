...
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2019/04/09 modified for protocol B */
...
#ifdef CONFIG_HQ_HARDWARE_INFO	//add for hardwareinfo
	unsigned char ctp_fwversion[10];
#endif
#include <mt-plat/mtk_boot.h>
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2018/02/21 add for proc node*/
#define  CONFIG_GTP_PROC
static int tpd_flag;
int tpd_halt = 0;
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2019/03/14 modified for debug level function */
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2019/04/09 modified for protocol B */
#ifndef GOODIX_PROTOCOL_B_EN
static int pre_id[10] = {0};
static int cur_id[10] = {0};
#endif
 struct  point{
	int id[10];
	int x[10];
...
static struct point pre;
static struct point cur;
extern struct tp_devinfo oppo_tp_data;
static int tpd_eint_mode = 1;
static struct task_struct *thread;
static int tpd_polling_time = 50;
/*Le.Wang@ODM_HQ.BSP.TP.Function, 2019/01/22 add for device info*/
#define GOODIX_NAME		"goodix"
#define GOODIX_UPGRADE_FW_FILE  "gt9xx_fw.bin"
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2018/03/25 add for major*/
#define GTP_SMART_GESTURE_THRESHOLD 0x0B
#define GTP_SMART_GESTURE_LOW_VALUE 0x05

static DECLARE_WAIT_QUEUE_HEAD(waiter);
static DEFINE_MUTEX(i2c_access);
static atomic_t  irq_enabled = ATOMIC_INIT(0);
static unsigned int touch_irq;
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2019/03/14 modified for irq function */
unsigned int oppo_irq;
...
#endif
//#define gt_use_reguler_vdd28
//add by ma for pinctr
#define gt_use_pinctrl_power_on
#ifdef  gt_use_pinctrl_power_on
extern struct pinctrl *pinctrl1;
struct pinctrl *pinctrl_gt;
struct pinctrl_state *gt_2v8_output_low =  NULL;
struct pinctrl_state *gt_2v8_output_high = NULL;
#endif

#ifdef  gt_use_pinctrl_power_on
static int gt_get_power_pinctl(struct i2c_client *client);
static void gt_tpd_power_output(int level);
#endif
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2018/02/21 add for proc node*/
...
#ifdef CONFIG_OF
static const struct of_device_id gt9xx_dt_match[] = { {.compatible = "mediatek,cap_touch_gt"}, {},};
#endif
...
s32 gtp_read_version(struct i2c_client *client, u16 *version)
{
		...
		if (buf[5] == 0x00) {
			...
			buf[2], buf[3], buf[4], buf[5], buf[7], buf[6]);
                ...
                /*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2018/02/23 add for firmware version*/
                oppo_tp_data.version= buf[6];
		return ret;
}
...
s32 gtp_init_panel(struct i2c_client *client)
{
	...
	(!cfg_info_len[3]) && (!cfg_info_len[4]) && (!cfg_info_len[5])) {
		sensor_id = 0;
		GTP_INFO("GTSensor_ID: %d", sensor_id);
	} else {
#ifdef CONFIG_GTP_COMPATIBLE_MODE
		if (CHIP_TYPE_GT9F == gtp_chip_type)
			msleep(50);
...
#else
	{
		...
		if (ret == SUCCESS) {
			...
			flash_cfg_version = opr_buf[0];
                        /*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2018/02/23 add for firmware version*/
			//oppo_tp_data.version= opr_buf[0];
...
void input_device_init()
{
	  ...
	  input_mt_init_slots(tpd->dev,10, INPUT_MT_DIRECT);
}
#endif
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/*Le.Wang@ODM_HQ.BSP.TP.Function, 2018/01/25 add for factory and meta*/

	boot_mode = get_boot_mode();
	GTP_INFO("==========boot_mode = %d\n",boot_mode);

	if ((boot_mode == META_BOOT|| boot_mode == FACTORY_BOOT )) {
	    GTP_ERROR("boot_mode is FACTORY or META mode not need to use tp\n");
	    goto out;
	}

#ifdef  gt_use_pinctrl_power_on    
	ret = gt_get_power_pinctl(client); //getting power pinctl
    if (ret){
         GTP_ERROR("get power pinctl failed");
    }
 
       gt_tpd_power_output(1);    //  ft5446 power on
#endif    
...
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2019/04/09 modified for protocol B */
#ifdef  GOODIX_PROTOCOL_B_EN
	input_device_init();//FOR b REPORT
#endif
...
#endif
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2019/04/09 modified for protocol B */
	memset(&pre,0,sizeof(struct point));
    memset(&cur,0,sizeof(struct point));

#ifdef CONFIG_GTP_AUTO_UPDATE
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2018/03/25 add for del vendor audo upload fw*/
/*
...
#endif
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2018/02/21 add for proc node*/
#ifdef CONFIG_GTP_PROC
	gtp_proc_init(client);
#endif

#ifdef CONFIG_HQ_HARDWARE_INFO
	sprintf(ctp_fwversion, "HXD V0x%x", tpd_info.vid);
	get_hardware_info_data(HWID_CTP_FW_INFO, ctp_fwversion);
#endif
/*Le.Wang@ODM_HQ.BSP.TP.Function, 2019/01/22 add for device info*/
	oppo_tp_data.manufacture = GOODIX_NAME;
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2019/03/14 modified for node function */
	oppo_tp_data.fw_name = GTP_UPGRADE_FW_FILE_SIGNED;
	return 0;
out:
	gtp_free_gpio_res();
	return -1;
...

{
	...
	GTP_INFO("[gt917d] focal_tpd_pinctrl+++++++++++++++++\n");
	//pinctrl_fts = devm_pinctrl_get(dev);
	pinctrl_gt = pinctrl1;
	if (IS_ERR(pinctrl_gt)) {
		ret = PTR_ERR(pinctrl_gt);
		GTP_ERROR("gt917 Cannot find pinctrl_gt!\n");
		return ret;
	}
	 gt_2v8_output_low = pinctrl_lookup_state(pinctrl_gt, "state_eint_output0");
	if (IS_ERR(gt_2v8_output_low)) {
		ret = PTR_ERR(gt_2v8_output_low);
		GTP_ERROR("Cannot find pinctrl state_eint_output0 %d!\n", ret);
		return ret;
	}
	 gt_2v8_output_high = pinctrl_lookup_state(pinctrl_gt, "state_eint_output1");
	if (IS_ERR(gt_2v8_output_high)) {
		ret = PTR_ERR(gt_2v8_output_high);
		GTP_ERROR("Cannot find pinctrl state_eint_output1 %d!\n", ret);
		return ret;
	}
	return 0;
}
...
{
	if (level)
			pinctrl_select_state(pinctrl_gt, gt_2v8_output_high);
		else
			pinctrl_select_state(pinctrl_gt, gt_2v8_output_low);
	GTP_INFO(" gt917 power output successful \n");
...
static void gtp_esd_check_func(struct work_struct *work)
{
		...
		if (1 == gtp_ito_test_on) {
			GTP_INFO("gtp_ito_test is running");
			return;
		}
		for (i = 0; i < 3; i++) {
...
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2019/04/09 modified for protocol B */
	...
	if((id & 0x80)) {//pen
		...
		input_mt_report_slot_state(tpd->dev, MT_TOOL_PEN, true);
		if(!size){
		...
		input_report_abs(tpd->dev, ABS_MT_PRESSURE, size);
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2019/04/09 modified for don't report major in y this areas */
                if ((x > 0) && (x < 720) && (y > 36) && (y < 1524)){
			 if(size > GTP_SMART_GESTURE_THRESHOLD){
		                 input_report_abs(tpd->dev, ABS_MT_TOUCH_MAJOR, size);
	                 }else{
	                         input_report_abs(tpd->dev, ABS_MT_TOUCH_MAJOR, GTP_SMART_GESTURE_LOW_VALUE);
			 }
                 }
         }
...
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2019/04/09 modified for protocol B */
			...
			if((id & 0x80)) {//pen
				id = 10;
				input_mt_slot(tpd->dev, id);
				input_mt_report_slot_state(tpd->dev, MT_TOOL_PEN, false);
			} else {
				//finger
				id = id & 0x0F;
				input_mt_slot(tpd->dev, id);
...
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2019/04/09 modified for protocol B */
		if (touch_num != 0 && pre_touch == 0) {
					input_report_key(tpd->dev, BTN_TOUCH, 1);
		} else if (touch_num == 0 && pre_touch != 0) {
					input_report_key(tpd->dev, BTN_TOUCH, 0);
		}
#ifdef GOODIX_PROTOCOL_B_EN
			...
			for (i = 0; i < GTP_MAX_TOUCH; i++) {
				if (touch_index & (0x01 << i)) {
					...
					input_y = TPD_WARP_Y(abs_y_max, input_y);
						cur.id[id] = 1;
						if (oppo_debug_level == 1 && pre.id[id] != 1) {
			                                GTP_INFO("oppo [B]P%d(%d, %d)[tm:%d] DOWN!\n", id,input_x, input_y, input_w);
							pre.id[id]=1;
							pre.x[id]=input_x ;
							pre.y[id]=input_y;
							pre.w[id]= input_w;
						} else if (oppo_debug_level == 2) {
			                                GTP_INFO("oppo [B]P%d(%d, %d)[tm:%d] DOWN!\n", id,input_x, input_y, input_w);
							pre.id[id]=1;
							pre.x[id]=input_x ;
							pre.y[id]=input_y;
					...
					tpd_up(0, 0, i);
					pre_touch &= ~(0x01 << i);
					 if ((oppo_debug_level == 1) || (oppo_debug_level == 2)){
						if(oppo_debug_level==1&&pre.id[i] ==1)
                                                {
                                                        GTP_INFO("oppo [B]P%d(%d, %d)[tm:%d] UP!\n", i,pre.x[i], pre.y[i], pre.w[i]);
					         }else if (oppo_debug_level==2&&pre.id[i] ==1){
							GTP_INFO("oppo [B]P%d(%d, %d)[tm:%d] UP!\n", i,pre.x[i], pre.y[i], pre.w[i]);
					         }
                                         }
					pre.id[i]= 0;
 ...
 /*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2019/03/14 modified for node function */
			cur_id[id] = 1;
                        if (oppo_debug_level == 1 && pre_id[id] != 1) {
                                GTP_INFO("oppo [B]P%d(%d, %d)[tm:%d] DOWN!\n", id,input_x, input_y, input_w);
                                pre_id[id] =1;
			} else if (oppo_debug_level == 2) {
                                GTP_INFO("oppo [B]P%d(%d, %d)[tm:%d] DOWN!\n", id,input_x, input_y, input_w);
...
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2019/03/14 modified for node function */
		...
		for(i = 0; i < 10; i++) {
			    ...
			    pre_finger = 0;
                         tpd_up(0, 0, 0);
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2019/03/14 modified for node function */
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2019/03/25 modified for debug print log */
                         if ((oppo_debug_level == 1) || (oppo_debug_level == 2)) {
                               GTP_INFO("oppo [B]P%d(%d, %d)[tm:%d] UP!\n", id,input_x, input_y, input_w);
			}
			for(i = 0; i < 10; i++) {
...
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2019/03/14 modified for node function */
/*Jianchao.Gu@ODM_HQ.BSP.TP.Function, 2019/03/14 modified for tp calibration value = NONE */
...
static void tpd_suspend(struct device *h)
{
	...
	if (gtp_ito_lcmoff_flag == 1){
     GTP_INFO("lcmoff_ito_test");
	    return;
	}
	if(1 == gtp_loading_fw)
	{
		GTP_INFO("TP in fw updating,suspend abort!\n");
		return;
	}
