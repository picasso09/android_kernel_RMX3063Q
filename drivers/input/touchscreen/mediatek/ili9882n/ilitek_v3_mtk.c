...
static int __parse_tag_videolfb(struct device_node *node)
{
	...
	ili_ctpmodule = strncmp(mtkfb_lcm_name,"ili9882n_txd_inx",strlen("ili9882n_txd_inx"));

	if ( ili_ctpmodule == 0 ) {
		ILI_INFO("this is ili9882n_txd_inx touchscreen\n");
	}else{
				ILI_ERR("nothing match lcd name\n");
				return -1;
	}

	ILI_INFO("ili_ctpmodule = %d\n", ili_ctpmodule);
