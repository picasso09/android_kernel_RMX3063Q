...
static int __parse_tag_videolfb(struct device_node *node)
{
	...
	nt_ctpmodule = strncmp(mtkfb_lcm_name,"nt36525b_hlt_boe",strlen("nt36525b_hlt_boe"));

	if ( nt_ctpmodule == 0 ) {
		NVT_LOG("this is nt36525b_hlt_boe touchscreen\n");
	}else{
...
*/
	...
	gpio_set_value(ts->reset_gpio, 1);
#endif
	if (nvt_update_firmware(fw->firmware_name)) {
