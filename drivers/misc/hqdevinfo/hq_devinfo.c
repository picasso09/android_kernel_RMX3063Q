...
static void match_sub_board(int boardid){
	...
	if(0x0 == MB){
		...
		if((0x3 == KB) ||(0x2 == KB) || ((0x0 == KB) && (0xc == MB))){
			sprintf(buff, "Device version: %s\nDevice manufacture: %s","MTK","sub-match");
		}else{
			sprintf(buff, "Device version: %s\nDevice manufacture: %s","MTK","sub-unmatch");
		}
...
/* wangcheng@ODM.Multimedia.LCD  2020/09/10 add  for add lcd device info   start */
		}else if(!strcmp(hq_lcm_name, "ili9882n_txd_inx")){
			sprintf(buff, "Device version: %s\nDevice manufacture: %s\n","ili9882n","TXD_INX");
		}else if(!strcmp(hq_lcm_name, "nt36525b_hlt_boe")){
			sprintf(buff, "Device version: %s\nDevice manufacture: %s\n","nt36525b","HLT_BOE");
/* wangcheng@ODM.Multimedia.LCD  2020/09/10 add  for add lcd device info   end */
		}else{
...
static int __init devinfo_init(void){
	...
	if (!is_project(OPPO_20701)) {
		...
		for (i = 0; i < num; i++) {
			...
			if (prEntry)
				continue;
			pr_err("[%s]: failed to create /proc/devinfo/%s\n", __func__, devinfo_proc_list[i]);
		}
