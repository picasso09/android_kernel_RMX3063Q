...
/* wangcheng@ODM.Multimedia.LCD  2020/08/31 add for LCD bring up  begin */
#ifdef CONFIG_SET_LCD_BIAS_ODM_HQ
#include "../../../lcd_bias/lcd_bias.h"
#endif //CONFIG_SET_LCD_BIAS_ODM_HQ
/* wangcheng@ODM.Multimedia.LCD  2020/08/31 add for LCD bring up  end */
/*****************************************************************************/
...
int mipi_clk_change(int msg, int en)
{
 ...
 /*wangcheng@PSW.MM.Display.LCD.Stability, 2020/10/13, add for mipi clk change  start */
		} else if (!strcmp(mtkfb_lcm_name,"ili9882n_txd_inx") || !strcmp(mtkfb_lcm_name,"ili9882n_mn228_txd_inx")) {
			def_data_rate = 640; /* data_rate = PLL_CLK*2 in lcm driver */
			def_dsi_hbp = 0x116; /*  adaptive HBP value (dsi_params->horizontal_backporch + dsi_params->horizontal_sync_active)*dsiTmBufBpp - 10)  */
			DISPERR("%s,ili9882n_txd_inx ili9882n_mn228_txd_inx def_data_rate=%d,def_dsi_hbp= %d, en=%d\n", __func__, def_data_rate, def_dsi_hbp,en);
		} else if (!strcmp(mtkfb_lcm_name,"nt36525b_hlt_boe")) {
			def_data_rate = 640; /* data_rate = PLL_CLK*2 in lcm driver */
			def_dsi_hbp = 0x10D; /*  adaptive HBP value (dsi_params->horizontal_backporch + dsi_params->horizontal_sync_active)*dsiTmBufBpp - 10)  */
			DISPERR("%s,nt36525b_hlt_boe def_data_rate=%d,def_dsi_hbp= %d, en=%d\n", __func__, def_data_rate, def_dsi_hbp,en);
		}else if (!strcmp(mtkfb_lcm_name,"hx83102d_txd_inx")) {
			def_data_rate = 564; /* data_rate = PLL_CLK*2 in lcm driver */
			def_dsi_hbp = 0x86; /*  adaptive HBP value (dsi_params->horizontal_backporch + dsi_params->horizontal_sync_active)*dsiTmBufBpp - 10)  */
			DISPERR("%s,hx83102d_txd_inx def_data_rate=%d,def_dsi_hbp= %d, en=%d\n", __func__, def_data_rate, def_dsi_hbp,en);
		}else if (!strcmp(mtkfb_lcm_name,"ft8006s_hlt_boe")) {
			def_data_rate = 564; /* data_rate = PLL_CLK*2 in lcm driver */
			def_dsi_hbp = 0x83; /*  adaptive HBP value (dsi_params->horizontal_backporch + dsi_params->horizontal_sync_active)*dsiTmBufBpp - 10)  */
			DISPERR("%s,ft8006s_hlt_boe def_data_rate=%d,def_dsi_hbp= %d, en=%d\n", __func__, def_data_rate, def_dsi_hbp,en);
		}else if (!strcmp(mtkfb_lcm_name,"hx83102d_truly_truly")) {
			def_data_rate = 564; /* data_rate = PLL_CLK*2 in lcm driver */
			def_dsi_hbp = 0x86; /*  adaptive HBP value (dsi_params->horizontal_backporch + dsi_params->horizontal_sync_active)*dsiTmBufBpp - 10)  */
			DISPERR("%s,hx83102d_truly_truly def_data_rate=%d,def_dsi_hbp= %d, en=%d\n", __func__, def_data_rate, def_dsi_hbp,en);
		} else if (!strcmp(mtkfb_lcm_name,"ft8722_hlt_boe_eighth")) {
			def_data_rate = 564; /* data_rate = PLL_CLK*2 in lcm driver */
			def_dsi_hbp = 0x83; /*  adaptive HBP value (dsi_params->horizontal_backporch + dsi_params->horizontal_sync_active)*dsiTmBufBpp - 10)  */
			DISPERR("%s,ft8722_hlt_boe_eighth def_data_rate=%d,def_dsi_hbp= %d, en=%d\n", __func__, def_data_rate, def_dsi_hbp,en);
		}else if (!strcmp(mtkfb_lcm_name,"ft8006saa_truly_truly")) {
			def_data_rate = 564; /* data_rate = PLL_CLK*2 in lcm driver */
			def_dsi_hbp = 0x83; /*  adaptive HBP value (dsi_params->horizontal_backporch + dsi_params->horizontal_sync_active)*dsiTmBufBpp - 10)  */
			DISPERR("%s,ft8006saa_truly_truly def_data_rate=%d,def_dsi_hbp= %d, en=%d\n", __func__, def_data_rate, def_dsi_hbp,en);
		}else {
 /*wangcheng@PSW.MM.Display.LCD.Stability, 2020/10/13, add for mipi clk change  end */
