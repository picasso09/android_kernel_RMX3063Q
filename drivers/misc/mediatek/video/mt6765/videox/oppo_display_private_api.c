				...
				struct device_attribute *attr, char *buf)
{
	...
	if ((oppo_display_panelid_support) && (lcm_id_addr != 0)) {
		if (oppo_display_panelid_support) {
...
/* Xiaojun.Lv@MM.Lcd.Driver, 2020/10/10, modify for highlight mode */
...
/* Xiaojun.Lv@MM.Lcd.Driver, 2020/09/20, Make dre global */
	if (get_project() == 20701) {
		disp_aal_set_dre_en(1);
		printk("%s 20701 enable dre\n", __func__);
	} else {
	if (CABC_mode == 0) {
        disp_aal_set_dre_en(1);
        printk("%s enable dre\n", __func__);
...
/* wangcheng@MM.Display.LCD.Stability, 2020/09/27, modify for  project read /sys/kernel/oppo_display/panel_serial_number*/
#ifdef ODM_HQ_EDIT
static DEVICE_ATTR(panel_id, S_IRUGO|S_IWUSR, lcm_id_info_show, lcm_id_info_store);
static DEVICE_ATTR(panel_serial_number, S_IRUGO|S_IWUSR, mdss_get_panel_serial_number, panel_serial_store);
#endif		//ODM_HQ_EDIT
