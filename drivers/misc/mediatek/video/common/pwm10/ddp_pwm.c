...
#include <ddp_drv.h>
#include <ddp_pwm.h>
/* Xiaojun.Lv@MM.Lcd.Driver, 2020/09/14, Mtk customized 12bit backlight */
...
static atomic_t g_pwm_max_backlight_rio[PWM_TOTAL_MODULE_NUM] = {
...
static atomic_t g_pwm_max_backlight_rio[PWM_TOTAL_MODULE_NUM] = { ATOMIC_INIT(4095) };
#endif
...
/* Xiaojun.Lv@MM.Lcd.Driver, 2020/09/14, Mtk customized 12bit backlight */
	if((get_project() == 20701)){
		atomic_set(&g_pwm_max_backlight_rio[index], level_1024);
	}else{
...
/* Xiaojun.Lv@MM.Lcd.Driver, 2020/09/14, Mtk customized 12bit backlight */
	if((get_project() == 20701)){
		return atomic_read(&g_pwm_max_backlight_rio[index]);
    }else{
