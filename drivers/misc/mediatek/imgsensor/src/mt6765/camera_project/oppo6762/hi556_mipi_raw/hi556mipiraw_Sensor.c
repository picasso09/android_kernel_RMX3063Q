    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
            ...
            if(Hi556_read_region(NULL,0,&vendor_id,1) < 0){
                pr_err("hi556 read otp failed");
                *sensor_id = 0xFFFFFFFF;
                return ERROR_SENSOR_CONNECT_FAIL;
            }else{
                 if(0x57 == vendor_id){
                     LOG_INF("hi556 i2c write id: 0x%x, sensor id: 0x%x vendor_id 0x%x\n",imgsensor.i2c_write_id,*sensor_id,vendor_id);
                     return ERROR_NONE;
                 }else{
                      pr_err("hi556 Read sensor id fail i2c write id: 0x%x, sensor id: 0x%x vendor_id 0x%x\n",imgsensor.i2c_write_id,*sensor_id,vendor_id);
                      *sensor_id = 0xFFFFFFFF;
                      return ERROR_SENSOR_CONNECT_FAIL;
                    }
                }
