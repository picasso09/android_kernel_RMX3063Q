...
#include "gc5035_ofg_mipi_raw.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "gc5035_ofg_camera_sensor"
#define LOG_1 LOG_INF("GC5035_OFG_MIPI, 2LANE\n")
/****************************   Modify end    *******************************************/
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC5035_OFG_SENSOR_ID,    /*record sensor id defined in Kd_imgsensor.h*/
    ...
    .normal_video = {
        ...
        .grabwindow_height = 1944,
        .mipi_data_lp2hs_settle_dc = 85,
        .mipi_pixel_rate = 350400000,//175200000
        .max_framerate = 300,
    },
    .hs_video = {
        .pclk = 175200000,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
    ...
    { 2592, 1944,    0,    0, 2592, 1944, 2592, 1944,    0,    0, 2592, 1944,    0,    0, 2592, 1944 }  /*sl video*/
};

static struct gc5035_ofg_otp gc5035_ofg_otp_data = { 0 };
...
static kal_uint8 gc5035_ofg_otp_read_byte(kal_uint16 addr)
{
...
static void gc5035_ofg_otp_read_sensor_info(void)
{
    kal_uint8 flag_dd = 0;
    ...
    /* DPC */
    flag_dd = gc5035_ofg_otp_read_byte(DPC_FLAG_OFFSET);
    LOG_INF("flag_dd = 0x%x\n", flag_dd);
    switch (flag_dd & 0x03) {
    case 0x00:
        LOG_INF("DD is empty !!\n");
        gc5035_ofg_otp_data.dd_flag = 0x00;
        break;
    case 0x01:
        LOG_INF("DD is valid!!\n");
        gc5035_ofg_otp_data.dd_num = gc5035_ofg_otp_read_byte(DPC_TOTAL_NUMBER_OFFSET)
                                + gc5035_ofg_otp_read_byte(DPC_ERROR_NUMBER_OFFSET);
        gc5035_ofg_otp_data.dd_flag = 0x01;
        LOG_INF("total_num = %d\n", gc5035_ofg_otp_data.dd_num);
        break;
    case 0x02:
    case 0x03:
        LOG_INF("DD is invalid !!\n");
        gc5035_ofg_otp_data.dd_flag = 0x02;
        break;
    default:
        break;
    }
    ...
    /*Register Update*/
    gc5035_ofg_otp_data.reg_flag = gc5035_ofg_otp_read_byte(REG_INFO_FLAG_OFFSET);
    LOG_INF("Register value flag = 0x%x\n", gc5035_ofg_otp_data.reg_flag);
    if ((gc5035_ofg_otp_data.reg_flag & 0x03) == 0x01)
        for (i = 0; i < REG_INFO_SIZE; i++) {
            temp = gc5035_ofg_otp_read_byte(REG_INFO_PAGE_OFFSET + REG_INFO_SIZE * 8 * i);
            for (j = 0; j < 2; j++)//每组2个寄存器
                if (((temp >> (4 * j + 3)) & 0x01) == 0x01) {// check
                    gc5035_ofg_otp_data.regs[gc5035_ofg_otp_data.reg_num][0] = (temp >> (4 * j)) & 0x07;// page
                    gc5035_ofg_otp_data.regs[gc5035_ofg_otp_data.reg_num][1] =
                        gc5035_ofg_otp_read_byte(REG_INFO_ADDR_OFFSET + REG_INFO_SIZE * 8 * i + 0x10 * j);
                    gc5035_ofg_otp_data.regs[gc5035_ofg_otp_data.reg_num][2] =
                        gc5035_ofg_otp_read_byte(REG_INFO_VALUE_OFFSET + REG_INFO_SIZE * 8 * i + 0x10 * j);
                    gc5035_ofg_otp_data.reg_num++;
                }
        }
}

static void gc5035_ofg_otp_update_dd(void)
{
    kal_uint8 state = 0;
    kal_uint8 n = 0;

    if (0x01 == gc5035_ofg_otp_data.dd_flag) {
        LOG_INF("DD auto load start!\n");
        write_cmos_sensor(0xfe, 0x02);
        write_cmos_sensor(0xbe, 0x00);//允许DD值写入
        write_cmos_sensor(0xa9, 0x01);// 清sram
        write_cmos_sensor(0x09, 0x33);
        write_cmos_sensor(0x01, (gc5035_ofg_otp_data.dd_num >> 8) & 0x07);
        write_cmos_sensor(0x02, gc5035_ofg_otp_data.dd_num & 0xff);
        write_cmos_sensor(0x03, 0x00);
        write_cmos_sensor(0x04, 0x80);//otp起始坏点的坐标
        ...
        write_cmos_sensor(0xf3, 0x80);// auto load开始脉冲
        while (n < 3) {
            state = read_cmos_sensor(0x06);//读取dd自动load是否using
            if ((state | 0xfe) == 0xff)
                mdelay(10);
            else
                n = 3;
        ...
        write_cmos_sensor(0xbe, 0x01);//dd sram值可通过内部读取生效
        write_cmos_sensor(0x09, 0x00);//关闭自动load
...
static void gc5035_ofg_otp_update_reg(void)
{
    kal_uint8 i = 0;
    ...
    if ((gc5035_ofg_otp_data.reg_flag & 0x03) == 0x01)
        for (i = 0; i < gc5035_ofg_otp_data.reg_num; i++) {
            write_cmos_sensor(0xfe, gc5035_ofg_otp_data.regs[i][0]);
            write_cmos_sensor(gc5035_ofg_otp_data.regs[i][1], gc5035_ofg_otp_data.regs[i][2]);
        }
...
#if defined(GC5035_OFG_OTP_DEBUG)
    //char pu_send_cmd[1] = { (char)(0xc0 + (addr & 0xFF)) };

    //iReadRegI2C(pu_send_cmd, 1, (u8 *)data, length, imgsensor.i2c_write_id);
static void gc5035_ofg_otp_debug(void)
{
    kal_uint8 debugdata[OTP_DATA_LENGTH] = {0};
    kal_uint16 i;

    memset(&debugdata, 0, OTP_DATA_LENGTH * sizeof(kal_uint8));
    gc5035_ofg_otp_read_group(OTP_START_ADDR, &debugdata[0], OTP_DATA_LENGTH);
    for(i = 0; i < OTP_DATA_LENGTH; i++)
        LOG_INF("addr = 0x%x, data = 0x%x\n", OTP_START_ADDR + i * 8, debugdata[i]);
}
#endif
static void gc5035_ofg_otp_read(void)
{
    gc5035_ofg_otp_read_sensor_info();

#if defined(GC5035_OFG_OTP_DEBUG)
    gc5035_ofg_otp_debug();
#endif
}

static void gc5035_ofg_otp_update(void)
{
    gc5035_ofg_otp_update_dd();

    gc5035_ofg_otp_update_reg();
}

static void gc5035_ofg_otp_function(void)
{
    memset(&gc5035_ofg_otp_data, 0, sizeof(gc5035_ofg_otp_data));
    write_cmos_sensor(0xfa, 0x10); //otp_clk_en
    write_cmos_sensor(0xf5, 0xe9);
    write_cmos_sensor(0xfe, 0x02);
    write_cmos_sensor(0x67, 0xc0); //[7]otp_en [6]otpcpclk_en
    write_cmos_sensor(0x59, 0x3f);//charge pump 电压为7.43V
    write_cmos_sensor(0x55, 0x80);//[7]高压保护
    write_cmos_sensor(0x65, 0x80);
    write_cmos_sensor(0x66, 0x03);
    write_cmos_sensor(0xfe, 0x00);

    gc5035_ofg_otp_read();
    gc5035_ofg_otp_update();
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    kal_uint16 GC5035_OFG_AGC_Param[MAX_GAIN_INDEX][2] = {
        {  256,  0 },
        {  302,  1 },
        {  358,  2 },
        {  425,  3 },
    ...
    for (gain_index = E_GAIN_INDEX - 1; gain_index >= 0; gain_index--)
        if (reg_gain >= GC5035_OFG_AGC_Param[gain_index][0])
            break;

    write_cmos_sensor(0xfe, 0x00);
    write_cmos_sensor(0xb6, GC5035_OFG_AGC_Param[gain_index][1]);
    temp_gain = reg_gain * Dgain_ratio / GC5035_OFG_AGC_Param[gain_index][0];
    write_cmos_sensor(0xb1, (temp_gain >> 8) & 0x0f);
    write_cmos_sensor(0xb2, temp_gain & 0xfc);
    LOG_INF("Exit! GC5035_OFG_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, reg_gain = %d\n",
        GC5035_OFG_AGC_Param[gain_index][1], temp_gain,reg_gain);
...
static void sensor_init(void)
{
    ...
    write_cmos_sensor(0x17, GC5035_OFG_MIRROR);
    ...
    write_cmos_sensor(0x54, GC5035_OFG_RSTDUMMY1);
    ...
    write_cmos_sensor(0x22, GC5035_OFG_RSTDUMMY2);
    write_cmos_sensor(0xfe, 0x00);
...
extern unsigned int gc5035_ofg_read_region(struct i2c_client *client, unsigned int addr,unsigned char *data,unsigned int size);
        ...
        do {
            *sensor_id = return_sensor_id() +1;
            if (*sensor_id == imgsensor_info.sensor_id) {
                if(gc5035_ofg_read_region(NULL,0,&vendor_id,1)<0){
                    pr_err("gc5035_ofg_read_region error read otp");
                    *sensor_id = 0xFFFFFFFF;
                    return ERROR_SENSOR_CONNECT_FAIL;
                }else{
                    if(0x7 == vendor_id){
                        LOG_INF("gc50353 i2c write id: 0x%x, sensor id: 0x%x vendor_id 0x%x\n",imgsensor.i2c_write_id,*sensor_id,vendor_id);
                        return ERROR_NONE;
                    }else{
                       pr_err("gc50353 Read sensor id fail i2c write id: 0x%x, sensor id: 0x%x vendor_id 0x%x\n",imgsensor.i2c_write_id,*sensor_id,vendor_id);
                       *sensor_id = 0xFFFFFFFF;
                       return ERROR_SENSOR_CONNECT_FAIL;
                    }
                }
            }
            LOG_INF("gc50353 Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
            retry--;
        } while (retry > 0);
        i++;
        retry = 2;
...
UINT32 GC5035_OFG_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
