...
#include "s5k3h7yx_ts_mipiraw_Sensor.h"

#define PFX "s5k3h7yx_ts_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = S5K3H7YX_TS_SENSOR_ID,
...
static void sensor_init(void)
{
    ...
    write_cmos_sensor(0x0B00, 0x0109);//lsc enable lux ratio 0x1c   0x0109
    #endif
}    /*    sensor_init  */
...
extern unsigned int s5k3h7yx_ts_read_region(struct i2c_client *client, unsigned int addr,unsigned char *data,unsigned int size);
        ...
        do {
            *sensor_id = return_sensor_id() + 2;
            if (*sensor_id == imgsensor_info.sensor_id) {
                write_cmos_sensor(0x0100,0x0103);
                if( s5k3h7yx_ts_read_region(NULL,0,&vendor_id,1) < 0){
                    pr_err("s5k3h7yx_ts_read_region otp failed");
                    *sensor_id = 0xFFFFFFFF;
                    return ERROR_SENSOR_CONNECT_FAIL;
                }else{
                    if(0x56 == vendor_id){
                        LOG_INF("s5k3h7yx3 i2c write id: 0x%x, sensor id: 0x%x vendor_id 0x%x\n",imgsensor.i2c_write_id,*sensor_id,vendor_id);
                        return ERROR_NONE;
                    }else{
                        pr_err("s5k3h7yx3 Read sensor id fail i2c write id: 0x%x, sensor id: 0x%x vendor_id 0x%x\n",imgsensor.i2c_write_id,*sensor_id,vendor_id);
                        *sensor_id = 0xFFFFFFFF;
                        return ERROR_SENSOR_CONNECT_FAIL;
                    }
                }
            }
            LOG_INF("s5k3h7yx3 Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
UINT32 S5K3H7YX_TS_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
