...
extern unsigned int gc5035_read_region(struct i2c_client *client, unsigned int addr,unsigned char *data,unsigned int size);
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                if(gc5035_read_region(NULL,4,&vendor_id,1)<0){
                    pr_err("gc5035_read_region error read otp");
                    *sensor_id = 0xFFFFFFFF;
                    return ERROR_SENSOR_CONNECT_FAIL;
                }else{
                    if(0x8 == vendor_id){
                        LOG_INF("gc50352 i2c write id: 0x%x, sensor id: 0x%x vendor_id 0x%x\n",imgsensor.i2c_write_id,*sensor_id,vendor_id);
                        return ERROR_NONE;
                    }else{
                      pr_err("gc50352 Read sensor id fail i2c write id: 0x%x, sensor id: 0x%x vendor_id 0x%x\n",imgsensor.i2c_write_id,*sensor_id,vendor_id);
                      *sensor_id = 0xFFFFFFFF;
                      return ERROR_SENSOR_CONNECT_FAIL;
                    }
                }
            }
            LOG_INF("gc50352 Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
            retry--;
        } while (retry > 0);
        i++;
        retry = 2;
