...
#ifdef VENDOR_EDIT
/*Huimin.Tang@Camera.DRV add for register device info 20200914*/
#define DEVICE_VERSION_GC8034    "gc8034"
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{ 3264, 2448,   0,   0, 3264, 2448, 3264, 1840, 0, 0, 3264, 1840, 0, 0, 3264, 1840}, /* video */
	{ 3264, 2448,   0,   0, 3264, 2448, 1280, 720, 0, 0, 1280, 720, 0, 0, 1280, 720}, /* hs video */
	{ 3264, 2448,   1312, 984, 640, 480, 640, 480, 0, 0, 640, 480, 0, 0, 640, 480} /* slim video */
};
static kal_uint16 read_eeprom_data(kal_uint32 addr)
{
	...
	iReadRegI2C(pu_send_cmd, 2, (u8 *)&get_byte, 1, 0xa1);
	return get_byte;
}
...
static void gc8034_gcore_read_otp_info(void)
{
	...
	gc8034_otp_info.ob_flag = read_eeprom_data(0x0789);
	LOG_INF("--travis-- ob_flag=0x%x\n", gc8034_otp_info.ob_flag);
	if(gc8034_otp_info.ob_flag == 1){
		for (i = 0; i < 6; i++){
			ob[i] = read_eeprom_data(0x078a + i);
			obchecksum += ob[i];
			LOG_INF("--travis-- ob[%d]=0x%x\n", i, ob[i]);
		}
		ob[6] = read_eeprom_data(0x0790);
		LOG_INF("--travis-- obchecksum=0x%x; ob[6]=0x%x\n", obchecksum, ob[6]);
		if ((obchecksum % 255 + 1) == ob[6]){
			gc8034_otp_info.ob_g_ratio_h = ob[0];
			gc8034_otp_info.ob_g_ratio_l = ob[1];
			gc8034_otp_info.ob_r_ratio_h = ob[2];
			gc8034_otp_info.ob_r_ratio_l = ob[3];
			gc8034_otp_info.ob_b_ratio_h = ob[4];
			gc8034_otp_info.ob_b_ratio_l = ob[5];
		}else{
			gc8034_otp_info.ob_flag = 0;
		}
	}
#if GC8034OTP_DEBUG
	for (i = 0; i < 10; i++) {
...
static void set_dummy(void)
{
	...
	if (SENSORFPS == 1) {
		LOG_INF("the max fps is 90 ");
		basic_line = 736;
	}
	if (SENSORFPS == 2) {
		LOG_INF("the max fps is 120 ");
		basic_line = 516;
	}
	LOG_INF("dummyline = %d,basic_line = %d, dummypixels = %d\n", imgsensor.dummy_line, basic_line, imgsensor.dummy_pixel);
...
static void gain2reg(kal_uint16 gain)
{
	...
	if(gc8034_otp_info.ob_flag == 0x01){
		LOG_INF("travis gain=0x%x   gain_level=0x%x\n", gain, gain_level[gain_index]);
		if(gain >= gain_level[5]){
			write_cmos_sensor(0xfd, 0x0a);
			write_cmos_sensor(0x58, gc8034_otp_info.ob_g_ratio_h);
			write_cmos_sensor(0x59, gc8034_otp_info.ob_r_ratio_h);
			write_cmos_sensor(0x5a, gc8034_otp_info.ob_b_ratio_h);
			write_cmos_sensor(0x5b, gc8034_otp_info.ob_g_ratio_h);
			write_cmos_sensor(0x5c, gc8034_otp_info.ob_g_ratio_l);
			write_cmos_sensor(0x5d, gc8034_otp_info.ob_r_ratio_l);
			write_cmos_sensor(0x5e, gc8034_otp_info.ob_b_ratio_l);
			write_cmos_sensor(0x5f, gc8034_otp_info.ob_g_ratio_l);
		...
		for(i=0; i<8; i++){
			LOG_INF("travis 0x%x=0x%x\n", 0x58+i, read_cmos_sensor(0x58+i));
		}
		}
}
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0xdb, 0x80);//0xb0 travis
	write_cmos_sensor(0xdc, 0x0f);//0x0e travis
	...
	write_cmos_sensor(0x5b, 0x38);//0x08 travis
	write_cmos_sensor(0x5c, 0x20);
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0x1a, 0x09);
	write_cmos_sensor(0x1c, 0x04);//add travis
...
static void normal_video_setting(void)
{
	...
	write_cmos_sensor(0x92, 0x30+GC8034_FULL_STARTY);
	write_cmos_sensor(0x93, 0x00);
	write_cmos_sensor(0x94, GC8034_FULL_STARTX);
...
static void hs_video_setting(void)
{
    ...
    write_cmos_sensor(0xfe, 0x00);
    write_cmos_sensor(0x3f, 0x00);
    write_cmos_sensor(0x08, 0x4c);
    write_cmos_sensor(0x09, 0x03);
    write_cmos_sensor(0x0a, 0x9a);
    write_cmos_sensor(0x0d, 0x02);
    write_cmos_sensor(0x0e, 0xe0);
    write_cmos_sensor(0x17, GC8034_MIRROR);
    write_cmos_sensor(0xfe, 0x10);
    write_cmos_sensor(0xfe, 0x00);
    write_cmos_sensor(0xfe, 0x10);
...
static void slim_video_setting(void)
{
	...
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0x3f, 0x00);
	write_cmos_sensor(0x08, 0x68);
	write_cmos_sensor(0x09, 0x04);
	write_cmos_sensor(0x0a, 0x12);
	...
	write_cmos_sensor(0x91, 0x00);
	write_cmos_sensor(0x92, GC8034_FULL_STARTY);
	write_cmos_sensor(0x93, 0x05);   //stephen
	write_cmos_sensor(0x94, 0x20+GC8034_FULL_STARTX);  //stephen
	write_cmos_sensor(0x95, 0x01);    //stephen
	write_cmos_sensor(0x96, 0xe0);    //stephen
	write_cmos_sensor(0x97, 0x02);    //stephen
	write_cmos_sensor(0x98, 0x80);    //stephen
	write_cmos_sensor(0xfe, 0x01);
	write_cmos_sensor(0xd3, 0x18);   //stephen
	write_cmos_sensor(0xd4, 0x10);   //stephen
	write_cmos_sensor(0xd7, 0x18);   //stephen
	write_cmos_sensor(0xdc, 0x03);  //stephen
	write_cmos_sensor(0xdd, 0xd8);   //stephen
	write_cmos_sensor(0xfe, 0x03);
	write_cmos_sensor(0x02, 0x03);
	write_cmos_sensor(0x04, 0x80);
	write_cmos_sensor(0x11, 0x2b);
	write_cmos_sensor(0x12, 0x20);  //stephen
	write_cmos_sensor(0x13, 0x03);  //stephen
	write_cmos_sensor(0x15, 0x10);
	write_cmos_sensor(0x16, 0x29);
	write_cmos_sensor(0x17,0xff);
	write_cmos_sensor(0x19,0xaa);
			...
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if (deviceInfo_register_value == 0x00) {
					register_imgsensor_deviceinfo("Cam_r", DEVICE_VERSION_GC8034, imgsensor_info.module_id);
