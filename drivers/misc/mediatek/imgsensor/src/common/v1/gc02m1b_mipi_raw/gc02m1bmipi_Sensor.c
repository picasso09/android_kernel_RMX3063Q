...
#define DEVICE_VERSION_GC02M1B    "gc02m1b_cxt"
...
static int is_1st_flag(void)
{
    //read otp
	sensor_init();
	write_cmos_sensor(0xf3, 0x30); //initial setting of otp reading
	write_cmos_sensor(0xfe, 0x02); //otp page
	write_cmos_sensor(0x17, 0x80);
	write_cmos_sensor(0xf3, 0x34); // enable read otp
	if (read_cmos_sensor(0x19) == 0x5D){
		return 0;
	} else {
		write_cmos_sensor(0xfe, 0x02); //otp page
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if (is_1st_flag() == 0){
		*sensor_id = 0xFFFFFFFF;
		printk("please check the sensor is right! \n");
			...
			*sensor_id = return_sensor_id();
			...
			if (*sensor_id == GC02M1B_SENSOR_ID) {
				*sensor_id = imgsensor_info.sensor_id;
				/*Pengfei.Zhao@Camera.DRV add for register device info 20201028*/
				imgsensor_info.module_id = 0x54;
				if (deviceInfo_register_value == 0x00) {
					register_imgsensor_deviceinfo("Cam_r3", DEVICE_VERSION_GC02M1B, imgsensor_info.module_id);
