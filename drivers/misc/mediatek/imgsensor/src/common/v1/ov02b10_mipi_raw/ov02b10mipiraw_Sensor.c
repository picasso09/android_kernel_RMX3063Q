...
#define MACRO_MODULE_FLAG_OFFSET1  0X12
#define MACRO_MODULE_FLAG_OFFSET2  0X18
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV02B10_MIPI_SENSOR_ID,
...
static kal_uint16 is_mono_flag(void)
{
    //read otp
    write_cmos_sensor(0xfd, 0x06); //otp page
    write_cmos_sensor(0x21, 0x00);
    write_cmos_sensor(0x2f, 0x01); // enable read otp

    if (read_cmos_sensor(MACRO_MODULE_FLAG_OFFSET1) == 0x31 || read_cmos_sensor(MACRO_MODULE_FLAG_OFFSET2) == 0x31)
        return 1;
    return 0;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    if (is_mono_flag() == 1){
        *sensor_id = 0xFFFFFFFF;
        printk("please check otp or ov02b10 position! \n");
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = (return_sensor_id() + 4);
            if (*sensor_id == OV02B10_MIPI_SENSOR_ID) {
                    /*Pengfei.Zhao@Camera.DRV add for register device info 20201028*/
                    imgsensor_info.module_id = 0x08;
                    if (deviceInfo_register_value == 0x00) {
                    	register_imgsensor_deviceinfo("Cam_r2", DEVICE_VERSION_OV02B10, imgsensor_info.module_id);
