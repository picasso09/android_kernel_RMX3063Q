...
#include "gc02m1_sw_mipi_Sensor.h"
/************************** Modify Following Strings for Debug **************************/
#define PFX "gc02m1_sw_camera_sensor"
#define LOG_1 LOG_INF("GC02M1_SW, MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
...
#define DEVICE_VERSION_GC02M1_SW    "gc02m1_sw"
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	LOG_INF("GC02M1_SW_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, reg_gain = %d\n",
			...
			*sensor_id = (return_sensor_id());
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if (deviceInfo_register_value == 0x00) {
					register_imgsensor_deviceinfo("Cam_r2", DEVICE_VERSION_GC02M1_SW, imgsensor_info.module_id);
					deviceInfo_register_value = 0x01;
				}
				printk("gc02m1_sw i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			printk("gc02m1_sw Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
...
UINT32 GC02M1_SW_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
