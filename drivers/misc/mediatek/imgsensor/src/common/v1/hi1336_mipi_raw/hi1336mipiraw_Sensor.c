...
#define DEVICE_VERSION_HI1336    "hi1336"
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.custom1 = { //24fps for dual camera
		.pclk = 600000000,                                //record different mode's pclk
		.linelength  = 8600,                                //record different mode's linelength
		.framelength = 2902,                        //record different mode's framelength
		.startx = 0,                                        //record different mode's startx of grabwindow
		.starty = 0,                                        //record different mode's starty of grabwindow
		...
		/*         following for GetDefaultFramerateByScenario()        */
		.max_framerate = 240,
		.mipi_pixel_rate = 321600000,//(804M * 4 / 10)
	},
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	...
	{ 4224, 3136, 832, 848, 2560, 1440,  1280,  720 , 0,  0, 1280,  720, 0, 0, 1280,  720},		// hight speed video (1280 x 720)
	{ 4224, 3136,   0, 482, 4224, 2172,  1408,  724,  64, 2, 1280,  720, 0, 0, 1280,  720},		// slim video (1280 x 720)
	{ 4224, 3136,   0, 342, 4224, 2452,  4224, 2452, 480, 2, 3264, 2448, 0, 0, 3264, 2448},		// VIDEO (4208 x 3120)
};
...
static struct SENSOR_VC_INFO_STRUCT SENSOR_VC_INFO[4]=
{
	...
	/* custom1 mode setting */
	 {0x02, //VC_Num
	  ...
	  0x00, 0x2B, 0x1070, 0x0C30,	// VC0 Maybe image data?
	  0x00, 0x00, 0x0000, 0x0000,	// VC1 MVHDR
	  0x01, 0x30, 0x0104, 0x0260,   // VC2 PDAF
	  0x00, 0x00, 0x0000, 0x0000},	// VC3 ??
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info_3264_2448 =
{
	...
	.i4SubBlkW =16,
	.i4SubBlkH =8,
	.i4PosL = {{37,12},{53,12},{45,16},{61,16},{37,28},{53,28},{45,32},{61,32}},
	.i4PosR = {{37,8},{53,8},{45,20},{61,20},{37,24},{53,24},{45,36},{61,36}},
	.i4BlockNumX = 100,
	.i4BlockNumY = 76,
...
#if MULTI_WRITE
#define I2C_BUFFER_LEN_BUR 1020
#define I2C_BUFFER_LEN 1020
static kal_uint16 hi1336_table_write_init_cmos_sensor(
					kal_uint16 *para, kal_uint32 len)
{
	char puSendCmd[I2C_BUFFER_LEN_BUR];
	...
	while (len > IDX) {
		...
		//if ((I2C_BUFFER_LEN_BUR - tosend) < 2 ||
		if(	len <= IDX ){
		//	addr != addr_last) {
			LOG_INF("table_write_init_cmos_sensor:%d\n", tosend );
			iBurstWriteReg(puSendCmd, tosend,
...
static void set_shutter_frame_length(kal_uint16 shutter, kal_uint16 frame_length)
{
	...
	write_cmos_sensor_8(0x020D, (shutter & 0xFF0000) >> 16 );
	write_cmos_sensor(0x020A, shutter);
	write_cmos_sensor(0x1002,0xc219);
		frame_cnt = ((read_cmos_sensor(0x1056) << 8) | read_cmos_sensor(1057));
				LOG_INF("dual cam frame_cnt is (%d)!\n",frame_cnt);
...
#if MULTI_WRITE
kal_uint16 addr_data_pari_init_hi1336_tg1[] = {
	0x2000, // address
	0x0021,
	0x04a5,
	0xb124,
...
kal_uint16 addr_data_pari_init_hi1336_tg2[] = {
	0x235c, // address
	0x434d,
	0x411f,
	0x0004,
...
kal_uint16 addr_data_pari_init_hi1336_init[] = {
	0x2ffe, 0xc114,
	0x3224, 0xf222,
	0x322a, 0xf23c,
	0x3230, 0xf03a,
	...
	0x1060, 0x3008,
	0x1062, 0x0000,
	0x0202, 0x0200,       //   0x0200 Gr ==>  0x0100 Gb
	0x0b10, 0x400c,
	0x0212, 0x0000,
	0x035e, 0x0701,
	0x040a, 0x0000,
...
kal_uint16 addr_data_pair_init_hi1336[] = {
	...
	0x06b4, 0x3ff0,
	0x0618, 0x0a80,
	0x0668, 0x4303, //change mipi clock for pass1 dequeue fail issue
	0x06ca, 0x02cc,
	0x066e, 0x0050,
	0x0670, 0x0050,
	0x113c, 0x0001,
...
{
 LOG_INF("test4 sensor_init\n");
#if MULTI_WRITE
	pr_err("burst_hi1336_tg1_start\n");

	hi1336_table_write_init_cmos_sensor(
		addr_data_pari_init_hi1336_tg1,
		sizeof(addr_data_pari_init_hi1336_tg1) /
		sizeof(kal_uint16));
	LOG_INF("burst_hi1336_tg1_end\n");

	hi1336_table_write_init_cmos_sensor(
		addr_data_pari_init_hi1336_tg2,
		sizeof(addr_data_pari_init_hi1336_tg2) /
		sizeof(kal_uint16));

	pr_err("burst_hi1336_tg2_end\n");

	hi1336_table_write_cmos_sensor(
		addr_data_pari_init_hi1336_init,
		sizeof(addr_data_pari_init_hi1336_init) /
		sizeof(kal_uint16));

		pr_err("burst_intial_end\n");
#else
	...
	write_cmos_sensor(0x0202, 0x0200); //image orient
...
//Sensor				  : Hi-1336
//Date		  			: 2020-10-23
//Customer        : Wingtech T99651
//Image size	  	: 4208x3120
//MCLK		  			: 24MHz
//MIPI speed(Mbps): 1428Mbps x 4Lane
//Max Fps 	  		: 30.01fps
//Pixel order 	  : GR
//PDAF Type       : PDAF 2 VC / PD-DPC on, Dyn-DPC on
/////////////////////////////////////////
	0x3250, 0xa060,
	0x0730, 0x770f,
	0x0732, 0xe0b0,
...
kal_uint16 addr_data_pair_hs_video_hi1336[] = {
	...
	//Sensor				  : Hi-1336
	//Date		  			: 2020-10-30
	//Customer        : OPPO AL3251(RIO)
	//Image size	  	: 1280x720
	//MCLK		  			: 24MHz
	//MIPI speed(Mbps): 476Mbps x 4Lane
	//Max Fps 	  		: 90.02fps
	//Pixel order 	  : GR
	//PDAF Type       : disable / PD-DPC on, Dyn-DPC on
	/////////////////////////////////////////
	0x3250, 0xa470,
	0x0730, 0x770f, //pll_cfg_mipi_a PLL_CLK=750mhz b7-6:00_isp_div2(1/1)
	0x0732, 0xe1b0,
...
#if MULTI_WRITE
//change mipi clock for pass1 dequeue fail issue
...
static void custom1_setting(void)
{
    ...
    hi1336_table_write_cmos_sensor(addr_data_pair_custom1_hi1336,
          sizeof(addr_data_pair_custom1_hi1336) / sizeof(kal_uint16));
}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				/*Pengfei.Zhao@Camera.DRV add for register device info 20201028*/
				imgsensor_info.module_id = 0x58;
				if (deviceInfo_register_value == 0x00) {
					register_imgsensor_deviceinfo("Cam_r", DEVICE_VERSION_HI1336, imgsensor_info.module_id);
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	/*Wujun@RM.Camera add for dualcam can't open 20200709*/
