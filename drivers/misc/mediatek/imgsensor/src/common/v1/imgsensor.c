...
#ifdef ODM_HQ_EDIT
/*Pengfei.Zhao@ODM 20200902 add for RIO bringup*/
...
#define DEVICE_MANUFACUTRE_TXD          "Txd"

#ifdef ODM_HQ_EDIT
/*Huimin.Tang@Camera.DRV add for register device info 20200914*/
#define DEVICE_MANUFACUTRE_HOLITECH     "Holitech"
#define DEVICE_MANUFACUTRE_CXT          "Cxt"
#define DEVICE_MANUFACUTRE_KINGCOME     "KingCome"
#define DEVICE_MANUFACUTRE_SW           "SunWin"
#endif
...
#define IMGSENSOR_MODULE_ID_HOLITECH_RIO    0x55
#define IMGSENSOR_MODULE_ID_LCE    0x57
#define IMGSENSOR_MODULE_ID_TXD    0x58
#define IMGSENSOR_MODULE_ID_CXT      0x54
#define IMGSENSOR_MODULE_ID_KINGCOME      0x08
#define IMGSENSOR_MODULE_ID_SW      0x5D
#endif
void register_imgsensor_deviceinfo(char *name, char *version, u8 module_id)
{
    char *manufacture;
	pr_err(" name = %s, version = %s , id = %d " , name, version, module_id);
    ...
    switch (module_id)
    {
        ...
        case IMGSENSOR_MODULE_ID_SHINE:  /* Shine */
            ...
            break;
        case IMGSENSOR_MODULE_ID_HOLITECH_RIO:  /* Holitech */
        ...
        case IMGSENSOR_MODULE_ID_TXD:  /* Txd */
            ...
            break;
        case IMGSENSOR_MODULE_ID_CXT:  /* Cxt */
            manufacture = DEVICE_MANUFACUTRE_CXT;
            break;
        case IMGSENSOR_MODULE_ID_KINGCOME:  /* KingCome*/
            manufacture = DEVICE_MANUFACUTRE_KINGCOME;
            break;
        case IMGSENSOR_MODULE_ID_SW:  /* SW */
            manufacture = DEVICE_MANUFACUTRE_SW;
            break;
        default:
...
#define STRINGIZE(stringizedName) TOSTRING(stringizedName)
	...
	int i = 0;
	int j = 0;
	int Operator_Rio = 0;//Pengfei.Zhao@ODM 20200922 add for RIO A&B&C compatible
	char *driver_name = NULL;

	imgsensor_mutex_init(psensor_inst);
	#ifdef ODM_HQ_EDIT
	/*Pengfei.Zhao@ODM 20200902 add for RIO bringup for RIO A&B&C compatible*/
	Operator_Rio = get_Operator_Version();
	if (Operator_Rio >= 151 && Operator_Rio <= 164) {
		//RIO&&RIO-A
		imgsensor_i2c_init(&psensor_inst->i2c_cfg,
			imgsensor_custom_config_20701_A[psensor->inst.sensor_idx].i2c_dev);
	} else if(Operator_Rio >= 165 && Operator_Rio <= 184) {
		//RIO-B&&RIO-C&&RIO-D
		imgsensor_i2c_init(&psensor_inst->i2c_cfg,
			imgsensor_custom_config_20701_BC[psensor->inst.sensor_idx].i2c_dev);
	} else {
		imgsensor_i2c_init(&psensor_inst->i2c_cfg,
