...
enum IMGSENSOR_RETURN imgsensor_hw_init(struct IMGSENSOR_HW *phw)
{
	...
	for (i = 0; i < IMGSENSOR_SENSOR_IDX_MAX_NUM; i++) {
		...
		if (Operator_Rio >= 151 && Operator_Rio <= 164) {
			//RIO&&RIO-A
			pcust_pwr_cfg = imgsensor_custom_config_20701_A;
		} else if(Operator_Rio >= 165 && Operator_Rio <= 184){
			//RIO-B&&RIO-C&&RIO-D
			pcust_pwr_cfg = imgsensor_custom_config_20701_BC;
		} else {
		...
		#endif
		#ifdef ODM_HQ_EDIT
		/*Pengfei.Zhao@ODM 20200915 add for RIO bringup  main/sub */
		while ((pcust_pwr_cfg->sensor_idx != IMGSENSOR_SENSOR_IDX_NONE)&&(pcust_pwr_cfg->sensor_idx != i))
			pcust_pwr_cfg++;
	...
	enum IMGSENSOR_HW_POWER_STATUS pwr_status)
{
	...
	/*Pengfei.Zhao@ODM 20200902 add for RIO bringup for RIO A&B&C compatible*/
	Operator_Rio = get_Operator_Version();
	pr_info("Operator_Rio = %d \n",Operator_Rio);
	if (Operator_Rio >= 151 && Operator_Rio <= 164) {
		//RIO&&RIO-A
		imgsensor_hw_power_sequence(
				phw,
				sensor_idx,
				pwr_status, sensor_power_sequence_20701_A, curr_sensor_name);
	} else if(Operator_Rio >= 165 && Operator_Rio <= 184){
		//RIO-B&&RIO-C&&RIO-D
		imgsensor_hw_power_sequence(
				phw,
				sensor_idx,
				pwr_status, sensor_power_sequence_20701_BC, curr_sensor_name);
	} else {
		imgsensor_hw_power_sequence(
				phw,
				sensor_idx,
