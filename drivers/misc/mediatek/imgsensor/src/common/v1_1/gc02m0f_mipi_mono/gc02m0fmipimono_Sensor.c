...
static kal_uint8 gGc02m0f_CamInfo_CamInfo[CAMERA_MODULE_INFO_LENGTH];
...
static void read_eeprom_CamInfo(void)
{
	...
	gGc02m0f_CamInfo_CamInfo[0] = get_byte[0];
	gGc02m0f_CamInfo_CamInfo[1] = get_byte[1];
	gGc02m0f_CamInfo_CamInfo[2] = get_byte[6];
	gGc02m0f_CamInfo_CamInfo[3] = get_byte[7];
	gGc02m0f_CamInfo_CamInfo[4] = get_byte[8];
	gGc02m0f_CamInfo_CamInfo[5] = get_byte[9];
	gGc02m0f_CamInfo_CamInfo[6] = get_byte[10];
	gGc02m0f_CamInfo_CamInfo[7] = get_byte[11];
}

static void read_eeprom_SN(void)
{
	kal_uint16 idx = 0;
	kal_uint8 *get_byte= &gGc02m0f_SN[0];
	for (idx = 0; idx <CAMERA_MODULE_SN_LENGTH; idx++) {
		char pusendcmd[2] = {0x00 , (char)((0x40 + idx) & 0xFF) };
		iReadRegI2C(pusendcmd , 2, (u8*)&get_byte[idx],1, 0xA8);
		LOG_INF("s5kgw1_SN[%d]: 0x%x  0x%x\n", idx, get_byte[idx], gGc02m0f_SN[idx]);
	}
}
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_GET_MODULE_INFO:
            ...
            *(feature_data_32 + 1) = (gGc02m0f_CamInfo_CamInfo[1] << 24)
                        | (gGc02m0f_CamInfo_CamInfo[0] << 16)
                        | (gGc02m0f_CamInfo_CamInfo[3] << 8)
                        | (gGc02m0f_CamInfo_CamInfo[2] & 0xFF);
            *(feature_data_32 + 2) = (gGc02m0f_CamInfo_CamInfo[5] << 24)
                        | (gGc02m0f_CamInfo_CamInfo[4] << 16)
                        | (gGc02m0f_CamInfo_CamInfo[7] << 8)
                        | (gGc02m0f_CamInfo_CamInfo[6] & 0xFF);
            break;
