...
static void read_eeprom_CamInfo(void)
{
	...
	for (idx = 0; idx <12; idx++) {
		...
		LOG_INF("s5k3m5_info[%d]: 0x%x\n", idx, get_byte[idx]);
	}

	gS5k3m5_CamInfo[0] = get_byte[0];
	gS5k3m5_CamInfo[1] = get_byte[1];
	gS5k3m5_CamInfo[2] = get_byte[6];
	gS5k3m5_CamInfo[3] = get_byte[7];
	gS5k3m5_CamInfo[4] = get_byte[8];
	gS5k3m5_CamInfo[5] = get_byte[9];
	gS5k3m5_CamInfo[6] = get_byte[10];
	gS5k3m5_CamInfo[7] = get_byte[11];
}

static void read_eeprom_SN(void)
{
	kal_uint16 idx = 0;
	kal_uint8 *get_byte= &gS5k3m5_SN[0];
	for (idx = 0; idx <CAMERA_MODULE_SN_LENGTH; idx++) {
		char pusendcmd[2] = {0x00 , (char)((0xB0 + idx) & 0xFF) };
		iReadRegI2C(pusendcmd , 2, (u8*)&get_byte[idx],1, 0xA0);
		LOG_INF("gS5k3m5_SN[%d]: 0x%x  0x%x\n", idx, get_byte[idx], gS5k3m5_SN[idx]);
	}
}
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				*sensor_id = S5K3M5_SENSOR_ID;
				...
				if(deviceInfo_register_value == 0x00){
					register_imgsensor_deviceinfo("Cam_f", DEVICE_VERSION_S5K3M5, imgsensor_info.module_id);
