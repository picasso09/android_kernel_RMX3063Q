...
#define TORCH_DUTY 2//3
#define FLASHLIGHT_DUTY_NUM 30//31
...
static int dummy_enable(void)
{
    ...
    if(TORCH_DUTY == global_level) {
        ...
        fl_pwm_config.PWM_MODE_OLD_REGS.THRESH = global_level + 1;//global_level ;

        if(global_level < TORCH_DUTY){
            fl_pwm_config.PWM_MODE_OLD_REGS.THRESH = global_level + 2;//global_level + 1;
        }
