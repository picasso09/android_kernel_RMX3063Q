...
/*Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 add for battery name and check battery id */
	if(get_project() != 20701){
		...
		} else if ((id_volt > ATL_BATTERY_VOLTAGE_MIN) && (id_volt < ATL_BATTERY_VOLTAGE_MAX))
		{
			...
			pr_err("battery id ATL %d\n", __LINE__);
		} else {
			gm.battery_id =2;
			strcpy(battery_name, "Unknown");
			pr_err("battery id unknown\n");																}
	} else {
	/* linjiaqi@ODM.HQ.Charger 2020/09/ modified for battery_id detections in Rio */
		if ((id_volt > 0) && (id_volt < LIWINON_BATTERY_VOLTAGE_MAX_5000))
		{
			gm.battery_id =1;
			strcpy(battery_name, "Liwinon");
			register_device_proc("battery", "4.45V", "Liwinon");
			pr_err("battery id Liwinon 5000mAh %d\n", __LINE__);
		} else if ((id_volt > ATL_BATTERY_VOLTAGE_MIN_5000) && (id_volt < ATL_BATTERY_VOLTAGE_MAX_5000))
		{
			gm.battery_id =0;
			strcpy(battery_name, "ATL");
			register_device_proc("battery", "4.45V", "ATL");
			pr_err("battery id ATL 5000mAh %d\n", __LINE__);
		} else {
			gm.battery_id =2;
			strcpy(battery_name, "Unknown");
			pr_err("battery id unknown\n");
