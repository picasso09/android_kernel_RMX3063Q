...
#ifndef VENDOR_EDIT
/*hongzhenglong@ODM.HQ.BSP.CHG 2020/11/10 modify for SYS_SHUTDOWN_VOL in Rio*/
#define SYS_SHUTDOWN_VOL 3380
#else
#define SYS_SHUTDOWN_VOL 3300
#endif
#endif /*ODM_HQ_EDIT*/
...
#ifdef ODM_HQ_EDIT
extern struct charger_manager *xinfo;
extern void mtk_battery_notify_VBatFull_check(struct charger_manager *info);
#endif
...
int cust_input_limit_current[5] = {500000,900000,1200000,1500000,2200000};
#else
	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		if (val->intval == 1)
		{
			...
			pr_err("[%s] POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED 1\n", __func__);
		}
		else if (val->intval == 0)
		{
			...
			pr_err("[%s] POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED 0\n", __func__);
		}
		else
			return -EINVAL;
		break;
	...
	case POWER_SUPPLY_PROP_MMI_CHARGING_ENABLE:
		if (val->intval == 1)
		{
			if(xinfo->unwakelock_chg == 1) {
				pr_notice("[%s] unwakelock testing , this test not allowed\n", __func__);
				return -EINVAL;
			} else {
				data->BAT_mmi_charging_enable = val->intval;
				charger_dev_enable_discharge(chg_dev, false);
				pr_err("[%s] POWER_SUPPLY_PROP_MMI_CHARGING_ENABLE 1\n", __func__);
			}
		}
		else if (val->intval == 0)
		{
			if(xinfo->unwakelock_chg == 1) {
				...
				pr_err("[%s] POWER_SUPPLY_PROP_MMI_CHARGING_ENABLE 0\n", __func__);
			}
		}
		else
			return -EINVAL;
...
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/* linjiaqi@ODM.HQ.Charger 2020/10/21 modified  bat_ovp for rio*/
		if(chg_type_batovp == true && mt_get_charger_type() != 0)
			val->intval = 1;
#endif
		break;
...
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/* linjiaqi@ODM.HQ.Charger 2020/10/21 modified  bat_ovp for rio*/
		fakeoff_out_data[0] = battery_main.BAT_CAPACITY;
#else
...
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/* linjiaqi@ODM.HQ.Charger 2020/10/21 modified  bat_ovp for rio*/
	static bool bat_ovp_full = false;
	static int i_bat_ovp = 0;
	static int diff_sec;
#endif
	charger_dev_get_charger_type(chg_dev, &chg_type);

	pr_err("in monitic_soc_work init_uisoc_done = %d now_soc = %d chg_type =%d\n",init_uisoc_done ,gm.ui_soc ,chg_type);
...
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/* linjiaqi@ODM.HQ.Charger 2020/10/21 modified  bat_ovp for rio*/
	if(battery_main.BAT_CAPACITY == 100 || mt_get_charger_type() == 0) {
		if(chg_type_batovp == true) {
			i_bat_ovp = 0;
			chg_type_batovp = false;
			if(battery_main.BAT_CAPACITY == 100) {
				bat_ovp_full = true;
			}
		}
	} else if(battery_main.BAT_CAPACITY < 100) {
		bat_ovp_full = false;
	}

	if ( bat_val > 4550 && mt_get_charger_type() != 0) {
		i_bat_ovp++;
		pr_err("%s : bat_val(%d) > 4550, i_bat_ovp =%d\n", __func__, bat_val, i_bat_ovp);
		if(i_bat_ovp > 2) {
			i_bat_ovp = 3;
			now_soc = 100;
			chg_type_batovp = true;
		}
	}
#endif
...
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/* linjiaqi@ODM.HQ.Charger 2020/10/21 modified  bat_ovp for rio*/
	if (chg_type == 3 || last_full || full_status == 1 || bat_ovp_full == true)
#else
...
/* linjiaqi@ODM.HQ.Charger 2020/10/21 modified  bat_ovp for rio*/
			...
			if (diff.tv_sec >= diff_sec)
			{
				pr_err("FULL need add temp = %d old_soc = %d now_soc = %d diff = %d healthd = %d diff_sec = %d\n",temp ,old_uisoc ,now_soc ,diff.tv_sec ,battery_healthd ,diff_sec);
				battery_main.BAT_CAPACITY += 1;
				old_uisoc = battery_main.BAT_CAPACITY;
				old_time = now_time;
				power_supply_changed(battery_main.psy);
...
/* linjiaqi@ODM.HQ.Charger 2020/10/21 modified  bat_ovp for rio*/
			if (chg_type_batovp == true && mt_get_charger_type() != 0) {
				...
				if (battery_main.BAT_STATUS != POWER_SUPPLY_STATUS_FULL) {
					mtk_battery_notify_VBatFull_check(xinfo);
				}
				pr_err("charge full high or low temp\n");
				battery_main.BAT_STATUS = POWER_SUPPLY_STATUS_FULL;
				power_supply_changed(battery_main.psy);
			}
#else
...
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/* linjiaqi@ODM.HQ.Charger 2020/10/21 modified  bat_ovp for rio*/
	} else if (chg_type == 2 || (chg_type_batovp == true && mt_get_charger_type() != 0)) {
#else
	} else if (chg_type == 2) {
#endif
		if (battery_main.BAT_STATUS != POWER_SUPPLY_STATUS_CHARGING  && full_status == 0 && last_full == 0)
...
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/* linjiaqi@ODM.HQ.Charger 2020/10/21 modified  bat_ovp for rio*/
	bat_ui_soc = battery_main.BAT_CAPACITY;
#endif
	if (old_uisoc > 0)
	{
