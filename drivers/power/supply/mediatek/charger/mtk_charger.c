...
#ifdef ODM_HQ_EDIT
/*Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 add notify_code*/
#include <mtk_auxadc.h>

#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
#include <linux/thermal.h>
#include "op_charge.h"
#include "../battery/mtk_battery_internal.h"
#endif
...
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/* linjiaqi@ODM.HQ.Charger 2020/10/21 modified for bat_ovp */
#define BATTERY_VOLTAGE_MAX		4550
#else
#define BATTERY_VOLTAGE_MAX		4500
#endif
#define NTC_DISCONNECT -25
#define JEITA_COUNT 4
#define ITERM_CURRENT 2600
#define HW_BAT_FULL_VOLT 64
#define HW_FULL_COUNT 4
#endif /*ODM_HQ_EDIT*/

#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/* hongzhenglong@ODM.HQ.Charger 2020/09/19 modified for showing power-off charging picture */
#define ATL_BATTERY_VOLTAGE_MAX_5000	790000
#define ATL_BATTERY_VOLTAGE_MIN_5000	550000
#define LIWINON_BATTERY_VOLTAGE_MAX_5000    180000
#define LIWINON_BATTERY_VOLTAGE_MIN_5000    7000
#define BATTERY_VOLTAGE_LOW_3000MV_CURRENT    360000
#endif

#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/* hongzhenglong@ODM.HQ.Charger 2020/09/26 add for flashlight */
extern int oppo_flashlight_ntc_adc3_read(void);
int oppo_flashlight_temp_check(void);
#endif
...
#ifdef ODM_HQ_EDIT
struct charger_manager *xinfo = NULL;
extern struct battery_data battery_main;
#endif

#ifdef ODM_HQ_EDIT
/* hongzhenglong@ODM.HQ.Charger 2020/09/14 add /proc/batt_param_noplug sysfs */
...
#ifdef ODM_HQ_EDIT
/*Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 move all odm add Variable in hear */
...
//unsigned int esd_recovery_backlight_level = 0; /*temp modify,reason is variable not define*/
...
bool cc_cool_flags = false;
#endif  /* ODM_HQ_EDIT */
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/* linjiaqi@ODM.HQ.Charger 2020/10/21 modified  vbus_ovp for rio*/
static int i_ovp = 0;
extern bool chg_type_batovp;
extern int bat_ui_soc;
extern int temp_bat;
#endif
...
#ifdef ODM_HQ_EDIT
/* Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 modify for board_id 255 pcb current to 1000mA start*/
	...
	int idx, int input_current)
{
	struct charger_manager *info = consumer->cm;
#ifdef ODM_HQ_EDIT
/*Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 High tempture does not ok in poweroff charging */
...
#ifdef ODM_HQ_EDIT
/*Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 High tempture does not ok in poweroff charging start */
...
#ifdef ODM_HQ_EDIT
/* Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 add is first flag */
	static bool is_first = true;
/* Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 Get battery voltage */
	int vbat = battery_get_bat_voltage();
#endif  /* ODM_HQ_EDIT */
	static int cc_count = 0;
	static int cc_flags = 0;
...
/* Mengchun.Zhang@ODM.HQ.BSP.CHG.Basic 2018/12/27 Adapt software jeita */
...
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
	chr_err("[SW_JEITA] Battery Temperature: %d temp_bat: %d\n", info->battery_temp, temp_bat);
	chr_err("[SW_JEITA] Battery sw_jeita->sm: %d\n", sw_jeita->sm);
#endif
	if (info->battery_temp >= info->data.temp_t5_thres) {
		...
		if ((sw_jeita->sm == BATTERY_STATUS_WARM_TEMP)
		    && (info->battery_temp
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
			>= info->data.temp_t4_thres_minus_x_degree)&& (temp_bat >= info->data.temp_t4_thres_minus_x_degree*10)) {
#else
		...
		if ((sw_jeita->sm == BATTERY_STATUS_NORMAL)
			&& (info->battery_temp
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
			>= info->data.temp_t3_thres_minus_x_degree) && (temp_bat >= info->data.temp_t3_thres_minus_x_degree*10)) {
#else
			...
			&& (info->battery_temp
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
			<= info->data.temp_t2_thres_plus_x_degree) && (temp_bat <= info->data.temp_t2_thres_plus_x_degree*10)) {
#else
		...
		if ((sw_jeita->sm == BATTERY_STATUS_LITTLE_COLD_TEMP)
			&& (info->battery_temp
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
			<= info->data.temp_t1_thres_plus_x_degree) && temp_bat <= info->data.temp_t1_thres_plus_x_degree*10) {
#else
...
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
	} else if (info->battery_temp >= info->data.temp_t0_thres && temp_bat >= info->data.temp_t0_thres*10) {
#else
		...
		if ((sw_jeita->sm == BATTERY_STATUS_COLD_TEMP)
			&& (info->battery_temp
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
			<= info->data.temp_t0_thres_plus_x_degree) && (temp_bat <= info->data.temp_t0_thres_plus_x_degree*10)) {
#else
...
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
		if (info->battery_temp <= info->data.temp_t0_thres && temp_bat <= info->data.temp_t0_thres*10) {
#else
		if (info->battery_temp <= info->data.temp_t0_thres) {
#endif
			sw_jeita->sm = BATTERY_STATUS_COLD_TEMP;
			chr_err("[SW_JEITA] Battery Temperature between %d and %d !!\n",
				info->data.temp_neg_3_thres,
				info->data.temp_t0_thres);
		}
	} else {
...
#ifdef ODM_HQ_EDIT
/* Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 set CV/CC value after software jeita level update. */
	if (sw_jeita->sm != BATTERY_STATUS_NORMAL) {
		if (sw_jeita->sm == BATTERY_STATUS_HIGH_TEMP)
			...
			if(vbat <= 3000)
				sw_jeita->cc = BATTERY_VOLTAGE_LOW_3000MV_CURRENT;
			else if (info->chr_type == STANDARD_HOST || vbat <= 3400)
				sw_jeita->cc = info->data.jeita_temp_t1_to_t2_cc;
			else if(info->chr_type == CHARGING_HOST){
				sw_jeita->cc = info->data.jeita_temp_t7_to_t8_cc;
				//chr_err("[SW_JEITA]hzl01:newState:%d cc:%d\n",sw_jeita->sm,sw_jeita->cc);
			}
			else
				sw_jeita->cc = info->data.jeita_temp_t7_to_t8_cc;
			...
			if(vbat <= 3000)
				...
				sw_jeita->cc = info->data.jeita_temp_t6_to_t7_cc;
#else
			...
			if(vbat <= 3000)
				...
				sw_jeita->cc = 1500000;
				//chr_err("[SW_JEITA]hzl02:newState:%d cc:%d\n",sw_jeita->sm,sw_jeita->cc);
			}
#else
			if (info->chr_type == STANDARD_HOST || info->chr_type == CHARGING_HOST)
				sw_jeita->cc = info->data.jeita_temp_t1_to_t2_cc;
			...
			{
				if(vbat <= 4180 && cc_flags == 0){
					sw_jeita->cc = info->data.jeita_temp_t7_to_t8_cc;
					//chr_err("[SW_JEITA]hzl03:newState:%d cc:%d\n",sw_jeita->sm,sw_jeita->cc);
				} else if(vbat > 4180){
					sw_jeita->cc = info->data.jeita_temp_t6_to_t7_cc;
					cc_flags = 1;
					//chr_err("[SW_JEITA]hzl04:newState:%d cc:%d\n",sw_jeita->sm,sw_jeita->cc);
				} else if (cc_flags != 0){
					cc_count ++;
					sw_jeita->cc = info->data.jeita_temp_t6_to_t7_cc;
					if(cc_count >= 3){
					...
					//chr_err("[SW_JEITA]hzl05:newState:%d cc:%d \n",sw_jeita->sm,sw_jeita->cc);
				}
				//chr_err("[SW_JEITA]hzl06:newState:%d cc:%d cc_flags:%d cc_count:%d\n",sw_jeita->sm,sw_jeita->cc,cc_flags,cc_count);
			} else {
			...
			if(vbat <= 3000)
				sw_jeita->cc = BATTERY_VOLTAGE_LOW_3000MV_CURRENT;
			else if (info->chr_type == STANDARD_HOST || info->chr_type == CHARGING_HOST)
		...
		if(vbat <= 3000)
			...
			sw_jeita->cc = info->data.charging_host_charger_current;
#else
		if (info->chr_type == STANDARD_HOST || info->chr_type == CHARGING_HOST)
			sw_jeita->cc = info->data.jeita_temp_t1_to_t2_cc;
#endif
		...
		if (!ret) {
			...
			if (info->battery_temp != tmp
				...
				chr_err("%s: %ld %s tmp:%d %d chr:%d\n",
					...
					mt_get_charger_type());
#ifdef ODM_HQ_EDIT
/*Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 add for charger*/
				...
				enum charger_type chr_type)
{
	...
	cc_cool_flags = false;
#ifdef ODM_HQ_EDIT
/* Liu.Yong@RM.CM.BSP.CHG.Basic 2020.06.22 modify for lcd on off current set*/
	last_backlight_state = -1;
	info->sw_jeita.sm = BATTERY_STATUS_INVALID;
	battery_main.BAT_mmi_charging_enable = 1;
#endif /*ODM_HQ_EDIT*/
#ifdef ODM_HQ_EDIT
	switch_usb_state(info->chr_type);
#endif
...
#ifdef ODM_HQ_EDIT
/* Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 modify for poweroff mode battery temp monitor start*/
	__pm_stay_awake(&mtk_charger_wake_lock);
/* Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 modify for poweroff mode battery temp monitor end*/
/*Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 add for vbus check*/
	notify_check = 1;
...
#ifdef ODM_HQ_EDIT
	switch_usb_state(info->chr_type);
	battery_main.BAT_mmi_charging_enable = 0;
#endif
#ifndef ODM_HQ_EDIT
/*Liu.Yong@RM.CM.BSP.CHG.Basic 2020.06.08 modify for charging aicl, use sw aicl*/
...
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/* linjiaqi@ODM.HQ.Charger 2020/10/21 modified  vbus_ovp for rio*/
	if (vchr > info->data.max_charger_voltage && i_ovp >2) {
...
#ifdef ODM_HQ_EDIT
/*Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 modify for notify code*/
...
static void mtk_battery_notify_VBatId_check(struct charger_manager *info)
{
	...
	//pr_err("battery id: id_volt:%d \n",id_volt);
	if(get_project() != 20701){
		if ((id_volt > SDI_BATTERY_VOLTAGE_MIN) &&  (id_volt < SDI_BATTERY_VOLTAGE_MAX) && info->battery_temp > NTC_DISCONNECT)
		{
			info->notify_code &= ~CHG_BAT_ID_STATUS;
			pr_err("battery id:SDI id_volt:%d \n",id_volt);
		} else if ((id_volt > ATL_BATTERY_VOLTAGE_MIN) && (id_volt < ATL_BATTERY_VOLTAGE_MAX) && info->battery_temp > NTC_DISCONNECT)
		{
			info->notify_code &= ~CHG_BAT_ID_STATUS;
		} else {
			info->notify_code |= CHG_BAT_ID_STATUS;
			pr_err("battery id:others id_volt:%d \n",id_volt);
			mtk_chgstat_notify(info);
		}
	} else {
/* hongzhenglong@ODM.HQ.Charger 2020/09/19 modified for showing power-off charging picture */
		if ((id_volt > ATL_BATTERY_VOLTAGE_MIN_5000) && (id_volt <  ATL_BATTERY_VOLTAGE_MAX_5000) && info->battery_temp > NTC_DISCONNECT)
		{
			info->notify_code &= ~CHG_BAT_ID_STATUS;
			pr_err("battery id:ATL id_volt:%d \n",id_volt);
		} else if ((id_volt > LIWINON_BATTERY_VOLTAGE_MIN_5000) &&  (id_volt < LIWINON_BATTERY_VOLTAGE_MAX_5000) && info->battery_temp > NTC_DISCONNECT){
			info->notify_code &= ~CHG_BAT_ID_STATUS;
			pr_err("battery id:LIWINON id_volt:%d \n",id_volt);
		} else {
			info->notify_code |= CHG_BAT_ID_STATUS;
			pr_err("battery id:others id_volt:%d \n",id_volt);
			mtk_chgstat_notify(info);
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
			/* linjiaqi@ODM.HQ.Charger 2020/10/10 non-battery&&disconnect id-pin discharging */
			_charger_manager_enable_charging(info->chg1_consumer,0, false);
#endif
		}
	}
...
void mtk_battery_notify_VBatFull_check(struct charger_manager *info)
{
	bool chg_done = false;
	charger_dev_is_charging_done(info->chg1_dev, &chg_done);
	if (chg_done || last_full) {
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/* linjiaqi@ODM.HQ.Charger 2020/10/21 modified  bat_ovp for rio*/
		if(temp_bat > 450) {//temp_bat
	...
	mtk_chgstat_notify(info);
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
	chr_err("chg_done = %d full_status = %d last_full = %d bat_ui_soc = %d\n", chg_done, full_status, last_full, bat_ui_soc);
#else
...
{
	int bat_current;
	//static bool hw_full_flag = false;
	static int hw_full_counts = 0;
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/* linjiaqi@ODM.HQ.Charger 2020/10/21 modified  bat_ovp for rio*/
	if (chg_type_batovp == true) {
		return;
	}
#endif
	...
	if(get_project() == 20701) {
		if (bat_volt >= full_bat_volt) {
			if (bat_current > 0 && bat_current <= ITERM_CURRENT) {
				vchg_full_flag++;
				rechg_flag = 0;
				chr_err("[Jeita full SW][%s][%d] info->sw_jeita.cv = %d bat_current = %d vchg_full_flag = %d bat_volt = %d last_full = %d\n",
							__func__, __LINE__, info->sw_jeita.cv, bat_current, vchg_full_flag, bat_volt, last_full);
				if (vchg_full_flag >= JEITA_COUNT) {
					info->sw_jeita.charging = false;
					charger_dev_enable(chg_dev, false);
					last_full = true;
					vchg_full_flag = 0;
					chr_err("[Jeita full SW][%s][%d] bat_current(%d) < ITERM_CURRENT(250)\n", __func__, __LINE__, bat_current);
					return;
				 }
			} else if(bat_current <= 0) {
				vchg_full_flag++;
				rechg_flag = 0;
				chr_err("[Jeita full SW][%s][%d] info->sw_jeita.cv = %d bat_current = %d vchg_full_flag = %d bat_volt = %d last_full = %d\n",
							__func__, __LINE__, info->sw_jeita.cv, bat_current, vchg_full_flag, bat_volt, last_full);
				if (vchg_full_flag >= JEITA_COUNT * 2) {
					info->sw_jeita.charging = false;
					charger_dev_enable(chg_dev, false);
					last_full = true;
					vchg_full_flag = 0;
					chr_err("[Jeita full SW][%s][%d] bat_current <= 0\n", __func__, __LINE__);
					return;
				 }
			}
		} else if ((bat_volt < (full_bat_volt - vbat_diff)) && (last_full == true)) {
			rechg_flag++;
			vchg_full_flag = 0;
			chr_err("[Jeita full rechg check][%s][%d] info->sw_jeita.cv = %d bat_current = %d rechg_flag = %d bat_volt = %d last_full = %d\n",
						__func__, __LINE__, info->sw_jeita.cv, bat_current, rechg_flag, bat_volt, last_full);
			if (rechg_flag >= JEITA_COUNT) {
				if (last_full) {
					info->sw_jeita.charging = true;
					charger_dev_enable(chg_dev, true);
					last_full = false;
					chr_err("[Jeita full rechg check][%s][%d] start rechg\n", __func__, __LINE__);
				}
				rechg_flag = 0;
			}
		}
		if (bat_volt >= (full_bat_volt + HW_BAT_FULL_VOLT)) {
			hw_full_counts++;
			if (hw_full_counts >= HW_FULL_COUNT) {
				//hw_full_flag = true;
				hw_full_counts = 0;
				info->sw_jeita.charging = false;
				charger_dev_enable(chg_dev, false);
				last_full = true;
				chr_err("[Jeita full HW][%s][%d] bat_volt:%d >= %d\n", __func__, __LINE__, bat_volt, full_bat_volt + HW_BAT_FULL_VOLT);
				return;
			}
		} else {
			hw_full_counts = 0;
			/*if(hw_full_flag == true) {
				hw_full_flag = false;
				info->sw_jeita.charging = true;
				charger_dev_enable(chg_dev, true);
				last_full = false;
...
static void update_vchg_work(struct work_struct *work)
{
	...
	/* wangtao@ODM.HQ.Charger 2020/10/10 modify rio charge cv voltage */
	if(get_project() == 20701) {
		if (info->chr_type != CHARGER_UNKNOWN) {
			switch (info->sw_jeita.sm) {
				case BATTERY_STATUS_COLD_TEMP:
					get_3_times_set_jeita(info, chg_dev, bat_volt, 3925, 3993, 245);
					break;
				case BATTERY_STATUS_LITTLE_COLD_TEMP:
					get_3_times_set_jeita(info, chg_dev, bat_volt, 4380, 4450, 150);
					break;
				case BATTERY_STATUS_LITTLE_COOL_TEMP:
				case BATTERY_STATUS_NORMAL:
					get_3_times_set_jeita(info, chg_dev, bat_volt, 4380, 4450, 50);
					break;
					...
					if (info->chr_type != STANDARD_HOST) {
						if (bat_volt >= 4180) {
							...
							if (set_540cc_flag >= JEITA_COUNT) {
								info->sw_jeita.cc = info->data.jeita_temp_t6_to_t7_cc;//660
								set_540cc_flag = 0;
								cc_540 = true;
								cc_960 = false;
							...
							if (set_960cc_flag >= JEITA_COUNT) {
								info->sw_jeita.cc = info->data.jeita_temp_t7_to_t8_cc;//1380
								set_960cc_flag = 0;
								cc_540 = false;
								cc_960 = true;
								chr_err("[Maple Test] [%s][%d] info->sw_jeita.cc=%d cc_540=%d cc_960=%d\n",
				...
				case BATTERY_STATUS_WARM_TEMP:
					get_3_times_set_jeita(info, chg_dev, bat_volt, 4075, 4150, 45);
					break;
			}
		} else {
			last_full = false;
...
#ifdef ODM_HQ_EDIT
/*Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 add for notify code*/
...
static void charger_check_status(struct charger_manager *info)
{
	...
	#ifdef ODM_HQ_EDIT
	/* hongzhenglong@ODM.HQ.Charger 2020/09/14 add /proc/batt_param_noplug sysfs */
		noplug_temperature = info->battery_temp * 10;
		noplug_batt_volt_max = battery_get_bat_voltage();
		noplug_batt_volt_min = battery_get_bat_voltage();
	#endif
		return;
	}
...
stop_charging:
#ifdef ODM_HQ_EDIT
/*Liu.Yong@RM.CM.BSP.CHG.Basic 2020.06.08 modify for charging notify check*/
	if (notify_check == 1)
				...
				struct device *dev)
{
	...
	/* common */
	if(get_project() == 20701) {
		info->data.battery_cv = 4432000;
		chr_err("rio battery_cv :%d\n", info->data.battery_cv);
	} else {
	...
	/* Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 modify for board_id 255 pcb current to 1000mA end*/
	#endif
...
/* Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 Adapt software jeita level */
	if (of_property_read_u32(np, "jeita_temp_t6_to_t7_cc", &val) >= 0)
		info->data.jeita_temp_t6_to_t7_cc = val;
	else {
		chr_err("use default JEITA_TEMP_T6_TO_T7_CC:%d\n",
			JEITA_TEMP_T6_TO_T7_CC);
		info->data.jeita_temp_t6_to_t7_cc = JEITA_TEMP_T6_TO_T7_CC;
	}
	if (of_property_read_u32(np, "jeita_temp_t7_to_t8_cc", &val) >= 0)
		info->data.jeita_temp_t7_to_t8_cc = val;
	else {
		chr_err("use default JEITA_TEMP_T7_TO_T8_CC:%d\n",
			JEITA_TEMP_T7_TO_T8_CC);
		info->data.jeita_temp_t7_to_t8_cc = JEITA_TEMP_T7_TO_T8_CC;
	}
...
#ifdef ODM_HQ_EDIT
/* Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 Adapt software jeita */
...
#ifdef ODM_HQ_EDIT
/* Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 Modify software jeita data */
...
#ifndef ODM_HQ_EDIT
/*Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.16 remove for notify code*/
...
#ifdef ODM_HQ_EDIT
/*Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 add runin control API*/
ssize_t charger_cycle_proc_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
	...
	{
#ifdef VENDOR_EDIT
/*hongzhenglong@ODM.HQ.BSP.CHG 2020/10/09 Add for usb wakelock*/
		if(pinfo->unwakelock_chg == 1){
...
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/*hongzhenglong@ODM.HQ.BSP.CHG 2020/09/21 add for runin control API*/
	if( !strncmp(kernel_buf, "dis808", 6) )
#else
...
#ifdef VENDOR_EDIT
/*hongzhenglong@ODM.HQ.BSP.CHG 2020/10/09 Add for usb wakelock*/
	//chr_err("runin---hzl003:%d\n",chr_type);
	if( !strncmp(kernel_buf, "wakelock", 8) ){
		pr_err( "set wakelock.\n");
		pinfo->unwakelock_chg = 0;
		if(chr_type == NONSTANDARD_CHARGER){
			pr_err( "set wakelock_nonstandard_charger.\n");
			mt_usb_connect();
		}
		__pm_stay_awake(&mtk_charger_wake_lock);
	...
	if(!strncmp(kernel_buf, "unwakelock", 10)){
		pr_err( "set unwakelock.\n");
		pinfo->unwakelock_chg = 1;
		charger_dev_enable(chg_dev, false);
		charger_dev_enable_discharge(chg_dev, true);
		
		__pm_relax(&mtk_charger_wake_lock);
		if(chr_type == NONSTANDARD_CHARGER){
			pr_err( "set unwakelock_nonstandard_charger.\n");
			mt_usb_disconnect();
		}
	}
	//unwakelock_chg_tmp = pinfo->unwakelock_chg;
#endif
#ifdef HQ_COMPILE_FACTORY_VERSION
...
static void tbatt_power_off_work(struct work_struct *work)
{
	...
	int batt_temp = 0;
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/* hongzhenglong@ODM.HQ.Charger 2020/09/26 add for flashlight NTC in Rio */
	int ret_temp = 0;
#endif
...
/* hongzhenglong@ODM.HQ.Charger 2020/09/26 add for flashlight NTC in Rio */
	if(get_project() == 20701){
		ret_temp = oppo_flashlight_temp_check();
	} else {
		ret_temp = 0;
	}
//pr_err(" oppo_flashlight_temp_check_Hzl01\n");
#endif
	chr_err("[oppo_chg_print_log]Vbat=%d,Ibat=%d,I=%d,VChr=%d,batt_temp=%d,Soc:UI_soc=%d:%d,CT:%d:%d hv:%d pd:%d:%d\n",
		battery_get_bat_voltage(), battery_get_bat_current(), pmic_get_charging_current(),
		battery_get_vbus(), battery_get_bat_temperature(),
		battery_get_soc(), battery_get_uisoc(),
		mt_get_charger_type(), info->chr_type,
		info->enable_hv_charging, info->pd_type,
...
void charger_proc_init(void)
{
	...
	charger_cycle = proc_create("charger_cycle", 0x0666, NULL, &charger_cycle_proc_fops);
	if (!charger_cycle) {
		pr_err("proc_create  fail!\n");
	}
}
...
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/*hongzhenglong@ODM.HQ.BSP.CHG 2020/09/22 add for linmitted charging current when backlight is on in Rio*/
	int bat_temp = 0;
	static int backlight_current_level = CHARGER_CURRENT_LIMET_2000MA;
#endif
	struct charger_device *chg_dev = get_charger_by_name("primary_chg");
...
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/* linjiaqi@ODM.HQ.Charger 2020/10/21 modified  bat_ovp for rio*/
	} else if(bat_volt <= 4370 && vbat_state == 1)
#else
...
/* linjiaqi@ODM.HQ.Charger 2020/10/21 modified  vbus_ovp for rio*/
		i_ovp++;
		pr_err("vbus_check VBUS: IS TO HIG i =%d\n",i_ovp);
		if(i_ovp > 2)
		{
			i_ovp = 3;
			vbus_state = 1;
			charger_dev_enable_discharge(chg_dev, true);
			pr_err("vbus_check VBUS:%d IS TO HIG, DISCHARGGIN! i =%d\n", vbus_vlot, i_ovp);
			schedule_delayed_work(&info->vbus_check, msecs_to_jiffies(3000));
			return;
		}
#else
...
/* linjiaqi@ODM.HQ.Charger 2020/10/21 modified  vbus_ovp for rio*/
		...
		{
			if (vbus_vlot <= 4400)
				input_current_aicl(4300);
		}
		else if (vbus_vlot <= 4400) {
			input_current_aicl(4300);
		}
	}

/*linjiaqi@ODM.HQ.BSP.CHG 2020/09/25 modify for limmitted chargint current when bakclight is on in Rio*/
	if(get_project() == 20701){
		if ((esd_recovery_backlight_level > 0 || call_mode == 1) /*&& last_backlight_state != 1*/)
		{
			...
			if ( bat_temp < 35){            //<35
				if(input_limit_level > CHARGER_CURRENT_LIMET_2000MA || backlight_current_level == CHARGER_CURRENT_LIMET_1200MA || backlight_current_level == CHARGER_CURRENT_LIMET_1500MA || backlight_current_level == CHARGER_CURRENT_LIMET_500MA){
					backlight_current_level = CHARGER_CURRENT_LIMET_2000MA;
					input_current_aicl(4350);
					input_limit_level = CHARGER_CURRENT_LIMET_2000MA;
					chr_err("[vbus_check_work]LED_ON,bat_temp:%d <35,charging current 2A\n",bat_temp);
				}
			} else if(bat_temp < 37){ //35<=bat_temp<37
				if(input_limit_level > CHARGER_CURRENT_LIMET_1500MA || backlight_current_level == CHARGER_CURRENT_LIMET_1200MA || backlight_current_level == CHARGER_CURRENT_LIMET_500MA){
					backlight_current_level = CHARGER_CURRENT_LIMET_1500MA;
					input_current_aicl(4350);
					input_limit_level = CHARGER_CURRENT_LIMET_1500MA;
					chr_err("[vbus_check_work] LED_ON,35<= bat_temp:%d <37,charging current 1.5A\n",bat_temp);
				}
			} else if(bat_temp < 45){                      //37<=bat_temp<45
				if(input_limit_level > CHARGER_CURRENT_LIMET_1200MA || backlight_current_level == CHARGER_CURRENT_LIMET_500MA){
					backlight_current_level = CHARGER_CURRENT_LIMET_1200MA;
					input_current_aicl(4350);
					input_limit_level = CHARGER_CURRENT_LIMET_1200MA;
					chr_err("[vbus_check_work] LED_ON,37<= bat_temp:%d <45,charging current 1.2A\n",bat_temp);
				}
			} else {
				if(input_limit_level > CHARGER_CURRENT_LIMET_500MA){
					backlight_current_level = CHARGER_CURRENT_LIMET_500MA;
					input_current_aicl(4350);
					input_limit_level = CHARGER_CURRENT_LIMET_500MA;
					chr_err("[vbus_check_work] LED_ON,45<= bat_temp:%d,charging current 0.5A\n",bat_temp);
				}
			}

			if(call_mode == 1){
				input_limit_level = CHARGER_CURRENT_LIMET_1200MA;
				chr_err("[vbus_check_work] LED_ON,In call_mode\n");
			}
...
int oppo_flashlight_temp_check(void)
{
	...
	int i = 0;
	//int notify = 0;
	int flashlight_temp = 0;
	int flashlight_volt = 0;
	int flashlight_separate = 0;

	flashlight_separate = 49;

	usbtemp_volt = oppo_flashlight_ntc_adc3_read();//get flashlight volt
	if (usbtemp_volt <= 0) {
		usbtemp_volt = USB_25C_VOLT;
	}
	flashlight_volt = usbtemp_volt;
	for (i = ARRAY_SIZE(con_volt_100K) - 1; i >= 0; i--) {
		if (con_volt_100K[i] >= flashlight_volt)
			break;
		else if (i == 0)
			break;
	}
	flashlight_temp = con_temp_100K[i];
	//if(flashlight_temp < 47)
		//notify = pinfo->notify_code;

	if(flashlight_temp >= flashlight_separate)
		pinfo->notify_code |= 1 << NOTIFY_FLASHLIGHT_OVER_TEMP;
	else
		pinfo->notify_code &= ~(1 << NOTIFY_FLASHLIGHT_OVER_TEMP);
	my_notify_code = pinfo->notify_code;
	mtk_chgstat_notify(pinfo);
	printk(KERN_ERR "[OPPO_CHG][%s]: flashlight_temp = %d,notify_code =%d\n", __func__,flashlight_temp,pinfo->notify_code);
	return flashlight_temp;
}

static int get_flashlight_ntc_temp(struct thermal_zone_device *tz,
					int *temp)
{
    *temp = 1000 * oppo_flashlight_temp_check();
	//printk("Hzl flashlight_ntc temp:%d\n ",*temp);
	return 0;
}
...
{
	chr_err("oppo_get_chg_unwakelock_hzl");
	if (!pinfo) {
                return 0;
	} else {
		return pinfo->unwakelock_chg;
	}
}
#endif

struct thermal_zone_device_ops flashlightntc_thermal_zone_ops = {
	.get_temp = get_flashlight_ntc_temp,
};
#endif
...
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/* hongzhenglong@ODM.HQ.Charger 2020/09/26 add for flashlight NTC in Rio */
	info->tzd = thermal_zone_device_register("mp2762_ntc",
			0, 0, info, &flashlightntc_thermal_zone_ops, NULL, 0, 0);
 	if (IS_ERR_OR_NULL(info->tzd)) {
	...
	_wake_up_charger(info);
#if defined(ODM_HQ_EDIT) && defined(CONFIG_MACH_MT6765)
/*hongzhenglong@ODM.HQ.BSP.CHG 2020/11/11 modify for factory otg node*/
	if(get_boot_mode() == FACTORY_BOOT){
		otg_switch_mode(1);
		pr_err("enter factory_mode:otg switch 1\n");
	}
#endif
	return 0;
}
...
static int mtk_charger_remove(struct platform_device *dev)
{
	...
	if(info){
		...
		thermal_zone_device_unregister(info->tzd);
		kfree(info);
	}
#endif
	return 0;
