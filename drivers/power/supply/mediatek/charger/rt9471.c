...
static int rt9471_bc12_postprocess(struct rt9471_chip *chip)
{
	...
	if (chip->dev_id != RT9470D_DEVID && chip->dev_id != RT9471D_DEVID && chip->dev_id != RT9471_DEVID)
		return 0;
...
static int rt9471_init_setting(struct rt9471_chip *chip)
{
	...
	if (ret < 0)
		dev_notice(chip->dev, "%s set mivrtrack fail(%d)\n",
				      __func__, ret);
#ifndef ODM_HQ_EDIT
/* hongzhenglong@ODM.HQ.Charger 2020/12/11 disable safe timer charging in Rio */
	ret = __rt9471_enable_safe_tmr(chip, desc->en_safe_tmr);
#else
	ret = __rt9471_enable_safe_tmr(chip, false);
#endif
	if (ret < 0)
...
static int oppo_rt9471_get_charger_type(struct charger_device *chg_dev,
							  u32 *charger_type)
{
	struct rt9471_chip *chip = dev_get_drvdata(&chg_dev->dev);
	enum rt9471_ic_stat ic_stat = RT9471_ICSTAT_SLEEP;
	int ret;
	static int old_ic_stat = -1;
...
static int oppo_rt9471_recharger(struct charger_device *chg_dev,bool flag)
{
	u8 val;
	struct rt9471_chip *chip = dev_get_drvdata(&chg_dev->dev);
	if (flag)
		val = RT9471_VRECHG_200MV << RT9471_VRECHG_SHIFT;
	else
		val = RT9471_VRECHG_100MV << RT9471_VRECHG_SHIFT;

	return rt9471_i2c_update_bits(chip, RT9471_REG_VCHG,
			val,
			RT9471_REG07_VRECHG_MASK);
}
...
static struct charger_ops rt9471_chg_ops = {
	...
	/* recharger*/
	.recharger = oppo_rt9471_recharger,
