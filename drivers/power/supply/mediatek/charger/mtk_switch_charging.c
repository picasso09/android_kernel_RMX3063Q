...
static void swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	if (mtk_is_TA_support_pd_pps(info)) {
		...
		chr_err("CHARGING_HOST pdata->input_current_limit =%d,pdata->charging_current_limit =%d",pdata->input_current_limit,pdata->charging_current_limit);
...
#ifdef ODM_HQ_EDIT
/* Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 Modify sw jeita level */
...
/*Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 add cust control API*/
	...
	chr_err("force:%d thermal:%d,%d pe4:%d,%d,%d setting:%d %d type:%d usb_unlimited:%d usbif:%d usbsm:%d aicl:%d %d atm:%d jeita_cc:%d\n",
		...
		info->atm_enabled,info->sw_jeita.cc);
	...
	if (pdata->input_current_limit > aicr1_min &&
		...
		chr_err("%s: pdata->charging_current_limit, info->can_charging %d\n", __func__, info->can_charging);
...
static bool mtk_switch_check_charging_time(struct charger_manager *info)
{
	...
	/*wangtao@ 2020.11.23 aging mode disable charing time*/	
	if(get_eng_version() == 1) {
		printk("mtk_switch_check_charging_time_is_good_aging_mode\n");
		return true;
	}
#ifdef ODM_HQ_EDIT
...
static int mtk_switch_chr_cc(struct charger_manager *info)
{
	...
	if (chg_done || last_full) {
		...
		chr_err("battery full! chg_done:%d, last_full:%d\n", chg_done, last_full);
	}
#else
	if (chg_done) {
		swchgalg->state = CHR_BATFULL;
	...
	if (info->enable_sw_jeita) {
		if(get_project() == 20701){
			...
			if ((info->sw_jeita.error_recovery_flag == false) &&
				(info->sw_jeita.sm != BATTERY_STATUS_LOW_TEMP) &&
				(info->sw_jeita.sm != BATTERY_STATUS_HIGH_TEMP)) {
...
/* Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 Modify sw jeita level */
	...
	chr_err("%s :swchgalg->state =%d, sw_jeita.sm =%d\n", __func__, swchgalg->state, info->sw_jeita.sm);
	swchgalg->total_charging_time = 0;
...
static int mtk_switch_charging_run(struct charger_manager *info)
{
	...
	ret = mtk_switch_check_charging_time(info);
	if(!ret)
		charger_dev_enable_discharge(info->chg1_dev, true);
...
#ifdef ODM_HQ_EDIT
/*Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 modify for charger*/
		charger_manager_notifier(info, CHARGER_NOTIFY_EOC);
#else /*ODM_HQ_EDIT*/
	...
	case CHARGER_DEV_NOTIFY_VBUS_OVP:
		...
		break;
#ifdef ODM_HQ_EDIT
/*Liu.Yong@RM.CM.BSP.CHG.Basic 2020.05.15 add for charger notify*/
