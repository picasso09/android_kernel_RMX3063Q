...
#ifdef VENDOR_EDIT
/*Peifeng.Li@PERFORMANCE.BSP.MM, 2020/09/15, support almk by vmpressure*/
				   ...
				   unsigned long action, void *data)
{
	...
	if (pressure >= 95) {
		...
		if (other_file + other_free < totalram_pages/almk_totalram_ratio) {
			atomic_set(&shift_adj, 1);
		}
...
#ifdef VENDOR_EDIT
/*gongpeicai@ODM.HQ.Android, 2021/04/25
	skip kill some process, because those process restart immediately.*/
		if ((strstr(tsk->comm, ".statistics.rom") != NULL)
		    || (strstr(tsk->comm, ".coloros.athena") != NULL)){
			//lowmem_print(1, "skip killing process '%s' (%d), adj %hd\n", tsk->comm, tsk->pid, selected_oom_score_adj);
			continue;
		}
#endif
...
#ifdef VENDOR_EDIT
/*Peifeng.Li@PERFORMANCE.BSP.MM, 2020/09/15
	it will not to use lowmem_deathpending_timeout in 2G/3G to speed memory-recleaim-rate up*/
			time_before_eq(jiffies, lowmem_deathpending_timeout) && totalram_pages >= TOTALRAM_4GB) {
#else
